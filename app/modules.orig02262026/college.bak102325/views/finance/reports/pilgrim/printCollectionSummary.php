...
foreach($collection->result() as $c):
        // if($c->t_type!=5 && $c->t_type!=2 && $c->t_type!=3):
        if($c->t_type!=2 && $c->t_type!=3 && $c->t_type != 4):
            $finItem = $c->t_charge_id;
            $cash = Modules::run('college/finance/getCashPayments', $from, $to, ($c->fused_category==0?$finItem:NULL), $c->fused_category, $c->t_school_year, $c->t_sem);
            if (!in_array($finItem, $finItems)) {
                
                $totalCash += $cash->amount;

                $cheque = Modules::run('college/finance/getChequePayments', $from, $to, ($c->fused_category==0?$c->item_id:NULL), $c->fused_category, $c->t_school_year, $c->t_sem);
                foreach($cheque as $chk):
                    $totalCheque += $chk->t_amount;
                    $totalChequePayments += $chk->t_amount;
                endforeach;   
                $totalCollection = $cash->amount + $totalCheque;
                ...
                $pdf->MultiCell(40, 3, number_format($cash->amount,2,'.',','),0, 'R', 0, 0, '', '', true, 0, false, true, 10, 'M');
                $pdf->MultiCell(5, 3, '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
...
endforeach;
        ...
        $chequePayments = Modules::run('college/finance/getChequePayments', $from, $to, NULL, NULL, $school_year, $semester);
        $cpCount = 0;
