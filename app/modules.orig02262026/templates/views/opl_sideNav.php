<aside class="main-sidebar sidebar-dark-primary elevation-4">
  ...
  <?php if (!$this->session->isParent):
    ...
    // endif;
    if ($this->session->avatar != NULL || $this->session->avatar != ""):
      if (file_exists('uploads/' . $this->session->avatar)):
        $avatar = site_url('uploads/' . $this->session->avatar);
      else:
        $avatar = site_url('images/avatar/' . ($this->session->sex == 'Female' ? 'female.png' : 'male.png'));
      endif;
    else:
      $avatar = site_url('images/avatar/' . ($this->session->sex == 'Female' ? 'female.png' : 'male.png'));
    endif;
  ...
  <?php else: ?>
    <div class="sidebar">
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="info">
          <a href="<?php echo base_url() . 'opl/p/viewParentDetails' ?>" class="d-block">Hi&nbsp;<?php echo ucfirst($this->session->username) ?>&nbsp;!</a>
          <!-- <a href="<?php // echo base_url() . 'opl/p/dashboard' 
                        ?>" class="d-block">Hi&nbsp;<?php // echo ucfirst($this->session->username) 
                                                    ?>&nbsp;!</a> -->
        </div>
      </div>
