...
foreach ($transaction->result() as $i => $t):
    if ($t->t_type != 2 && $t->t_type != 3):
        ...
        if ($t->ref_number != $lastOR):
            ...
            if (strpos($t->item_description, 'Textbooks') !== false || strpos($t->item_description, 'Textbook') !== false):
                $pdf->SetXY($b8x, $b8y);
                $pdf->MultiCell(20, 0, $t->t_date, 1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$pdf->MultiCell(18, 0, '', 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
$pdf->MultiCell(15, 0, number_format((($reg - $totalreg) + ($misc - $totalmisc) + ($tuitionFee - $totalTuition)), 2, '.', ','), 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
