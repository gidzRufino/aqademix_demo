...
foreach ($charges as $c):
    ...
    if (strpos($c->item_description, 'Textbooks') !== false || strpos($c->item_description, 'book') !== false || strpos($c->item_description, 'Book') !== false || strpos($c->item_description, 'quipper') !== false):
        $booksFee += $c->amount;
...
foreach ($transaction->result() as $i => $t):
    if ($t->t_type != 2 && $t->t_type != 3):
        ...
        foreach ($tBuks as $b):
            if ($b->t_charge_id == 258130 || $b->t_charge_id == 268314 || $b->t_charge_id == 245781 || $b->t_charge_id == 254538):
                $aa++;
...
//            $pdf->MultiCell(18, 0, ($paid_total == 0 ? '' : number_format($paid_total, 2, '.', ',')), 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
            ...
            $pdf->MultiCell(15, 0, '', 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
            // books //
            if ($student->grade_level_id >= 5 && $student->grade_level_id <= 7):
                if ($t->t_charge_id == 258130 || $t->t_charge_id == 268314 || $t->t_charge_id == 245781 || $t->t_charge_id == 254538):
                    $pdf->SetXY($b8x, $b8y);
                    $pdf->MultiCell(20, 0, $t->t_date, 1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
                    ...
                    if ($aa == 1 && $xx == 1):
                        $amt_paid += $amt_total;
                        $pdf->MultiCell(21, 0, number_format(($booksFee - $amt_paid), 2, '.', ','), 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
//                    else:
//                        $pdf->MultiCell(21, 0, '', 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
                    endif;
                endif;
...
$pdf->MultiCell(18, 0, number_format(($totalreg + $totalmisc + $totalTechFee + $totalTuition), 2, '.', ','), 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$pdf->MultiCell(18, 0, '', 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
$pdf->MultiCell(15, 0, number_format((($reg - $totalreg) + ($misc - $totalmisc) + ($techFee - $totalTechFee) + ($tuitionFee - $totalTuition)), 2, '.', ','), 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$pdf->MultiCell(18, 0, (($techFee - $totalTechFee) == 0 ) ? '-' : number_format($techFeeMonthly_nxt, 2, '.', ','), 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$pdf->MultiCell(14, 0, '', 1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
$pdf->MultiCell(21, 0, (($subtot + $arrearsBooks + $arrears) < 0 ? '0.00' : number_format($subtot + $arrearsBooks + $arrears, 2, '.', ',')), 1, 'L', 0, 0, '', '', true, 0, false, true, 0, 'M');
