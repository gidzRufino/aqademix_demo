...
class Finance extends MX_Controller
{
    ...
    // Disbursement Module Start Here!!!!
...
?>
        ...
        <?php
            ...
            $chid = base64_decode($this->input->post('usid', true));
            $status = $this->input->post('statid', true);
            $ch_status = 3;
            switch ($status) {
                case 'rlstat':
                    $ch_status = 3;
            ...
            $response = $this->finance_model->update_ds($data, $chid, 'ds_cheque', 'chid');
            // $baid = $this->finance_model->update_ds($account, $proc, 'ds_bank_account', 'baid');
            $success = $response > 0;
            ...
            echo json_encode([
                ...
                'response' => $response,
                'success' => $success,
                'message' => $success ? 'Transaction status updated successfully.' : 'No updates were made.',
            ]);
        }
    }
    ...
    function search()
    {
        ...
        <?php
            ...
            $process = substr($this->input->post('proc'), 1);
            $success = false;
            if ($search) {
                $success = true;
                switch ($process) {
                    case 'disburse':
                        $items = $this->finance_model->disbursements(null, null, $search);
                        $tbody = "";
                        $tfooter = "";
                        if ($items->num_rows() > 0) {
                            $i = 0;
                            foreach ($items->result() as $it) {
                                $i++;
                                $tbtn = '
                           <button type="button" id="ed' . base64_encode($it->diid) . '" onclick="eddis(this.id)" class="btn btn-info edbtn btn-xs"><i class="fa fa-pencil"></i></button>
                           <button type="button" id="dd' . base64_encode($it->diid) . '" onclick="dldis(this.id)" class="btn btn-danger delbtn btn-xs"><i class="fa fa-trash"></i></button>
                           ';
                                $tbody = $tbody .
                                    "<tr>
                           <td class='text-center'>" . str_pad($it->diid, 7, "0", STR_PAD_LEFT) . "</td>
                           <td class='text-center'>" . $it->di_description . "</td>
                           <td class='text-center'>" . $it->di_datetime . "</td>
                           <td class='text-center'>" . number_format($it->di_amount, 2) . "</td>
                           <td class='text-center'>" . $it->ve_name . "</td>
                           <td class='text-center'>" . $it->pt_type . "</td>
                           <td class='text-center'>" . $it->di_auth_id . "</td>
                           <td class='text-center " . $it->ds_color . "'>" . $it->ds_status . "</td>
                           <td class='text-center'>" . $tbtn . "</td>
                        </tr>
                        ";
                            }
                            $pending = $this->finance_model->disburse(null, 1, null, $search);
                            $approved = $this->finance_model->disburse(null, 2, null, $search);
                            $released = $this->finance_model->disburse(null, 3, null, $search);
                            $overall = $this->finance_model->disburse(null, null, null, $search);
                            $tfooter = "
                     <div class='col-md-3 col-sm-12 col-xs-12'>
                        <div class='info-box clickable' id='num_properties'>
                           <span class='info-box-icon bg-aqua dicon'><i class='fa fa-clock'></i></span>
                           <div class='info-box-content'>
                              <span class='info-box-text'>" . $pending->row()->count . " Pending</span>
                              <span class='info-box-number'><small>PhP</small> " . number_format($pending->row()->amount, 2) . "</span>
                           </div>
                        </div>
                      </div>
                      <div class='col-md-3 col-sm-12 col-xs-12'>
                         <div class='info-box clickable' id='num_members'>
                             <span class='info-box-icon bg-orange dicon'><i class='fa fa-thumbs-up'></i></span>
                             <div class='info-box-content'>
                             <span class='info-box-text'>" . $approved->row()->count . " Approved</span>
                             <span class='info-box-number'><small>PhP</small> " . number_format($approved->row()->amount, 2) . "</span>
                             </div>
                         </div>
                      </div>
                      <div class='col-md-3 col-sm-12 col-xs-12'>
                         <div class='info-box clickable' id='num_vehicles'>
                             <span class='info-box-icon bg-green dicon'><i class='fa fa-rocket'></i></span>
                             <div class='info-box-content'>
                             <span class='info-box-text'>" . $released->row()->count . " Released</span>
                             <span class='info-box-number'><small>PhP</small> " . number_format($released->row()->amount, 2) . "</span>
                             </div>
                         </div>
                      </div>
                      <div class='col-md-3 col-sm-12 col-xs-12'>
                         <div class='info-box clickable' id='num_vehicles'>
                             <span class='info-box-icon bg-teal dicon'><i class='fa fa-cubes'></i></span>
                             <div class='info-box-content'>
                             <span class='info-box-text'><b>" . $items->num_rows() . " TOTAL</b></span>
                             <span class='info-box-number'><small>PhP</small> " . number_format($overall->row()->amount, 2) . "</span>
                             </div>
                         </div>
                      </div>
                     ";
                        ...
                        <tr><td colspan='9' class='text-center'> - No Disbursement record detected - </td></tr>
                        <tr><td colspan='9' class='text-center'></td></tr>
                     ";
                        }
                        break;
                    ...
                    case 'cheque':
                        $items = $this->finance_model->cheques(null, 2, $search);
                        $tbody = "";
                        if ($items->num_rows() > 0) {
                            $i = 0;
                            foreach ($items->result() as $it) {
                                $i++;
                                $tbtn = '
                           <span type="button" id="ech' . base64_encode($it->chid) . '" onclick="edchq(this.id)" class="btn edbtn btn-xs"><i class="fa fa-gears"></i></span>
                           ';
                                $tbody = $tbody .
                                    "<tr>
                           <td class='text-center'>" . $i . "</td>
                           <td class='text-center'>" . $it->ch_cheque_number . "</td>
                           <td class='text-center'>" . $it->di_description . "</td>
                           <td class='text-center'>" . number_format($it->ch_amount, 2) . "</td>
                           <td class='text-center'>" . $it->ve_name . "</td>
                           <td class='text-center'>" . $it->ba_act_name . ' | ' . $it->ba_bank_name . "</td>
                           <td class='text-center'>" . $it->ch_issued . "</td>
                           <td class='text-center'>" . $it->ch_cleared . "</td>
                           <td class='text-center'>" . $it->uname . "</td>
                           <td class='text-center " . $it->ds_color . "'><i class='fa " . $it->ds_icon . "'></i> " . $it->ds_status . "</td>
                           <td class='text-center'>" . $tbtn . "</td>
                        </tr>
                        ";
                            }
                            $pending = $this->finance_model->disburse(null, 1, 2, $search);
                            $approved = $this->finance_model->disburse(null, 2, 2, $search);
                            $released = $this->finance_model->disburse(null, 3, 2, $search);
                            $overall = $this->finance_model->disburse(null, null, 2, $search);
                            $tfooter = "
                    ...
                    <div class='col-md-3 col-sm-12 col-xs-12'>
                       <div class='info-box clickable' id='num_members'>
                           <span class='info-box-icon bg-teal dicon'><i class='fa fa-thumbs-up'></i></span>
                           <div class='info-box-content'>
                           <span class='info-box-text'>" . $approved->row()->count . " Approved</span>
                           <span class='info-box-number'><small>PhP</small> " . number_format($approved->row()->amount, 2) . "</span>
                           </div>
                    ...
                    <div class='col-md-3 col-sm-12 col-xs-12'>
                       <div class='info-box clickable' id='num_vehicles'>
                           <span class='info-box-icon bg-orange dicon'><i class='fa fa-rocket'></i></span>
                           <div class='info-box-content'>
                           <span class='info-box-text'>" . $released->row()->count . " Released</span>
                           <span class='info-box-number'><small>PhP</small> " . number_format($released->row()->amount, 2) . "</span>
                           </div>
                    ...
                    <div class='col-md-3 col-sm-12 col-xs-12'>
                       <div class='info-box clickable' id='num_vehicles'>
                           <span class='info-box-icon bg-green dicon'><i class='fa fa-cubes'></i></span>
                           <div class='info-box-content'>
                           <span class='info-box-text'><b>" . $items->num_rows() . " TOTAL</b></span>
                           <span class='info-box-number'><small>PhP</small> " . number_format($overall->row()->amount, 2) . "</span>
                           </div>
                     ...
                     ";
                        ...
                        <tr><td colspan='10' class='text-center'> - No Disbursement record detected - </td></tr>
                        <tr><td colspan='10' class='text-center'></td></tr>
                     ";
                        }
                        break;
                    ...
                    case 'transfer':
                        $items = $this->finance_model->cheques(null, 3, $search);
                        $tbody = "";
                        if ($items->num_rows() > 0) {
                            $i = 0;
                            foreach ($items->result() as $it) {
                                $i++;
                                $tbtn = '
                           <span type="button" id="ect' . base64_encode($it->chid) . '" onclick="edchq(this.id)" class="btn edbtn btn-xs"><i class="fa fa-gears"></i></span>
                           ';
                                $tbody = $tbody .
                                    "<tr>
                           <td class='text-center'>" . $i . "</td>
                        ...
                        ";
                            ...
                            $pending = $this->finance_model->disburse(null, 1, 3, $search);
                            $approved = $this->finance_model->disburse(null, 2, 3, $search);
                            $released = $this->finance_model->disburse(null, 3, 3, $search);
                            $overall = $this->finance_model->disburse(null, null, 3, $search);
                            $tfooter = "
                    ...
                    case 'cash':
                        $items = $this->finance_model->cheques(null, 1, $search);
                        $tbody = "";
                        if ($items->num_rows() > 0) {
                            $i = 0;
                            foreach ($items->result() as $it) {
                                ...
                                $tbtn = '
                           <span type="button" id="eca' . base64_encode($it->chid) . '" onclick="edchq(this.id)" class="btn ' . $it->ds_color . ' edbtn btn-xs"><i class="fa ' . $it->ds_icon . '"></i>la</span>
                           ';
                                $tbody = $tbody .
                                    "<tr>
                           <td class='text-center'>" . $i . "</td>
                        ...
                        ";
                            ...
                            $pending = $this->finance_model->disburse(null, 1, 1, $search);
                            $approved = $this->finance_model->disburse(null, 2, 1, $search);
                            $released = $this->finance_model->disburse(null, 3, 1, $search);
                            $overall = $this->finance_model->disburse(null, null, 1, $search);
                            $tfooter = "
    ...
    function fetch_dis_account()
    {
        ...
        <?php
            ...
            $baid = base64_decode($this->input->post('id'));
            $bank =  $this->finance_model->accounts($baid);
            $ba = $bank->row();
            $data = [
                'act_num'      => $ba->ba_act_num,
                'act_name'     => $ba->ba_act_name,
                'act_bname'    => $ba->ba_bank_name,
                'act_branch'   => $ba->ba_branch_name,
                'act_type'     => $ba->ba_account_type,
                'act_id'       => base64_encode($baid),
            ];
            echo json_encode($data);
        }
    }

    function fetch_dis_vendor()
    {
        if (!$this->session->userdata('is_logged_in')) {
        ?>
            <script type="text/javascript">
        ...
        <?php
            ...
            $veid = base64_decode($this->input->post('id'));
            $vendor =  $this->finance_model->vendors($veid);
            $ve = $vendor->row();
            $data = [
                've_name'         => $ve->ve_name,
                've_address'      => $ve->ve_address,
                've_email'        => $ve->ve_email,
                've_contact_num'  => $ve->ve_contact_num,
                've_ba_act_num'   => $ve->ve_ba_act_num,
                've_ba_act_name'  => $ve->ve_ba_act_name,
                've_bank_name'    => $ve->ve_bank_name,
                've_process'      => base64_encode($veid),
            ];
            echo json_encode($data);
        }
    }

    function fetch_dis_record()
    {
        if (!$this->session->userdata('is_logged_in')) {
        ?>
            <script type="text/javascript">
        ...
        <?php
            ...
            $diid = base64_decode($this->input->post('id'));
            $disburse = $this->finance_model->disbursements($diid);
            $cheque = $this->finance_model->cheques($diid);
            $ch = $cheque->row();
            $db = $disburse->row();
            $data['disburse_id'] = base64_encode($diid);
            $data['check_id'] = base64_encode($ch->chid);
            $data['name'] = $db->di_description;
            $data['ctid'] = $db->di_ctid;
            $data['category'] = $db->ct_category;
            $data['status'] = $db->ds_status;
            $data['statid'] = $db->di_status;
            $data['vendor'] = $db->ve_name;
            $data['vendid'] = base64_encode($db->di_veid);
            $data['amount'] = number_format($db->di_amount, 2);
            $data['paytype'] = $db->pt_type;
            $data['paytypeid'] = $db->di_type;
            $data['account'] = $ch->ba_act_num . ' ' . $ch->ba_act_name . ' | ' . $ch->ba_bank_name . ' ' . $ch->ba_branch_name;
            $data['accountid'] = base64_encode($ch->ch_baid);
            $data['cheque'] = $ch->ch_cheque_number;
            echo json_encode($data);
        }
    }

    function del_dis_vendor()
    {
        if (!$this->session->userdata('is_logged_in')) {
        ?>
            <script type="text/javascript">
        ...
        <?php
            ...
            $veid = base64_decode($this->input->post('id'));
            $success = $this->finance_model->delete_ds($veid, 'ds_vendor', 'veid');
            $data['success'] = $success > 0;
            echo json_encode($data);
        }
    }

    function del_dis_account()
    {
        if (!$this->session->userdata('is_logged_in')) {
        ?>
            <script type="text/javascript">
        ...
        <?php
            ...
            $baid = base64_decode($this->input->post('id'));
            $success = $this->finance_model->delete_ds($baid, 'ds_bank_account', 'baid');
            $data['success'] = $success > 0;
            echo json_encode($data);
        }
    }

    function del_dis_record()
    {
        if (!$this->session->userdata('is_logged_in')) {
        ?>
            <script type="text/javascript">
        ...
        <?php
            ...
            $cheque = $this->finance_model->cheques($diid);
            $ch = $cheque->row();
            $this->finance_model->delete_ds($ch->chid, 'ds_cheque', 'chid');
            $ret = $this->finance_model->delete_ds($diid, 'ds_disbursement', 'diid');
            $success = false;
            if ($ret > 0) {
                $success = true;
            }
    ...
    function update_account()
    {
        ...
        <?php
            ...
            $account = [
                'ba_act_num'      => $this->input->post('num'),
                'ba_act_name'     => $this->input->post('name'),
                'ba_bank_name'    => $this->input->post('bname'),
                'ba_branch_name'  => $this->input->post('branch'),
                'ba_account_type' => $this->input->post('type'),
            ];
            if (!empty($proc)) {
                $proc = base64_decode($proc);
                $baid = $this->finance_model->update_ds($account, $proc, 'ds_bank_account', 'baid');
            } else {
                $baid = $this->finance_model->save_ds($account, 'ds_bank_account');
            }
            echo json_encode(['success' => $baid > 0]); // Inline success response
        }
    }
    ...
    function update_vendor()
    {
        ...
        <?php
            ...
            $proc = $this->input->post('proc');
            $account = [
                've_name'           => $this->input->post('name'),
                've_address'        => $this->input->post('address'),
                've_email'          => $this->input->post('email'),
                've_contact_num'    => $this->input->post('contact'),
                've_ba_act_name'    => $this->input->post('act_name'),
                've_ba_act_num'     => $this->input->post('act_num'),
                've_bank_name'      => $this->input->post('bank'),
            ];
            if (!empty($proc)) {
                $proc = base64_decode($proc);
                $baid = $this->finance_model->update_ds($account, $proc, 'ds_vendor', 'veid');
            } else {
                $baid = $this->finance_model->save_ds($account, 'ds_vendor');
            }
            echo json_encode(['success' => $baid > 0]); // Inline success response
        }
    }
    ...
    function add_disbursement()
    {
        ...
        <?php
            ...
            $ctid = $this->input->post('category'); // category
            ...
            $res = 0;
            $disburse = array(
                'di_veid'         => base64_decode($vendor),
                'di_amount'       => $amount,
                'di_ctid'         => $ctid, // category
                'di_status'       => 1,
                'di_auth_id'      => $user,
                'di_date'         => date('Y-m-d'),
                'di_datetime'     => date('F d, Y h:i a'),
                'di_description'  => $name,
                'di_type'           => $type,
            );
            $diid = $this->finance_model->save_ds($disburse, 'ds_disbursement');
            // if ($type==2) {
            $cheque = array(
                'ch_cheque_number'   => $check,
                'ch_diid'            => $diid,
                'ch_type'            => $type,
                'ch_baid'            => base64_decode($bank),
                'ch_amount'          => $amount,
                'ch_status'          => 4, // issued
                'ch_issued'          => date('Y-m-d'),
                'ch_auth_id'         => $user,
            );
            $chid = $this->finance_model->save_ds($cheque, 'ds_cheque');
            // }
            if ($diid > 0) {
                $success = true;
            }
    ...
    function update_disbursement()
    {
        ...
        <?php
            ...
            $chid = base64_decode($this->input->post('chid'));
            $diid = base64_decode($this->input->post('diid'));
            $name = $this->input->post('name');
            ...
            $udiid = $this->finance_model->update_ds($disburse, $diid, 'ds_disbursement', 'diid');
            // if ($type==2) {
            $cheque = array(
                'ch_cheque_number'   => $check,
                'ch_diid'            => $diid,
            ...
            if ($chid == 0) {
                ...
                $this->finance_model->update_ds($cheque, $chid, 'ds_cheque', 'chid');
            }
            // }
            if ($udiid > 0) {
                $success = true;
    ...
    function disbursement()
    {
        ...
        <?php
            ...
            $data['vendors'] = $this->finance_model->vendors();
            $data['category'] = $this->finance_model->categories();
            $data['accounts'] = $this->finance_model->accounts();
            $data['modules'] = 'finance';
            $data['main_content'] = 'disbursement/dashboard';
            echo Modules::run('templates/main_content', $data);
        }
    }


    function fetch_disbursements($record = null)
    {
        if (!$this->session->userdata('is_logged_in')) {
        ?>
            <script type="text/javascript">
        ...
        <?php
            ...
            switch ($record) {
                ...
                case 'vendor':
                  ...
                  <div class='col-md-8'>
                     <button type='button' onclick='add_vendor()' class='btn btn-sm pull-right'><i class='fa fa-plus'></i> Add A Vendor</button>
                  </div>
                  ";
                    $pheader = "<b><i class='fa fa-shopping-cart'></i> List of Vendors</b>";
                    $thead = "
                  ...
                  ";
                    $items = $this->finance_model->vendors();
                    $tbody = "";
                    if ($items->num_rows() > 0) {
                        $i = 0;
                        foreach ($items->result() as $it) {
                            $i++;
                            $tbtn = '
                        <button type="button" id="ev' . base64_encode($it->veid) . '" onclick="editVendor(this.id)" class="btn btn-info edbtn btn-xs"><i class="fa fa-pencil"></i></button>
                        <button type="button" id="dv' . base64_encode($it->veid) . '" onclick="delVendor(this.id)" class="btn btn-danger delbtn btn-xs"><i class="fa fa-trash"></i></button>
                        <!-- <button type="button" id="vv' . base64_encode($it->veid) . '" onclick="vwven(this.id)" class="btn btn-warning edbtn btn-xs"><i class="fa fa-eye"></i></button> -->
                        ';
                            $tbody = $tbody .
                                "<tr>
                        <td class='text-center'>" . $i . "</td>
                        <td class='text-center'>" . $it->ve_name . "</td>
                        <td class='text-center'>" . $it->ve_address . "</td>
                        <td class='text-center'>" . $it->ve_email . "</td>
                        <td class='text-center'>" . $it->ve_contact_num . "</td>
                        <td class='text-center'>" . $it->ve_ba_act_name . "</td>
                        <td class='text-center'>" . $it->ve_ba_act_num . "</td>
                        <td class='text-center'>" . $it->ve_bank_name . "</td>
                        <td class='text-center'>" . $tbtn . "</td>
                     </tr>
                     ";
                        }
                    ...
                    $tbanner = "
                     <div class='col-md-4'>
                        <div class='input-group'>
                           <input type='text' id='dtransfer' class='form-control searchit' placeholder='Search Disbursement'>
                           <span class='input-group-addon'><i class='fa fa-search'></i></span>
                        </div>
                     </div>
                     <div class='col-md-8'>
                        <button type='button' onclick='add_disbursement()' class='btn btn-sm pull-right'><i class='fa fa-plus'></i> Add A Disbursement</button>
                     </div>
                     ";
                    $pheader = "<b><i class='fa fa-wallet'></i> List of Bank Transfers</b>";
                    $thead = "
                     <tr style='background-color:lightblue;'>
                        <th class='text-center'>#</th>
                     ...
                     ";
                    $items = $this->finance_model->cheques(null, 3);
                    $tbody = "";
                    if ($items->num_rows() > 0) {
                        $i = 0;
                        foreach ($items->result() as $it) {
                            $i++;
                            $tbtn = '
                           <button type="button" id="ect' . base64_encode($it->chid) . '" onclick="edchq(this.id)" class="btn btn-success edbtn btn-xs"><i class="fa fa-check"></i></button>
                           ';
                            $tbody = $tbody .
                                "<tr>
                           <td class='text-center'>" . $i . "</td>
                        ...
                        $pending = $this->finance_model->disburse(null, 1, 3);
                        $approved = $this->finance_model->disburse(null, 2, 3);
                        $released = $this->finance_model->disburse(null, 3, 3);
                        $overall = $this->finance_model->disburse(null, null, 3);
                        $tfooter = "
                ...
                case 'cash':
                    $tbanner = "
                     <div class='col-md-4'>
                        <div class='input-group'>
                           <input type='text' id='dcash' class='form-control searchit' placeholder='Search Disbursement'>
                           <span class='input-group-addon'><i class='fa fa-search'></i></span>
                        </div>
                     </div>
                     <div class='col-md-8'>
                     ...
                     ";
                    $pheader = "<b><i class='fa fa-money'></i> List of Cash Issued</b>";
                    $thead = "
                     <tr style='background-color:lightblue;'>
                        <th class='text-center'>#</th>
                        <th class='text-center'>Reference #</th>
                        ...
                        <th class='text-center'>Date Issued</th>
                        <th class='text-center'>Auth</th>
                        <th class='text-center'>Cash Status</th>
                        <th class='text-center'>Action</th>
                     </tr>
                     ";
                    $items = $this->finance_model->cheques(null, 1);
                    $tbody = "";
                    if ($items->num_rows() > 0) {
                        $i = 0;
                        foreach ($items->result() as $it) {
                            $i++;
                            $tbtn = '
                           <span type="button" id="eca' . base64_encode($it->chid) . '" onclick="edchq(this.id)" class="btn edbtn btn-xs"><i class="fa fa-gears"></i></span>
                           ';
                            $tbody = $tbody .
                                "<tr>
                           <td class='text-center'>" . $i . "</td>
                        ...
                        $pending = $this->finance_model->disburse(null, 1, 1);
                        $approved = $this->finance_model->disburse(null, 2, 1);
                        $released = $this->finance_model->disburse(null, 3, 1);
                        $overall = $this->finance_model->disburse(null, null, 1);
                        $tfooter = "
                ...
                case 'cheque':
                  ...
                  <div class='col-md-4'>
                     <div class='input-group'>
                        <input type='text' id='dcheque' class='form-control searchit' placeholder='Search Disbursement'>
                        <span class='input-group-addon'><i class='fa fa-search'></i></span>
                     </div>
                  </div>
                  <div class='col-md-8'>
                  ...
                  ";
                    $pheader = "<b><i class='fa fa-cc'></i> List of Cheques Issued</b>";
                    $thead = "
                  <tr style='background-color:lightblue;'>
                     <th class='text-center'>#</th>
                  ...
                  ";
                    $items = $this->finance_model->cheques(null, 2);
                    $tbody = "";
                    if ($items->num_rows() > 0) {
                        $i = 0;
                        foreach ($items->result() as $it) {
                            $i++;
                            $tbtn = '
                        <button type="button" id="ech' . base64_encode($it->chid) . '" onclick="edchq(this.id)" class="btn btn-success edbtn btn-xs"><i class="fa fa-check"></i></button>
                        ';
                            $tbody = $tbody .
                                "<tr>
                        <td class='text-center'>" . $i . "</td>
                     ...
                     ";
                        ...
                        $pending = $this->finance_model->disburse(null, 1, 2);
                        $approved = $this->finance_model->disburse(null, 2, 2);
                        $released = $this->finance_model->disburse(null, 3, 2);
                        $overall = $this->finance_model->disburse(null, null, 2);
                        $tfooter = "
                 ...
                 <div class='col-md-3 col-sm-12 col-xs-12'>
                  ...
                  ";
                     ...
                     <tr><td colspan='11' class='text-center'> - No Disbursement record detected - </td></tr>
                     <tr><td colspan='11' class='text-center'></td></tr>
                  ";
                    }
                    break;
                ...
                case 'accounts':
                    $tbanner = "
                  <div class='col-md-6'><span style='font-size: 12px;'>This is our finance accounts (Bank Accounts, GCASH, )</span></div>
                  <div class='col-md-6'>
                     <button type='button' onclick='add_account()' class='btn btn-sm pull-right'><i class='fa fa-plus'></i> Add An Account</button>
                  </div>
                  ";
                    $pheader = "<b><i class='fa fa-bank'></i> Our Accounts</b>";
                    $thead = "
                  <tr style='background-color:lightblue;'>
                     <th class='text-center'>#</th>
                  ...
                  ";
                    $items = $this->finance_model->accounts();
                    $tbody = "";
                    if ($items->num_rows() > 0) {
                        $i = 0;
                        foreach ($items->result() as $it) {
                            $i++;
                            $tbtn = '
                        <button type="button" id="ea' . base64_encode($it->baid) . '" onclick="editAccount(this.id)" class="btn btn-info edbtn btn-xs"><i class="fa fa-pencil"></i></button>
                        <button type="button" id="da' . base64_encode($it->baid) . '" onclick="delAccount(this.id)" class="btn btn-danger delbtn btn-xs"><i class="fa fa-trash"></i></button>
                     <!--   <button type="button" id="va' . base64_encode($it->baid) . '" onclick="vwacc(this.id)" class="btn btn-warning edbtn btn-xs"><i class="fa fa-eye"></i></button> -->
                        ';
                            $tbody = $tbody .
                                "<tr>
                        <td class='text-center'>" . $i . "</td>
                        <td class='text-center'>" . $it->ba_act_num . "</td>
                        <td class='text-center'>" . $it->ba_act_name . "</td>
                        <td class='text-center'>" . $it->ba_bank_name . "</td>
                        <td class='text-center'>" . $it->ba_branch_name . "</td>
                        <td class='text-center'>" . $it->ba_account_type . "</td>
                        <td class='text-center'>" . $tbtn . "</td>
                     </tr>
                     ";
                        }
                     ...
                     <tr><td colspan='7' class='text-center'> - No Disbursement record detected - </td></tr>
                     <tr><td colspan='7' class='text-center'></td></tr>
                  ";
                    }
                    break;
                ...
                case 'disburse':
                  ...
                  <div class='col-md-4'>
                     <div class='input-group'>
                        <input type='text' id='ddisburse' class='form-control searchit' placeholder='Search Disbursement'>
                        <span class='input-group-addon'><i class='fa fa-search'></i></span>
                     </div>
                  </div>
                  <div class='col-md-8'>
                  ...
                  ";
                    $pheader = "<b><i class='fa fa-tags'></i> List of Disbursements</b>";
                    $thead = "
                  <tr style='background-color:lightblue;'>
                     <th class='text-center'>#</th>
                     <th class='text-center'>Description</th>
                  ...
                  ";
                    $items = $this->finance_model->disbursements();
                    $tbody = "";
                    if ($items->num_rows() > 0) {
                        $i = 0;
                        foreach ($items->result() as $it) {
                        ...
                        <button type="button" id="ed' . base64_encode($it->diid) . '" onclick="eddis(this.id)" class="btn btn-info edbtn btn-xs"><i class="fa fa-pencil"></i></button>
                        <button type="button" id="dd' . base64_encode($it->diid) . '" onclick="dldis(this.id)" class="btn btn-danger delbtn btn-xs"><i class="fa fa-trash"></i></button>
                        <!-- <button type="button" id="vd' . base64_encode($it->diid) . '" onclick="vwdis(this.id)" class="btn btn-warning edbtn btn-sm"><i class="fa fa-eye"></i></button> -->
                        ';
                            $tbody = $tbody .
                                "<tr>
                        <td class='text-center'>" . str_pad($it->diid, 7, "0", STR_PAD_LEFT) . "</td>
                     ...
                     ";
                        ...
                        $pending = $this->finance_model->disburse(null, 1);
                        $approved = $this->finance_model->disburse(null, 2);
                        $released = $this->finance_model->disburse(null, 3);
                        $overall = $this->finance_model->disburse();
                        $tfooter = "
    ...
    function getPlanByGrade($level, $sy)
    {
        return $this->finance_model->getPlanByGrade($level, $sy);
    }
    ...
    function printSOA($st_id, $school_year = NULL, $semseter = 0, $showPayment = NULL, $grade_id = null)
    {
        ...
        if (file_exists(APPPATH . 'modules/finance/views/reports/' . strtolower($this->eskwela->getSet()->short_name) . '/' . strtolower($this->eskwela->getSet()->short_name) . '_printSOA.php')):
            if ($school_year < 2023):
                $this->load->view('reports/' . strtolower($this->eskwela->getSet()->short_name) . '/' . strtolower($this->eskwela->getSet()->short_name) . '_printSOA_old', $data);
            else:
                if ($grade_id >= 5 && $grade_id <= 7):
                    ...
                    $this->load->view('reports/' . strtolower($this->eskwela->getSet()->short_name) . '/' . strtolower($this->eskwela->getSet()->short_name) . '_printSOA_lower', $data);
                endif;
            endif;
        else:
            if ($this->eskwela->getSet()->short_name == "csfl"):
                //$this->load->view('reports/'.strtolower($this->eskwela->getSet()->short_name).'/'.strtolower($this->eskwela->getSet()->short_name).'_printSOA', $data);
                if ($school_year < 2023):
                    $this->load->view('reports/csfl/csfl_printSOA_old', $data);
                else:
                    if ($grade_id >= 5 || $grade_id <= 7):
                        $this->load->view('reports/csfl/csfl_printSOA', $data);
                    else:
                        $this->load->view('reports/csfl/csfl_printSOA_lower', $data);
                    endif;
                endif;
            // echo 'true';
    ...
    function searchFinanceStaff($value, $year = NULL)
    {
        ...
        ?>
            <li style="font-size:18px;"
                onclick="$('#searchName').hide(), $('#searchBox').val('<?php echo $s->firstname . ' ' . $s->lastname ?>'), $('#assign_employee_id').val('<?php echo $s->employee_id ?>')">
                <?php echo strtoupper($s->firstname . ' ' . $s->lastname) ?></li>
        <?php
        endforeach;
        echo '</ul>';
    }
    ...
    function searchReceipt($value = NULL, $school_year = NULL)
    {
        ...
        ?>
            <li style="font-size:18px;"
                onclick="$('#searchReceipt').hide(), $('#searchReceiptBox').val('<?php echo $r->ref_number ?>'),loadORDetails('<?php echo $r->ref_number ?>', '<?php echo $school_year ?>')">
                <?php echo $r->ref_number ?></li>
            <?php
        endforeach;
        echo '</ul>';
    }
    ...
    {
        $this->load->library('pdf');
        $grade_level = $this->uri->segment(3);
        $data['studperlevel'] = $this->finance_model->studperlevel($grade_level);
        $data['due_date'] = $this->uri->segment(4);
        // $data['st_id'] = $st_id;
        $this->load->view('finance/reports/print_soa', $data);
    }

    public function collection()
    {
        $data['get_level'] = $this->finance_model->showgradelevel();
        $data['main_content'] = 'print_collect';
        $data['modules'] = 'finance';
    ...
    public function getCollectible($sy = NULL, $limit = NULL, $offset = NULL)
    {
        ...
        $totalBalance = 0;
        // $result = $this->finance_model->getCollectibles(NULL,NULL,$year,null, null);


        // $result = $this->finance_model->getCollectibles(NULL,NULL,$year,null, null);
        $result = $this->finance_model->getStudents(NULL, NULL, $year);
        ...
        // $page = $this->finance_model->getCollectibles(null, null, $year,null,null);
        $page = $this->finance_model->getStudents($config['per_page'], $seg, $year);
        // return print_r($page);
        $data['students'] = $page->result();
        $data['totalBalance'] = number_format($totalBalance, 2, '.', ',');
        $data['school_year'] = $year;
    ...
    public function printCollectible($year = null)
    {
        $page = $this->finance_model->getStudents(null, null, $year);
        // return print_r($page);
        $data['students'] = $page->result();
        $this->load->view('collectible_pdf', $data);
    }
    ...
    public function index()
    {
        if ($this->session->userdata('is_logged_in')):
            $settings = Modules::run('main/getSet');
            $data['gradeLevel'] = Modules::run('registrar/getGradeLevel', $settings->level_catered, NULL);
    ...
    function printCollectionReport($from = NULL, $to = NULL, $report_type = NULL, $fin_cat = NULL, $sem = NULL, $sy = NULL, $level = NULL)
    {
        $this->load->library('pdf');
        switch ($report_type):
            case 1:
            ...
            case 5:
                ...
                break;
            default:
                $cat = ($fin_cat == 0 ? 0 : explode('-', $fin_cat));
                $collection = array();
                if (count($cat) > 0):
                    for ($c = 0; $c < count($cat); $c++):
                        $cc = $this->finance_model->getCollection($from, $to, $report_type, $cat[$c], $sem, $sy, $level);
                        array_push($collection, $cc->result());
                    endfor;
                else:
                    $collection = $this->finance_model->getCollection($from, $to, $report_type, NULL, $sem, $sy, $level)->result();
                endif;
    ...
    public function collectionReport($from = NULL, $to = NULL, $report_type = NULL, $fin_cat = NULL, $sem = NULL, $sy = NULL, $level = NULL)
    {
        $cat = ($fin_cat == 0 ? 0 : explode('-', $fin_cat));
        $settings = Modules::run('main/getSet');
        $data['settings'] = $settings;
        $data['dept_level'] = Modules::run('coursemanagement/getLevelCatered', $settings->level_catered);
        ...
        $data['collection'] = $collection;
        // $data['collection'] = $this->finance_model->getCollection($from, $to, $report_type, ($fin_cat == 0 ? NULL : $cat[1]));
        $data['finCat'] = $this->finance_model->getFinItems(NULL);
        $data['catSelected'] = $fin_cat;
        $data['modules'] = 'finance';
        $data['cat'] = count($cat);
    ...
    // public function collectionReport($from = NULL, $to = NULL, $report_type = NULL, $fin_cat = NULL)
    // {
    //     $settings = Modules::run('main/getSet');
    //     $data['ro_year'] = Modules::run('registrar/getROYear');
    //     $data['others'] = $this->finance_model->getother();
    //     $data['collection'] = $this->finance_model->getCollection($from, $to, $report_type, ($fin_cat == 0 ? NULL : $fin_cat));
    //     $data['finCat'] = $this->finance_model->getFinItems(NULL);
    //     $data['modules'] = 'finance';
    //     if (file_exists(APPPATH . 'modules/finance/views/' . strtolower($settings->short_name) . '_sales.php')):
    //         $data['main_content'] = strtolower($settings->short_name) . '_sales';
    ...
    function getPerRefNum($st_id, $sem, $school_year, $ref_num)
    {
        return $this->finance_model->getPerRefNum($st_id, $sem, $school_year, $ref_num);
    }
    ...
    {
        $charges = $this->finance_model->getxtraFinCharges($st_id, $sem, $school_year, $item_id);
    ...
    function getPlanByLevel($grade_id)
    {
        return $this->finance_model->getPlanByLevel($grade_id);
    }
...
<?php endif;
    ...
    public function addFinanceCharges()
    {
        ...
        $q = $this->finance_model->addFinanceCharges($charge, $plan_id, $item, 0, $sy);
        if (!$q) {
            echo json_encode(array('status' => false, 'msg' => 'Item Already Exist'));
        } else {
            echo json_encode(array('status' => true, 'msg' => 'Successfully Added'));
            // echo Modules::run('finance/financeCharges', $course_id, 0, $sy, 0, null);
        }
    }
    ...
    public function getPayments($school_year, $sem, $st_id = NULL, $opt = NULL)
    {
        $student = Modules::run('college/getSingleStudent', base64_decode($st_id), $school_year, $sem, $opt);
        $payments = $this->finance_model->getTransactionByRefNumber($student->uid, $sem, $school_year, NULL, NULL);
        ...
        $online = $this->finance_model->getTransactionByRefNumber($student->uid, $sem, $school_year, 4, NULL);
        $onlineTotal = 0;
        ...
        $excess = $this->finance_model->getTransactionByRefNumber($student->uid, $sem, $school_year, 5, NULL);
        $excessTotal = 0;
        ...
        $payrollDeduction = $this->finance_model->getTransactionByRefNumber($student->uid, $sem, $school_year, 6, NULL);
        ...
        $forwardedBalance = $this->finance_model->getTransactionByRefNumber($student->uid, $sem, $school_year, 7, NULL);
    ...
    function getPrevBal($stid, $sem, $sy, $present_sem, $present_sy)
    {
        $student = Modules::run('finance/getBasicStudent', $stid, $sy, $sem);
        ...
        $transaction = $this->getTransaction($stid, $sem, $sy);
        ...
        if ($present_sy >= 2021):
            $item_id = 126240;
            $charge = array(
                'extra_id' => $this->eskwela->code(),
                'extra_st_id' => $student->user_id,
                'extra_item_id' => $item_id,
                'extra_amount' => $paymentTotal,
                'extra_sem' => $present_sem,
                'extra_school_year' => $present_sy
            );
            $this->finance_model->addExtraFinanceCharges($charge, $present_sy, $item_id, $student->user_id);
        endif;

        switch ($sem):
            case 0:
                $s = 'Regular School Year';
                break;
            case 3:
                $s = 'Summer';
                break;
        ...
        if ($total > 0):
            ...
            $balanceArray = array('status' => FALSE, 'msg' => $total);
        endif;

        return json_encode($balanceArray);
    }

    function deleteReceipt($id, $link)
    {
        $q = $this->finance_model->deleteReceipt($id, base64_decode($link));
        echo ($q ? 'Receipt Successfuly Deleted' : 'An Error Occured during the delete operation');
    }

    function paymentVsCollectibleGraph()
    {
        $settings = Modules::run('main/getSet');
        $school_year = $settings->school_year;
        $current_year = date('Y');
        $gs_start = date('n', strtotime($settings->bosy));
        $gs_end = date('n', strtotime($settings->eosy));
        $ms = date('m', strtotime($settings->bosy));
        $ys = date('Y', strtotime($settings->bosy));
        $mo = date('n');
        // $mo = 9;
        $st = 0;
        $en = 0;
        ...
        $max = 0;
        $collectibles = $this->getCollectibles($this->session->school_year); // Total collection for the whole year
        $mc = '2024-03-01';
        $total_collected = 0;
        $toCollect = $collectibles;

        $s = strtotime($current_year . '-' . ($st > 12 ? (($st - 12) < 10 ? '0' . ($st - 12) : ($st - 12)) : ($st < 10 ? '0' . $st : $st)) . '-01');
        // $s = strtotime($current_year . '-' . $st . '-01');

        if (strtotime(date('Y-m', $s)) >= strtotime(date('Y-m', strtotime($settings->bosy . "-1 month")))):
            if ($st < 5):
                $ee = 12 + $st;
            else:
                $ee = $st;
            ...
            for ($a = ($gs_start - 1); $a < $ee; $a++):
                ...
                $b = ($a > 12 ? (($a - 12) < 10 ? '0' . ($a - 12) : ($a - 12)) : ($a < 10 ? '0' . $a : $a));
                $f = $ay . '-' . $b . '-01';
                $t = $ay . '-' . $b . '-' . date('t', strtotime($ay . '-' . $b));
                $c = $this->finance_model->getMonthlyCollection($f, $t);
                $total_collected += ($c->row()->amount != '' ? $c->row()->amount : 0);
                $toCollect -= ($c->row()->amount != '' ? $c->row()->amount : 0);
            endfor;
        else:
            $toCollect = $collectibles;
        endif;
        ...
        for ($x = $st; $x <= $en; $x++):
            ...
            $n = ($x > 12 ? (($x - 12) < 10 ? '0' . ($x - 12) : ($x - 12)) : ($x < 10 ? '0' . $x : $x));
            $from = $sy . '-' . $n . '-01';
            $to = $sy . '-' . $n . '-' . date('t', strtotime($sy . '-' . $n));

            $z = (strtotime($sy . '-' . $n) >= strtotime($ys . '-' . ($ms - 1)) ? 1 : 0);
            // $z = 1;

            if (strtotime($sy . '-' . $n) >= strtotime(date('Y-m', strtotime($settings->bosy . "-1 month")))):
                $r = $this->finance_model->getMonthlyCollection($from, $to); // ----- monthly collection
                $amount = ($r->row()->amount != '' ? $r->row()->amount : 0);
                // $z = $amount != 0 ? 1 : 0;
                $toCollect -= ($r->row()->amount != '' ? $r->row()->amount : 0);
                $max = ($toCollect > $amount ? $toCollect : ($amount > $max ? $amount : $max));
                $total_collected += $amount; // total Payments received from the start of SY to present
            else:
                $amount = 0;
                $total_collected = 0;
            endif;
            ...
            $d = array(
                ...
                'a' => ($z == 0 ? 0 : $toCollect),
                'b' => ($z == 0 ? 0 : $amount),
                'c' => ($z == 0 ? 0 : $total_collected)
            );
            array_push($data1, $d);
        endfor;

        // --------------------------- generate weekly collection graph ------------------------------- //
        $year = date('Y');
        $first_week_no = date('W');
        $range = range($first_week_no - 4, $first_week_no);
        ...
        foreach ($range as $week_no) {
            ...
            for ($i = 0; $i < 7; $i++) {
                $todate = $week_start->format('Y-m-d');
                $dl = $this->finance_model->getDailyCollection($todate);
                if ($i == 0 || $i == 6):
                    if ($i == 0):
                        $dStart = $week_start->format('Y-m-d');
                    else:
                        $dEnd = $week_start->format('Y-m-d');
                    endif;
                    $week_start->modify('+1 day');
                else:
                    $week_start->modify('+1 day');
                ...
                if (strtotime(date('Y-m-d')) >= strtotime($todate)):
                    $e = array(
                        'd' => $todate,
                        'e' => ($dl->amount != NULL ? $dl->amount : 0)
                    );
                endif;

                array_push($daily, $e);
            }
            $wc = $this->finance_model->getMonthlyCollection($dStart, $dEnd);

            $w = array(
                'a' => $dEnd,
                'b' => ($wc->row()->amount != null ? $wc->row()->amount : 0)
            );
            array_push($weekly, $w);
        }

        $data['data1'] = json_encode($data1);
        $data['weekly'] = json_encode($weekly);
        $data['daily'] = json_encode($daily);
        // $data['main_content'] = 'graphs/paymentVsCollectible';
        // $data['modules'] = 'finance';
        // echo Modules::run('templates/main_content', $data);
        $this->load->view('graphs/paymentVsCollectible', $data);
    }

    function getCollectibles($year)
    {
        $students = $this->finance_model->getOfficialStudents($year);
        ...
        foreach ($students->result() as $s):
            $total = 0;
            $total_transaction = 0;
            $accountDetails = json_decode(Modules::run('finance/getRunningBalance', base64_encode($s->user_id), $year));

            $plan = Modules::run('finance/getPlanByCourse', $s->grade_level_id, 0, $s->st_type, $s->school_year);
