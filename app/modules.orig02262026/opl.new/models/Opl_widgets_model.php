...
class Opl_widgets_model extends CI_Model
{
    ...
    function getActiveTask($grade_id, $section_id, $subject_id, $faculty_id)
    {
        $this->db->where('task_grade_id', $grade_id);
        $this->db->where('task_section_id', $section_id);
        $this->db->where('task_subject_id', $subject_id);
        $this->db->where('task_author_id', $faculty_id);
        $this->db->where('task_start_time <=', date('Y-m-d G:i:s'));
        $this->db->where('task_end_time >', date('Y-m-d G:i:s'));
        return $this->db->get('opl_tasks');
    }
}
