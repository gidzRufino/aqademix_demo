...
class Opl_model extends MX_Controller
{
    ...
    {
        $this->db->where('sr_stid', $st_id);
        $this->db->where('sr_ref_id', $task_id);
        return $this->db->update('opl_rubric_student', array('is_posted' => 1));
    }
    ...
    public function searchRubric($keyword)
    {
        $this->db->like('LOWER(ru_alias)', strtolower($keyword));
        $this->db->limit(10);
        $query = $this->db->get('opl_rubric_info'); // replace with your actual rubric table
        return $query->result();
    }
    ...
    function updateAssessment($assess_id, $details)
    {
        $this->db->where('assess_id', $assess_id);
        $this->db->update('gs_assessment', $details);
    }

    function deleteAssessment($assess_id)
    {
        $this->db->where('assess_id', $assess_id);
    ...
    function updateRubric($details, $id)
    {
        $this->db->where('ruid', $id);
        $this->db->update('opl_rubric_info', $details);
    ...
    function getPost($id, $grade = NULL, $section = NULL, $subject = NULL, $limit = 100)
    {
