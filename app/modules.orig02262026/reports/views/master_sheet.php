...
class MYPDF extends Pdf
{
    ...
    public function Header()
    {
        ...
        $adviser = Modules::run('academic/getAdvisory', '', $CI->uri->segment(5), $CI->uri->segment(3));

        // $subject = Modules::run('academic/getSpecificSubjects', segment_4);
        $nextYear = $CI->uri->segment(5) + 1;
        ...
        $this->Ln();
        $this->Cell(0, 4.3, "SY  ".$CI->uri->segment(5).' - '.$nextYear, 0, 0, 'C');
        $this->Ln(10);
    ...
    switch ($singleSub->parent_subject) {
        ...
        case 18:
            if ($singleSub->subject_id == 13):
                $pdf->MultiCell(60, 3, 'MAPEH', 'TB', 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            endif;
            break;
        default:
            $pdf->MultiCell(15, 3, $singleSub->short_code, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
...
foreach ($male->result() as $s) {
    ...
    foreach ($subject_ids as $sub) {
        ...
        $finalGrade = Modules::run('gradingsystem/getFinalGrade', $s->st_id, $singleSub->subject_id, $CI->uri->segment(4), $CI->uri->segment(5));
        if (!empty($finalGrade)):
            ...
            $pdf->MultiCell(15, 3, $ass_gr, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        else:
            $finalAssessment += 0;
    ...
    $plg = round($finalAssessment / count($subject_ids), 2);
    ...
    endswitch;
    $pdf->MultiCell(5, 3, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, $val, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    if ($plg >= 75):
...
foreach ($female->result() as $fem) {
    ...
    foreach ($subject_ids as $sub) {
        $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sub_id);
        $finalGrade = Modules::run('gradingsystem/getFinalGrade', $fem->st_id, $singleSub->subject_id, $CI->uri->segment(4), $CI->uri->segment(5));
        if (!empty($finalGrade)):
