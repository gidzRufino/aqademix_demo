<?php

class MYPDF extends Pdf {

    //Page header
    public function Header() {
        $CI = &get_instance();
        // Logo
        $section = Modules::run('registrar/getSectionById', $CI->uri->segment(3));
        $settings = Modules::run('main/getSet');
        $adviser = Modules::run('academic/getAdvisory', '', $CI->uri->segment(3));

        $subject = Modules::run('academic/getSpecificSubjects', $CI->uri->segment(4));
        $nextYear = $CI->uri->segment(6) + 1;

        $this->SetTitle('Top Ten');
        $this->SetTopMargin(3);
        $this->Ln(5);
        $this->SetX(10);
        $this->SetFont('helvetica', 'B', 12);
        $this->Cell(0, 0, $settings->set_school_name, 0, false, 'C', 0, '', 0, false, 'M', 'T');
        $this->Ln();
        $this->SetFont('helvetica', 'n', 8);
        $this->Cell(0, 15, $settings->set_school_address, 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $image_file = K_PATH_IMAGES . '/' . $settings->set_logo;
        $this->Image($image_file, 300, 8, 20, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);

        $image_file = K_PATH_IMAGES . '/depEd_logo.jpg';
        $this->Image($image_file, 10, 8, 20, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        switch ($CI->uri->segment(5)) {
            case 1:
                $term = 'FIRST QUARTER';
                break;
            case 2:
                $term = 'SECOND QUARTER';
                break;
            case 3:
                $term = 'THIRD QUARTER';
                break;
            case 4:
                $term = 'FOURTH QUARTER';
                break;
        }
        $this->Ln(12);
        $this->SetFont('helvetica', 'B', 12);
        $this->Cell(0, 4.3, $section->level . ' ' . $term . ' LIST OF HONORS', 0, 0, 'C');
        $this->Ln();
        $this->Cell(0, 4.3, "SY  " . $CI->uri->segment(6) . ' - ' . $nextYear, 0, 0, 'C');
        $this->Ln(10);
    }

    // Page footer
    public function Footer() {
        // Position at 15 mm from bottom

        $this->SetY(-15);
        // Set font
        $this->SetFont('helvetica', 'I', 8);
        // Page number
        $this->Cell(0, 10, 'Page ' . $this->getAliasNumPage() . '/' . $this->getAliasNbPages(), 0, false, 'R', 0, '', 0, false, 'T', 'M');
    }

}

$CI = &get_instance();
$pdf = new MYPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);

// set auto page breaks
$pdf->SetAutoPageBreak(TRUE, 5);

// set image scale factor
$pdf->setImageScale(PDF_IMAGE_SCALE_RATIO);
// remove default header/footer
$resolution = array(345, 216);
$pdf->AddPage('L', $resolution);

$pdf->SetY(35);
$pdf->SetFont('helvetica', 'B', 8);
// set cell padding


$section = Modules::run('registrar/getSectionById', $CI->uri->segment(3));
$subject_ids = Modules::run('academic/getSpecificSubjectPerlevel', $section->grade_id);
//$subject = explode(',', $subject_ids->subject_id);
$tm = 0;
foreach ($subject_ids as $s):
    $singleSub = Modules::run('academic/getSpecificSubjects', $s->sub_id);
    if ($singleSub->parent_subject == 11):
        $tm++;
    endif;
endforeach;
$pdf->SetX(10);
$pdf->setCellPaddings(1, 1, 1, 1);
$pdf->MultiCell(60, 10, 'Full Name', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
foreach ($subject_ids as $s) {
    $singleSub = Modules::run('academic/getSpecificSubjects', $s->sub_id);
    switch ($singleSub->parent_subject) {
        case 0:
        case 18:
            $pdf->MultiCell(20, 10, $singleSub->short_code, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            break;
        case 11:
            if ($singleSub->subject_id == 13):
                $pdf->MultiCell((20 * $tm) + 20, 10, 'MAPEH', 'TB', 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            endif;
            break;
        default:
            $pdf->MultiCell(20, 10, $singleSub->short_code, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            break;
    }
}
$pdf->MultiCell(20, 10, 'GPA', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
$pdf->MultiCell(30, 10, 'RANK', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
$pdf->Ln();

$pdf->SetX(10);
$pdf->setCellPaddings(1, 1, 1, 1);
$pdf->MultiCell(60, 3, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
foreach ($subject_ids as $s) {
    $singleSub = Modules::run('academic/getSpecificSubjects', $s->sub_id);
    switch ($singleSub->parent_subject) {
        case 0:
        case 18:
            $pdf->MultiCell(20, 3, '', 'TB', 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            break;
        case 11:
            $pdf->MultiCell(20, 3, $singleSub->short_code, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            break;
    }
    if ($s->sub_id == 206517):
        $pdf->MultiCell(20, 3, 'TOTAL', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    endif;
}
$pdf->MultiCell(20, 3, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
$pdf->MultiCell(30, 3, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
$pdf->Ln();

$x = 0;
$nm = 0;
$finalTotal = 0;
// List of Top Ten
foreach ($getTopTen as $tt => $gt):
    $x++;

    if ($x < 11):
        $pdf->SetX(10);

        $pdf->SetFont('helvetica', 'B', 7);
        $pdf->MultiCell(60, 3, strtoupper($gt->lastname . ', ' . $gt->firstname . ' ' . substr($gt->middlename, 0, 1)) . '.', 1, 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->SetFont('helvetica', 'B', 8);
        $fa = 0;
        foreach ($subject_ids as $sub):
            $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sub_id);
            if ($singleSub->parent_subject == 11):
                $fg = Modules::run('gradingsystem/getFinalGrade', $gt->st_id, $singleSub->subject_id, $CI->uri->segment(5), $this->session->userdata('school_year'));
                $tf += $fg->row()->final_rating;
            else:
                $nm++;
            endif;
        endforeach;
        $total_mapeh = round($tf / $tm);
        
        foreach ($subject_ids as $sub) {
            $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sub_id);
            $finalGrade = Modules::run('gradingsystem/getFinalGrade', $gt->st_id, $singleSub->subject_id, $CI->uri->segment(5), $this->session->userdata('school_year'));
            if (!empty($finalGrade)):
                switch ($gs_settings->gs_used):
                    case 1:
                        $ass_gr = $finalGrade->row()->final_rating;
                        break;
                    case 2:
                        $ass_gr = ($finalGrade->row()->final_rating == "" ? "" : $finalGrade->row()->final_rating);
                        break;
                endswitch;

                if ($singleSub->parent_subject != 11):
                    $fa++;
                    $finalTotal += $finalGrade->row()->final_rating;
                endif;

                $pdf->MultiCell(20, 3, $ass_gr, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');

                if ($sub->sub_id == 206517):
                    $pdf->MultiCell(20, 3, $total_mapeh, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                endif;
            else:
                $finalTotal += 0;
                $pdf->MultiCell(20, 3, '0', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            endif;
        }

        $plg = round(($finalTotal + $total_mapeh) / ($nm + 1), 2);
        
        $pdf->MultiCell(20, 3, number_format(round($plg, 2), 2), 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        //$pdf->MultiCell(20, 3, round($finalAverage/$fa, 3),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        if (number_format(round($plg, 2), 2) >= 98):
            $rank = 'With Highest Honors';
        elseif (number_format(round($plg, 2), 2) <= 97.99 && number_format(round($plg, 2), 2) >= 95):
            $rank = 'With High Honors';
        elseif (number_format(round($plg, 2), 2) <= 94.99 && number_format(round($plg, 2), 2) >= 90):
            $rank = 'With Honors';
        else:
            $rank = '';
        endif;
        $pdf->MultiCell(30, 3, $rank, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->Ln();
        unset($finalTotal);
        $fa = 0;
    endif;
    $mapeh = 0;
    $finalAssessment = 0;
    $tf = 0;
    $nm = 0;
endforeach;
//
//
//for ($bl = 1; $bl <= 2; $bl++):
//    $pdf->SetX(10);
//    if ($bl == 2):
//        $pdf->MultiCell(60, 3, 'Teachers\'s Signature', 0, 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
//    else:
//        $pdf->MultiCell(60, 3, '', 0, 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
//    endif;
//
//    foreach ($subject_ids as $s) {
//        $singleSub = Modules::run('academic/getSpecificSubjects', $s->sub_id);
//        switch ($singleSub->parent_subject) {
//            case 0:
//            case 11:
//                $pdf->MultiCell(20, 3, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
//                break;
//            case 18:
//                //$pdf->MultiCell(15, 3, ,0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
//                break;
//        }
//    }
//    $pdf->MultiCell(20, 3, '', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
//    $pdf->MultiCell(30, 3, '', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
//    $pdf->Ln();
//endfor;

$settings = Modules::run('main/getSet');
$adviser = Modules::run('academic/getAdvisory', NULL, $settings->school_year, $CI->uri->segment(3));
$pdf->SetFont('helvetica', 'B', 10);

$asstPrincipal = Modules::run('hr/getEmployeeByPosition', 'Asst. to the Principal');

$pdf->Ln();
$pdf->MultiCell(30, 10,  'Prepared By:',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->Ln();
$pdf->MultiCell(25, 10,  '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(75, 10,  strtoupper($adviser->row()->firstname.' '.$adviser->row()->lastname),0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
//$pdf->MultiCell(160, 10,  '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
//$pdf->MultiCell(50, 10,  strtoupper($asstPrincipal->firstname.' '.$asstPrincipal->lastname),0, 'L', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->Ln();
$pdf->SetFont('helvetica', 'B', 8);
$pdf->MultiCell(25, 10,  '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(75, 10,  'Adviser','T', 'C', 0, 0, '', '', true, 0, false, true, 10, 'T');

$pdf->SetFont('helvetica', 'B', 10);
$principal = Modules::run('hr/getEmployeeByPosition', 'Principal');
$pdf->Ln(15);
$pdf->MultiCell(30, 10, 'Verified By:', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(200, 10, 'Validated By: ', '', 'R', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->Ln();
$pdf->MultiCell(25, 10, '', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(75, 10, '', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(125, 10, '', '', 'R', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(75, 10, '', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->Ln();
$pdf->SetFont('helvetica', 'B', 8);
$pdf->MultiCell(25, 10, '', 0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(75, 10, 'Asst. to the Principal', 'T', 'C', 0, 0, '', '', true, 0, false, true, 10, 'T');
$pdf->MultiCell(125, 10, '', '', 'R', 0, 0, '', '', true, 0, false, true, 10, 'B');
$pdf->MultiCell(75, 10, 'School Principal', 'T', 'C', 0, 0, '', '', true, 0, false, true, 10, 'T');

//$html = Modules::run('reports/form1');
//
//$pdf->writeHTML($html, true, false, true, false, '');
// ---------------------------------------------------------
// set default header data
//Close and output PDF document
ob_end_clean();
$pdf->Output('topTen.pdf', 'I');

//============================================================+
// END OF FILE
//============================================================+
    
