<div class="row">
    <div class="col-lg-12">
        <h2 class="page-header">Reports <?php echo $this->session->position ?>
...
<div class="row">
    <div class="col-lg-12">
        <?php
        if ($this->session->userdata('position') == 'Faculty' || $this->session->userdata('position') == 'Grade School Faculty' || $this->session->userdata('position') == 'High School Faculty') {
            ?>
                ...
                <div class="controls span12">
                    <select name="selectReport" onclick="action(this.value)" style="width:100%;" id="selectReport" class="controls-row" required>
                        ...
                        <?php if ($this->session->userdata('is_adviser')): ?>
                            ...
                            <?php if ($this->session->position_id == 1 || $this->session->position == 'Registrar' || $this->session->position == 'Registrar\'s Staff' || $this->session->position == 'Student Assistant (Registrar)'): ?>
            ...
            <div class="control-group pull-left" style="margin-right:20px; width:200px;">
                <div class="controls" id="AddedSection">
                    <select name="inputSection" id="inputSection" onchange="$('#sec_grade_id').val(($(this).children('option:selected').data('id'))), getSubjectPerSection('<?php echo $this->session->employee_id ?>', $(this).children('option:selected').data('id'), $(this).val())" style="width:100%;" class="pull-left controls-row span12" required>
                        <option>Select Section</option> 
                        <?php
                        $assignment = Modules::run('academic/getSectionAssign', $this->session->employee_id, $this->session->school_year);
                        foreach ($assignment as $ga) {
                            ?>
                            <option value="<?php echo $ga->section_id; ?>" data-id="<?php echo $ga->grade_id ?>"><?php echo $ga->level . ' ( ' . $ga->section . ' )' ?></option>  
                        <?php } ?>
                    </select>
                </div>
            </div>
            <div class="control-group pull-left" style="margin-right:20px; width:200px;">
                <div class="controls" id="AddedSection">
                    <select name="inputSubject" id="inputSubject" style="width:100%;"  class="pull-left controls-row span12" required>
                        ...
                        <?php
                        foreach ($assignment as $ga) {
                            $subj = Modules::run('academic/getSpecificSubjects', $ga->subject_id);
                            ?>
                            <option value="<?php echo $ga->subject_id; ?>"><?php echo $subj->subject ?></option>  
                        <?php } ?>
                    </select>
                </div>
            </div>
    ...
    if ($this->session->userdata('is_admin') && $this->session->userdata('is_superAdmin')) {
        ...
        <div style="margin:25px auto !important;">
            ...
            <div class="control-group pull-left"  style="margin-right:20px; width:200px;">
                <select name="inputGrade" onclick="selectSection(this.value)" onchange="$('#sec_grade_id').val(this.value)" id="inputGrade" style="width:100%;" required>
...
//        alert(grade_id);
    ...
    function getSubjectPerSection(emp_id, grade_id, sec_id) {
        var url = '<?php echo base_url() . 'academic/getSubjectPerSection/' ?>';
        $.ajax({
            type: 'POST',
            url: url,
            data: 'faculty_id=' + emp_id + '&grade_id=' + grade_id + '&section_id=' + sec_id + '&csrf_test_name=' + $.cookie('csrf_cookie_name'),
            success: function (data) {
    ...
    function getSubjectOffer(level) {
        var sy = $("#inputSY").val();
        var url = '<?php echo base_url() . 'academic/getSpecificSubjectPerlevel/' ?>' + level + '/' + sy + '/1';
        $.ajax({
            type: 'GET',
            url: url,
            success: function (data) {
    ...
    function generateReport()
    {
        ...
        switch (report) {
            case 'depEdForm2':
                ...
                var url = "<?php echo base_url() ?>reports/" + report + '/' + section_id + '/' + month + '/' + school_year + '/' + grade_id;
            ...
            case 'grading_sheet':
                ...
                var url = "<?php echo base_url() ?>reports/" + report + '/' + section_id + '/' + subject_id + '/' + school_year + '/' + strand + '/' + grade_id;
                window.open(url, '_blank');
                break;
            case 'master_sheet':
                var url = "<?php echo base_url() ?>reports/" + report + '/' + section_id + '/' + term + '/' + school_year + '/' + strand + '/' + grade_id;
//                    if(grade_id!=12 && grade_id!=13){
//                        var url = "<?php echo base_url() ?>reports/"+report+'/'+section_id+'/'+term+'/'+school_year;
//                    }else{
//                        if(strand != '0'){
//                            var url = "<?php echo base_url() ?>reports/"+report+'/'+grade_id+'/'+term+'/'+school_year+'/'+strand;
//                        }else{
//                            var url = "<?php echo base_url() ?>reports/"+report+'/'+section_id+'/'+term+'/'+school_year;
//                        }
//                    }
            ...
            case 'enrollmentList':
                if (grade_id != 12 && grade_id != 13) {
                    ...
                    if (strand != '0') {
                        var url = "<?php echo base_url() ?>reports/" + report + '/' + grade_id + '/' + school_year + '/' + strand + '/' + section_id;
                    } else {
                        var url = "<?php echo base_url() ?>reports/" + report + '/' + section_id + '/' + school_year;
                    }
                }
            ...
            case 'generateCard':
                ...
                break;
            case 'generateForm137':
                var url = "<?php echo base_url() ?>reports/" + report + '/NULL/' + school_year;
                document.location = url;
                break;
            case 'generateTopTen':
...
//                document.location = url;
                break;
            case 'classRanking':
                var url = "<?php echo base_url() ?>reports/class_ranking/" + section_id + '/' + grade_id + '/' + term + '/' + school_year;
                window.open(url, '_blank');
                break;
