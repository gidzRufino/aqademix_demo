...
class MYPDF extends Pdf
{
    ...
    public function Header()
    {
        ...
        $this->Ln();
        $this->Cell(0, 4.3, "SY  ".$CI->uri->segment(4).' - '.$nextYear, 0, 0, 'C');
        $this->Ln();
...
if (count($male->result()) < 32):
    foreach ($male->result() as $s) {
        $x++;
        $xy++;
        $pdf->MultiCell(8, 3, $xy, 1, 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(36, 3, strtoupper($s->lastname), 1, 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(15, 3, ($s->middlename != '' /* || $f->middlename != '.' || $f->middlename != '-' */ ? strtoupper(substr($s->middlename, 0, 1)).'.' : ''), 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    ...
    foreach ($female->result() as $f) {
        ...
        $yx++;
        $pdf->SetX(110);
        $pdf->MultiCell(8, 3, $yx, 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(36, 3, strtoupper($f->lastname), 1, 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(15, 3, ($f->middlename != '' /* || $f->middlename != '.' || $f->middlename != '-' */ ? strtoupper(substr($f->middlename, 0, 1)).'.' : ''), 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
