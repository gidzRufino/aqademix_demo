    ...
    <div class="col-lg-12">
        <h3 style="margin-top: 20px;" class="page-header">List of College Students
            ...
            <?php if ($this->session->userdata('is_admin') || $this->session->userdata('is_superAdmin')) { ?>
                ...
                <?php if ($this->session->userdata('position') == 'Registrar' || $this->session->userdata('position') == 'IBDE Department Head' || $this->session->userdata('position') == 'Super Administrator'): ?>
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            ...
                            switch ($this->session->userdata('position')):
                                ...
                                case 'Super Administrator':
                                    ...
                                    <li onclick="listWithdrawStudent(7)"><a href="#">List of Students Withdrawn</a></li>
                                    <li onclick="listWithdrawStudent(8)"><a href="#">List of Students Transferred Out</a></li>
                                    <li onclick="listWithdrawStudent(9)"><a href="#">List of Students Transferred In</a></li>
                                    <?php
                                    break;
                                case 'IBDE Department Head':
                                    ?>
                                    <li onclick="getSubjectOfferedPerSem(), $('#studentsPerSubject').modal('show')"><a href="#">List of Students Per Subject</a></li>
                                    <?php
                                    break;
                ...
                <div class="form-group">
                    ...
                    <div class="controls">
                        ...
                        <select id="inputRemarks" style="width:300px">
                            ...
                            <option value ="1" >Regular [Officially Enrolled]</option>
                            <option value ="2" >Irregular [Officially Enrolled]</option>
                            <option value ="12" >Unofficially Enrolled</option>
                            <option value ="3" >Partial PAYMENT</option>
                            <option value ="4" >For Payment Evaluation</option>
                            <option value ="5" >For Payment Confirmation</option>
                            <option value ="0" >For Evaluation</option>       
                            <option value ="7" >Withdraw</option>
                            <option value ="8" >Transferred out</option>
                            <option value ="9" >Transferred In</option>
                        </select>
                    </div>
                    <div class="form-group">
...
<script type="text/javascript">
    ...
    function submitRemarks()
    {
        ...
        var stid = $('#st_id').val();
        var dateEffect = $('#inputEffectivity').val();
        alert(stid + ' ' + dateEffect);
        $.ajax({
            type: "POST",
            url: url,
            data: "code=" + code + "&st_id=" + $('#st_id').val() + "&user_id=" + user_id + "&semester=" + sem + "&sy=" + sy + '&info=' + info + '&stid=' + stid + '&date=' + dateEffect + '&csrf_test_name=' + $.cookie('csrf_cookie_name'), // serializes the form's elements.
...
//                }
    ...
    function listWithdrawStudent(value) {
        var sem = $('#inputSem').val();
        var sy = $('#inputSY').val();
        var url = '<?php echo base_url() . 'college/getWithdrawStudentList/' ?>' + value + '/' + sem + '/' + sy;
        window.open(url, '_blank');
    }
