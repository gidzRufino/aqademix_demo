...
class Opl_variables extends MX_Controller
{
    ...
    {
        $comid = $this->input->post('commentid');
        $q = $this->opl_variables_model->deleteComment($comid);

        echo json_encode(($q ? (array('status' => TRUE, 'msg' => 'Successfuly Deleted!')) : (array('status' => FALSE, 'msg' => 'An Error Occured!'))));
    }
}
