...
foreach ($comments as $comment):
    ...
    if ($profile->avatar != NULL || $profile->avatar != ""):
        if (file_exists('uploads/' . $profile->avatar)):
            $avatar = site_url('uploads/' . $profile->avatar);
        else:
            $avatar = site_url('images/avatar/' . ($profile->sex == 'Female' ? 'female.png' : 'male.png'));
        endif;
    else:
        $avatar = site_url('images/avatar/' . ($profile->sex == 'Female' ? 'female.png' : 'male.png'));
    endif;
            ...
            <div id="comments">
                ...
                if (count($replies) != 0):
                    foreach ($replies as $reply):
                        ...
                        //     $avatar = site_url("images/forms/") . $assets->set_logo;
                        // endif;
                        if ($rprofile->avatar != NULL || $rprofile->avatar != ""):
                            if (file_exists('uploads/' . $rprofile->avatar)):
                                $img = site_url('uploads/' . $rprofile->avatar);
                            else:
                                if ($rprofile->sex != ''):
                                    $img = site_url('images/avatar/' . ($rprofile->sex == 'Female' ? 'female.png' : 'male.png'));
                                else:
                                    $img = site_url("images/forms/") . $assets->set_logo;
                                endif;
                            endif;
                        else:
                            if ($rprofile->sex != ''):
                ...
                ?>
                        <div class="mb-3">
                            <img class="img-circle img-sm" src="<?php echo $img; ?>" alt="">
                            <div class="comment-text">
                                <small>
                                    <span class="username">
