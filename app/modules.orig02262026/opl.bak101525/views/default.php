...
<script>
    ...
    function setStudentExceptions(type) {
        switch (type) {
            case '3':
                $.ajax({
                    url: "<?php echo site_url('opl/gradeLevelList/grades/');
                            echo (!$this->session->isOplAdmin) ? $this->session->username : '' ?>",
                    type: "GET",
                    success: function(data) {
                        if (data !== null) {
            ...
            case '4':
                $.ajax({
                    url: "<?php echo site_url('opl/gradeLevelList/classes/');
                            echo (!$this->session->isOplAdmin) ? $this->session->username : '' ?>",
                    type: "GET",
                    success: function(data) {
                        if (data !== null) {
...
<section id="postLoad">
    <!-- <img style="display:block; margin:0 auto; width:125px;"  src="<?php // echo base_url() 
                                                                        ?>/images/loading.gif"> -->
</section>
...
<script type='text/javascript'>
    ...
    var loadBody = async () => {
        ...
        while (i < length) {
            ...
            if (data.avatar != '') {
                const fileExist = await checkFileExist(data.avatar);
                if (fileExist) {
                    var ilink = '<?php echo base_url() . 'uploads/' ?>' + data.avatar;
                } else {
                    if (data.sex != '') {
                        if (data.sex == 'Female') {
                            var ilink = '<?php echo base_url() . 'images/avatar/female.png' ?>';
                        } else {
                            var ilink = '<?php echo base_url() . 'images/avatar/male.png' ?>';
                        }
                    } else {
                        var ilink = '<?php echo base_url() . 'images/forms/logo.png' ?>'
                    }
                }
            } else {
                if (data.sex != '') {
            ...
            let image = ilink;
            // let image = (data.avatar != '') ? '<?php echo base_url() . 'uploads/' ?>' + data.avatar : '<?php echo base_url() . 'images/forms/logo.png' ?>',
            // let image = '<?php echo base_url() . 'uploads/' ?>' + data.avatar;
    ...
    function checkFileExist(filePath) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: '<?php echo base_url() . 'opl/isFileExists/' ?>' + filePath,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
