...
<table id="tableSort" class="tablesorter table table-striped table-bordered">
    <thead style="background: #E6EEEE;">
        <tr>
            ...
            <th class="text-center" colspan="6">4th Quarter</th>
            <th class="text-center" colspan="3">Achievement Test</th>
            <th class="text-center" rowspan="2">Final</th>
        ...
        <tr>
            ...
            <th class="text-center">AT Initial Grade</th>
            <th class="text-center">AT Transmuted Grade</th>
            ...
            <tr class="main">
                ...
                ?>
                    <td class="text-center pointer" data-target="#assess_details" data-toggle="modal" onclick="getIndividualAssessment('<?php echo $student->st_id ?>',<?php echo $subject_id ?>,<?php echo $k->code ?>, <?php echo $term ?>)" class="values">
                        <?php
                        $record = Modules::run('gradingsystem/getTotalScoreByStudent', $student->st_id, $k->code, $term, $subject_id, $this->session->userdata('employee_id'), $student->section_id);

                        $numberOfAssessment = Modules::run('gradingsystem/getEachScoreByStudent', $student->st_id, $k->code,   $term, $subject_id, 1, $student->section_id, $this->session->userdata('employee_id'), $strand_id);
                ...
                <td class="text-center">
                    <?php
                    $atScore = Modules::run('gradingsystem/getTotalScoreByStudent', $student->st_id, $subject_id . '04', $term, $subject_id, $this->session->userdata('employee_id'), $student->section_id);

                    $atAssess = Modules::run('gradingsystem/getEachScoreByStudent', $student->st_id, $subject_id . '04',   $term, $subject_id, 1, $student->section_id, $this->session->userdata('employee_id'), $strand_id);
                    if ($atAssess->num_rows() > 0):
                        $score = $atScore->row()->score;
                        $items = $atAssess->row()->TPS;

                        $atInitial = (($score / $items) * 100);
                        $atTrans = Modules::run('gradingsystem/new_gs/getTransmutation', round($atInitial, 2));
                        $atFinal = $atTrans * 0.3;
                    else:
                        $atInitial = 0;
                        $atTrans = 0;
                    endif;
                    echo round($atInitial, 2);
                    ?>
                </td>
                <td class="text-center">
                    <?php echo $atTrans; ?>
                </td>
                <td class="text-center"><?php echo $atFinal ?></td>
                <td class="text-center">
                    <?php
                    $gTotal = round($fourth + $atFinal);
                    echo $gTotal;
                    ?>
                </td>
                <td id="<?php echo $student->st_id ?>_btn_validate">
                    ...
                    ?>
                            <button id="<?php echo $student->st_id ?>_validate" onclick="validateGrade('<?php echo trim($student->st_id) ?>',<?php echo $subject_id ?>,<?php echo $term ?>, <?php echo $gTotal ?>)" class="btn btn-small btn-info hide">Validate</button>
                            <button id="<?php echo $student->st_id ?>_invalidate" onclick="invalidateGrade('<?php echo trim($student->st_id) ?>',<?php echo $subject_id ?>,<?php echo $term ?>, <?php echo $gTotal ?>)" class="btn btn-small btn-success">Validated</button>
                        <?php
                        else:
                        ?>
                            <button id="<?php echo $student->st_id ?>_validate" onclick="validateGrade('<?php echo trim($student->st_id) ?>',<?php echo $subject_id ?>,<?php echo $term ?>, <?php echo $gTotal ?>)" class="btn btn-small btn-info">Validate</button>
                            <button id="<?php echo $student->st_id ?>_invalidate" onclick="invalidateGrade('<?php echo trim($student->st_id) ?>',<?php echo $subject_id ?>,<?php echo $term ?>, <?php echo $gTotal ?>)" class="btn btn-small btn-success hide">Validated</button>
                        <?php
                        endif;
                    else:
