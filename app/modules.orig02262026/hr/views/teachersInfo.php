...
<div class="well col-lg-12">
    ...
    <div class="col-lg-6">
        ...
        <h4 style="color:black; margin:3px 0;">
            <span style="color:#BB0000;" id="grade">Position: 
                <?php
                // show all positions (possibly multiple departments)
            if (isset($employeePositions) && is_array($employeePositions) && count($employeePositions)) {
                ...
                // Fallback: if none marked primary, pick first
                if (!$activePosition && count($employeePositions)) {
                    $activePosition = $employeePositions[0];
                }
            ?>
                <!-- ACTIVE POSITION -->
                <span class="badge label-success" style="margin:3px;">
                    <?php echo ($activePosition->department ? $activePosition->department . ' - ' : '') . $activePosition->position; ?>
                    <small style="color:white">[ active ]</small>
                </span>

                <?php if (!empty($otherPositions)): ?>
                    <!-- OTHER POSITIONS POPOVER -->
                    <span
                        class="badge label-info pointer"
                        data-toggle="popover"
                        data-trigger="click"
                        data-placement="bottom"
                        data-html="true"
                        data-popover-target="#positions-popover-<?php echo $basicInfo->employee_id; ?>"
                        style="margin:3px;"
                        data-content='
                            <div id="positions-popover-<?php echo $basicInfo->employee_id; ?>" class="d-none">
                                <?php foreach ($otherPositions as $op): ?>
                                    <div style="margin-bottom:6px; padding-bottom:6px; border-bottom:1px solid #eee">
                                        <strong>
                                            <?php echo ($op->department ? $op->department . ' - ' : '') . $op->position; ?>
                                        </strong>
                                        <?php if($this->session->employee_id == $basicInfo->employee_id): ?>
                                        <button class="btn btn-xs btn-success btn-block"
                                            onclick="activateEmployeePosition(<?php echo $basicInfo->employee_id; ?>,<?php echo $op->position_id; ?>,<?php echo $op->user_id; ?>, <?= $op->dept_id ?>)">
                                        ...
                                        <?php if ($this->session->userdata('is_admin')): ?>
                                            <button class="btn btn-xs btn-danger btn-block"
                                                onclick="deleteEmployeePosition(<?php echo $basicInfo->employee_id; ?>,<?php echo $op->position_id; ?>)">
                        ...
                        style="margin:3px;"
                    >
                        +<?php echo count($otherPositions); ?> more
                    </span>
                <?php endif;
                 } else {
