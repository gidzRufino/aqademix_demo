...
<div class="row">
    ...
    List of Students
    <small id="num_students">
        [ <?= $allStudents->num_rows . ' / ' . $num_of_students ?> ]
    </small>

    <!-- RFID Scanner -->
    <input type="text" id="rfid"
           style="position:absolute; left:-1000px;"
           onchange="scanStudents(this.value)" />
    ...
    <?php if ($this->session->userdata('is_admin') || $this->session->userdata('is_superAdmin')): ?>
        ...
        <a href="#"
           onclick="document.location='<?= base_url('search/exportStudentListToExcell/profile_students_admission.section_id/') ?>'
           + $('#inputSection').val() + '/' + $('#inputSY').val()"
        ...
        <div class="form-group pull-right">
            <select id="inputSY"
                    class="populate select2-offscreen span2"
                    style="width:200px;font-size:15px;"
                    onclick="getStudentByYear(this.value)">
                <option>School Year</option>
                <?php foreach ($ro_year as $ro): ?>
                    <option value="<?= $ro->ro_years ?>"
                        <?= ($this->session->userdata('school_year') == $ro->ro_years ? 'selected' : '') ?>>
        ...
        <div class="form-group pull-right">
            <select id="sem"
                    class="populate select2-offscreen span2"
                    style="width:200px;font-size:15px;"
                    onchange="getStudentByTerm(this.value)">
