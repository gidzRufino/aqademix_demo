...
class registrar_model extends CI_Model
{
    ...
    function getLatestIDs()
    {
        ...
        if ($q->num_rows() == 0):
            ...
            $this->db->order_by('esk_profile_students_admission_code', 'DESC');
            $query = $this->db->get();
    ...
    function generateTempId($st_id, $lastThree)
    {
        ...
        if ($g->num_rows() > 0):
            ...
            $this->db->insert('profile_temp_id', array('temp_id' => $this->eskwela->code(), 'generated_id' => $gen_id));
        else:
            $gen_id = $st_id;
            $this->db->insert('profile_temp_id', array('temp_id' => $this->eskwela->code(), 'generated_id' => $gen_id));
        endif;
    ...
    function getGradeLevelById($id = NULL)
    {
        ($id != NULL ? $this->db->where('grade_id', $id) : '');
        $query = $this->db->get('grade_level');
    ...
    function getEducAttainByID($id)
    {
        $this->db->where('ea_id', $id);
        return $this->db->get('profile_educ_attain')->row();
    }
    ...
    function saveProfile($data, $user_id = NULL, $school_year = NULL)
    {
        ...
        if ($user_id != NULL):
            ...
            if ($q->num_rows() > 0):
                ...
                $this->db->select('user_id');
                $this->db->where('esk_profile_code', $id);
    ...
    function setBarangay($barangay, $school_year = NULL)
    {
        ...
        if ($query->num_rows() > 0):
            ...
            $code = $this->eskwela->code();
            $data = array('barangay_id' => $code, 'barangay' => $barangay);
    ...
    function setAddress($data, $school_year = NULL)
    {
        ...
        $this->db->where('esk_profile_address_info_code', $id);
        $aid = $this->db->get('profile_address_info')->row();
    ...
    function saveOverload($st_id, $level, $section, $subject, $ifRegular, $term)
    {
        $this->db->where('st_id', $st_id);
        $this->db->where('level_id', $level);
        $this->db->where('sec_id', $section);
        $this->db->where('sub_id', $subject);
        $this->db->where('sem', ($term != NULL ? $term : 0));
        ...
        $data = array(
            ...
            'sec_id' => $section,
            'sub_id' => $subject,
            'sem' => ($term != NULL ? $term : 0),
            'is_overload' => ($ifRegular == 0 ? 2 : 1)
        );
        ...
        if ($q->num_rows() > 0):
            ...
            $this->db->where('sub_id', $subject);
            $this->db->where('sem', ($term != NULL ? $term : 0));
            $this->db->update('subject_overload', $data);
        else:
            $this->db->insert('subject_overload', $data);
        endif;
    }
    ...
    function delSelSubj($id)
    {
        ...
        $this->db->delete('subject_overload');
    }

    function autoGenPass($stid, $key, $uType)
    {
        ...
        if ($q->num_rows() > 0):
            ...
            $this->db->where('uname', $stid);
            $q = $this->db->update('ua_students', $data);
        else:
            $data = array(
                'u_id' => $stid,
                'uname' => $stid,
