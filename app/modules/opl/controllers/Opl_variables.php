...
class Opl_variables extends MX_Controller
{
    ...
    function sendReply()
    {
        ...
        if ($response == TRUE):
            ...
            //     Modules::run('notification_system/sendNotification', 8, 3, 'system', $this->session->oplSessions['section'], $remarks, date('Y-m-d'));
            // endif;
            if ($this->session->isStudent == 1):
                $rprofile = $this->getStudentBasicEdInfoByStId($this->session->st_id, $this->session->school_year);
            else:
                $rprofile = $this->getBasicEmployee($this->session->employee_id, $this->session->school_year);
            endif;
            $assets = $this->eskwela->getSet();
            $avatar = site_url("/uploads/") . $rprofile->avatar;
            $avloc = FCPATH . "uploads/" . ($rprofile->avatar ? $rprofile->avatar : "none.png");
            ...
            $html .= '<div id="reply-' . $replyCode . '" class="mb-3">' .
                '<div class="d-flex justify-content-between align-items-start">' .
                // '<img class="img-circle img-sm" src="' . $avatar . '" alt="User Image" />' .
                '<div>' .
                '<strong>' . ucwords(strtolower($rprofile->firstname . ' ' . $rprofile->lastname)) . '</strong>' .
                '<span class="comment-time"></span>' .
                '<div>' . nl2br(htmlspecialchars($this->post('reply'))) . '</div>' .
                '</div>' .
                '<i class="fa fa-trash text-danger" title="Delete reply" onclick="openDeleteModal(&qoutereply&qoute, &qoute' . $replyCode . '&qoute)"></i>' .
                '</div>' .
                '</div>';
            echo $html;
        else:
    ...
    function deleteComment()
    {
        ...
        $que = $this->opl_variables_model->deleteComment($id);
        if ($que):
            $this->opl_variables_model->deleteReply($id, null);
    ...
    {
        $id = $this->input->post('replyid');
        $q = $this->opl_variables_model->deleteReply($id, 1);
        if ($q):
            echo json_encode(['success' => true]);
        else:
            echo json_encode(['success' => false, 'message' => 'Delete failed']);
    ...
    function getCommentList($id, $type, $school_year)
    {
        return $this->opl_variables_model->getCommentList($id, $type, $school_year);
    }
}
