...
class Opl extends MX_Controller
{
    ...
    function cbPost()
    {
        ...
        // echo '<br>';
        // print_r($this->session->oplSessions['grade_level']);
        // echo '<br>';
        // print_r($this->session->oplSessions['section']);
        // echo '<br>';
        // print_r($this->session->oplSessions['subject_id']);
        // echo '<br>';
        // print_r($this->session->school_year);
        // echo '<br>';


        $data['postDetails'] = $this->opl_model->getTask($this->session->oplSessions['grade_level'], $this->session->oplSessions['section'], $this->session->oplSessions['subject_id'], $school_year);
        // print_r($data['postDetails']);
        print_r($this->load->view('cbPost', $data));
    }

    public function sbPost()
    {
        $data['post'] = $this->getPost(($this->session->isOplAdmin) ? NULL : $this->session->username);
    ...
    {
        $result = $this->opl_model->highlightPost($this->post('id'), $this->post('type'));
        if ($result == TRUE) :
            echo "Successfully highlited posts";
        else :
            echo "Something went wrong, please try again";
        endif;
    }

    function getStudent($stid)
    {
        return $this->opl_model->getStudent($stid);
    }

    function deletePost()
    {
        $post_id = $this->post('postid');
        $result = $this->opl_model->deletePost($post_id);
        if ($result == TRUE) :
    ...
    function gradeLevelList($type, $username = NULL, $search = NULL)
    {
        $t = strtolower($type);
        $data['type'] = $t;
        ...
        switch ($t):
            case 'grades':
                $data['grades']  = Modules::run('registrar/getGradeLevel');
                $data['college'] = Modules::run('coursemanagement/getCourses');
                break;
            case 'classes':
                $data['section']  = $this->opl_model->getSection($username, $search)->result();
                $data['classes'] = $this->opl_model->getClasses($username, $search)->result();
                break;
            case 'subjects':
                $data['subjects'] = $this->opl_model->getSubjectAssigned($username, $search)->result();
                break;
        endswitch;
        echo $this->load->view('gradelist', $data);
    }

    function create_message($grade_level, $section, $subject)
    {
        $classDetails = json_decode(Modules::run('opl/opl_variables/getClassDetails', $grade_level, $section, $subject, $this->session->school_year));
        $data['headerTitle'] = 'Create Message | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
        $data['gradeDetails'] = $classDetails->basicInfo;
        $data['section_id'] = $section;
        $data['subject_id'] = $subject;
        $data['grade_level'] = $grade_level;
        $data['title'] = '';
        $data['main_header'] = 'Create Message';
        $data['subjectDetails'] = $classDetails->subjectDetails;
        // $data['students'] = Modules::run('opl/student/getStudentsBySection', $grade_level, $section);
        $data['students'] = $this->getStudentsBySection($grade_level, $section);
        $data['login_page'] = 'login/logout';
    ...
    function getStudentsBySection($level, $section)
    {
        return $this->opl_model->getStudentsBySection($level, $section);
    }
    ...
    function uploadFile($file = NULL)
    {
        if ($file == NULL) :
            ...
            $type = strtolower($this->post('uploadType'));
            switch ($type):
                case 'task':
                    $grade = explode("-", $this->post('grade'))[0];
                    $config['upload_path'] = FCPATH . 'uploads/' . $this->session->school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $this->session->username . DIRECTORY_SEPARATOR . $grade . DIRECTORY_SEPARATOR . 'task';
                    break;
                case 'unit':
                    $config['upload_path'] = FCPATH . 'uploads/' . $this->session->school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $this->session->username . DIRECTORY_SEPARATOR . $this->post('subject_id') . DIRECTORY_SEPARATOR . 'unit';
                    break;
                case 'discussion':
                    $grade = explode("-", $this->post('grade'))[0];
                    $config['upload_path'] = FCPATH . 'uploads/' . $school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $this->session->username . DIRECTORY_SEPARATOR . $grade . DIRECTORY_SEPARATOR . 'discussion';
                    break;
            endswitch;
            if (!is_dir($config['upload_path'] . DIRECTORY_SEPARATOR)) :
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                print_r($data);
                echo $config['upload_path'] . DIRECTORY_SEPARATOR;
            ...
            switch ($type):
                ...
                case 'unit':
                    $data = array(
                        'ou_unit_attachments'  =>  $filename
                    );
                    break;
                case 'discussion':
                    $data = array(
                        'dis_attachments'   =>  $filename
                    );
                    break;
            endswitch;

            if ($this->opl_model->uploadFile($this->post('code'), $data, $type) == TRUE) :
                echo json_encode(array("msg" => "Successfully uploaded a file"));
            else :
        ...
        switch (strtolower($link)):
            ...
            case 'student':
                switch (strtolower($this->uri->segment(3))):
                    case 'classbulletin':
                        if (strtolower($this->uri->segment(6)) == 'list') :
                            return $this->load->view('students/taskSearch', $data);
                        endif;
                        break;
                endswitch;
                break;
...
?>
    ...
    {
        $rubricArray = array(
            'srid'          => $this->eskwela->codeCheck('opl_rubric_student', 'srid', $this->eskwela->code()),
            'sr_stid'       => $this->post('sr_stid'),
            'sr_ref_id'     => $this->post('sr_ref_id'),
            'sr_rcid'      => $this->post('rcid'),
    ...
    function updateStudentRubric($st_id, $task_id)
    {
        return $this->opl_model->updateStudentRubric($st_id, $task_id);
    }
    ...
    public function searchRubric()
    {
        $query = $this->input->get('q');
        $results = $this->opl_model->searchRubric($query);
    ...
    public function rubricList($school_year)
    {
        ...
        $classDetails = json_decode(Modules::run('opl/opl_variables/getClassDetails', $this->session->oplSessions['grade_level'], $this->session->oplSessions['section'], $this->session->oplSessions['subject_id'], $this->session->school_year));
        $data['headerTitle'] = 'List of Available Rubric | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
        $data['title'] = 'Learn Manager';
        $data['subjectDetails'] = [];
        $data['gradeDetails'] = [];
        $data['main_header'] = '';
    ...
    {
        if ($this->post('rcid') == 0) :
            $rcid = $this->eskwela->codeCheck('opl_rubric_criteria', 'rcid', $this->eskwela->code());
        else :
            $rcid = $this->post('rcid');
        endif;
        $school_year = $this->post('school_year');
        ...
        if ($this->opl_model->saveCriteria($criteriaDetails, $school_year, $this->post('rcid'), $rcid, $this->post('ruid'), $this->post('criteriaName'))) :
            ...
            foreach ($scales as $s) :
                $sc++;
                $rdDetails = array(
                    'rdid'      => $this->eskwela->codeCheck('opl_rubric_description', 'rdid', $this->eskwela->code()),
                    'rd_rcid'   => $rcid,
                    'rd_scale'  => $s->rd_scale,
    ...
    {
        $ruid = $this->eskwela->codeCheck('opl_rubric_info', 'ruid', $this->eskwela->code());
        $details = array(
            'ruid'          => $ruid,
    ...
    function updateRubric()
    {
        ...
        $alias = $this->input->post('ru_alias');
        $type = $this->input->post('ru_type');
        $scale = $this->input->post('ri_scale');
        ...
        $this->opl_model->updateRubric($details, $ruid);
    }
    ...
    public function createRubric($school_year, $ruid = NULL)
    {
        if (!$this->session->is_logged_in) :
            ...
            $classDetails = json_decode(Modules::run('opl/opl_variables/getClassDetails', $this->session->oplSessions['grade_level'], $this->session->oplSessions['section'], $this->session->oplSessions['subject_id'], $this->session->school_year));
            $data['headerTitle'] = 'Create a Rubric | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
            $data['title'] = 'Learn Manager';
            $data['subjectDetails'] = [];
            $data['gradeDetails'] = [];
            $data['main_header'] = '';
    ...
    function printAdmission($st_id)
    {
        $studentDetails = array(
            ...
            'details'   => Modules::run('registrar/getSingleStudent', base64_decode($st_id), null, null),
    ...
    public function getAssessCatDropdown($details = NULL, $year = NULL, $component_id = NULL)
    {
        ...
        if ($gs_settings->customized) :
            ...
            foreach ($cat as $category) {
            ?>
                <option value="<?php echo $category->code ?>" <?= $component_id == $category->code ? 'selected' : '' ?>><?php echo $category->component ?></option>
    ...
    {
        $code = $this->post('code');
        $response = $this->opl_model->deleteRubric($code);
        if ($response == TRUE) :
            echo json_encode(array("message" => "Successfully removed rubric"));
        else :
            echo json_encode(array("message" => "Something went wrong with deleting the rubric"));
        endif;
    }
    ...
    function deleteTasks()
    {
        ...
        if ($response == TRUE) :
            $this->opl_model->deleteAssessment($code);
    ...
    // {
    //     $school_year = $this->session->school_year;
    //     $taskGrade = explode('-', $this->post('taskGrade'));
    //     $taskCode = $this->post('taskCode');
    //     $level = explode("-", $this->post('taskGrade'));
    //     $file = $_FILES['userfile'] ?? null;
    //     $data['error'] = '';
    //     $config['upload_path'] = 'uploads/' . $school_year . '/faculty/' . $this->session->username . '/' . $taskGrade[0] . '/tasks';
    ...
    //         'task_submission_type' => $this->post('submissionType'),
    ...
    public function updateTasks()
    {
        ...
        $taskGrade   = explode('-', $this->input->post('taskGrade'));
        $taskCode    = $this->input->post('taskCode');
        $oldFile     = $this->input->post('taskAttachment');     // old file name
        $currentFile = $this->input->post('currentAttachment');  // hidden field, if set keep it
        $file        = $_FILES['userfile'] ?? null;

        $attachment = null; // default

        // ✅ Upload path
        $uploadPath = FCPATH . 'uploads/' . $school_year . '/faculty/' . $this->session->username . '/' . $taskGrade[0] . '/task';
        $alink = 'uploads/' . $school_year . '/faculty/' . $this->session->username . '/' . $taskGrade[0] . '/task';
        ...
        } elseif (!empty($currentFile)) {
            // ✅ No new upload, keep existing one
        ...
        // ✅ Prepare data for DB
        $data = [
            'task_title'           => $this->input->post('taskTitle'),
            'task_details'         => $this->input->post('taskDetails'),
            'task_type'            => $this->input->post('taskType'),
            'task_section_id'      => $taskGrade[2] ?? null,
            'task_subject_id'      => $taskGrade[0] ?? null,
            'task_grade_id'        => $taskGrade[1] ?? null,
            'task_attachments'     => $attachment,
            'task_submission_type' => $this->input->post('submissionType'),
            'task_lesson_id'       => $this->input->post('taskUnitLink'),
            'task_start_time'      => date('Y-m-d', strtotime($this->input->post('taskStartDate'))) . ' ' . date('H:i:s', strtotime($this->input->post('taskTimeStart'))),
            'task_end_time'        => date('Y-m-d', strtotime($this->input->post('taskEndDate'))) . ' ' . date('H:i:s', strtotime($this->input->post('taskTimeEnd'))),
            'task_term'            => $this->input->post('taskTerm'),
            'task_total_score'     => $this->input->post('numItems'),
            'attachment_link'      => ($attachment != null ? $alink : ''),
            'gs_component_id'      => $this->input->post('gsComponent')
        ];
        ...
        if ($response === TRUE) {
            $details = [
                'assess_title' => $this->input->post('taskTitle'),
                'assess_date'  => date('Y-m-d', strtotime($this->input->post('taskStartDate'))),
                'section_id'   => $taskGrade[2] ?? null,
                'subject_id'   => $taskGrade[0] ?? null,
                'no_items'     => $this->input->post('numItems'),
                'term'         => $this->input->post('taskTerm'),
                'quiz_cat'     => $this->input->post('gsComponent')
            ];
            $this->opl_model->updateAssessment($taskCode, $details);
    ...
    function getDiscussionDetails($discuss_id, $school_year = NULL)
    {
        echo json_encode($this->opl_model->getDiscussionDetails($discuss_id, $school_year));
    }

    public function discussionDetails($discuss_id, $school_year)
    {
        $classDetails = json_decode(Modules::run('opl/opl_variables/getClassDetails', $this->session->oplSessions['grade_level'], $this->session->oplSessions['section'], $this->session->oplSessions['subject_id'], $school_year));
        $taskDetails = array(
            'isClass' => FALSE,
            ...
            'main_header' => '',
            'grade_level' => $this->session->oplSessions['grade_level'],
            'section_id' => $this->session->oplSessions['section'],
            'subject_id' => $this->session->oplSessions['subject_id'],
            'title' => 'Online Platform for Learning',
        ...
        if ($this->session->isStudent) :
            $this->opl_model->addReader($this->session->username, $discuss_id);
        endif;

        echo Modules::run('templates/opl_content', $taskDetails);
    }

    public function getPost($id = NULL, $grade_id = NULL, $section_id = NULL)
    {
        $postDetails = $this->opl_model->getPost($id, $grade_id, $section_id);
        return $postDetails;
    }
    ...
    public function submitQuickPost()
    {
        ...
        $type = ($this->post('type') != "none") ? $this->post('type') : NULL;
        $targets = NULL;

        if ($type == '3' || $type == '4' || $type == '5') :
            $targets = $this->post('targets');
        endif;
    ...
    public function sendDiscussComment()
    {
        ...
        $com_syscode = $this->eskwela->codeCheck('opl_comments', 'com_sys_code', $this->eskwela->code());
    ...
    function getTaskDetails($task_id, $school_year = NULL)
    {
        echo json_encode($this->opl_model->getTaskDetails($task_id, $school_year));
    }
    ...
    public function viewTaskDetails($task_id, $grade_level, $section, $subject, $school_year = NULL)
    {
        ...
        $taskDetails = array(
            ...
            'unitDetails' => Modules::run('opl/opl_variables/getAllUnits', $this->session->username, $sy, $grade_level),
            'basicInfo' => $classDetails->basicInfo,
            'subjectDetails' => $classDetails->subjectDetails,
            'school_year' => $sy,
            'headerTitle' => 'Task Details | ' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]',
            'main_header' => '',
            'title' => 'Online Platform for Learning',
    ...
    public function getTaskByCode($code, $school_year)
    {
        ...
        foreach ($task as $t) :
            echo '<li> <a href="' . base_url('opl/viewTaskDetails/') . $t->task_code . '/' . $t->task_grade_id . '/' . $t->task_section_id . '/' . $t->task_subject_id . '/' . $school_year . '">' . $t->task_title . '</a></li>';
        endforeach;
    }
    ...
    function getUnitDetails($code, $school_year)
    {
        ...
        if ($unitDetails->ou_unit_attachments != '') :
            ...
            $unitDetails->file = site_url(FCPATH . 'uploads/' . $school_year) . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $unitDetails->ou_owners_id . DIRECTORY_SEPARATOR . $unitDetails->ou_subject_id . DIRECTORY_SEPARATOR . 'unit' . DIRECTORY_SEPARATOR . $unitDetails->ou_unit_attachments;
        endif;
    ...
    public function saveUnit()
    {
        ...
        if ($hasUpload == 1) :
            ...
            $config['upload_path'] = FCPATH . 'uploads/' . $school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $this->session->username . DIRECTORY_SEPARATOR . $this->post('subject_id') . DIRECTORY_SEPARATOR . 'unit';
            if (!is_dir($config['upload_path'] . '/')) :
                mkdir($config['upload_path'], 0777, TRUE);
            endif;
        ...
        $unitDetails = array(
            ...
            'ou_opl_code' => $this->eskwela->codeCheck('opl_units', 'ou_opl_code', $this->eskwela->code())
        );
        $isSave = $this->opl_model->saveUnit($unitDetails, $this->post('unitTitle'), $school_year);

        if ($isSave) :
            echo json_encode(array('msg' => 'Lesson Successfully Added'));
        elseif ($isSave == 2) :
            echo json_encode(array('msg' => 'Lesson Title Already Exist'));
        else :
    ...
    public function addDiscussion($code = NULL)
    {
        ...
        if ($hasUpload == 1) :
            ...
            $config['upload_path'] = 'uploads/' . $school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $this->session->username . DIRECTORY_SEPARATOR . $subGrSec[0] . DIRECTORY_SEPARATOR . 'discussion';
            if (!is_dir($config['upload_path'] . '/')) :
                mkdir($config['upload_path'], 0777, TRUE);
            endif;
        ...
        $discussionDetails = array(
            ...
            'dis_subject_id'    => $subGrSec[0],
            'dis_grade_id'      => $subGrSec[1],
            'dis_section_id'    => $subGrSec[2],
            // 'dis_section_id'    => ($isPublic == 0 ? $subGrSec[2] : 0),
            'dis_start_date'    => $this->post('startDate'),
            'dis_author_id'     => $this->session->username,
            'dis_attachments'   =>  $uploaded_file,
            'dis_sys_code'      => $this->eskwela->codeCheck('opl_discussion', 'dis_sys_code', $this->eskwela->code())
        );

        if ($this->opl_model->addDiscussion($discussionDetails, $code, $school_year)) :
            $remarks = $this->session->name . ' has added a lesson for Discussion ';
            Modules::run('notification_system/sendNotification', 8, 3, 'system', $subGrSec[2], $remarks, $this->post('startDate'));
            echo json_encode(array('msg' => 'Successfully Saved'));
        else :
            echo json_encode(array('msg' => 'Sorry, Something went wrong. Please try again later'));
        endif;
    }
    ...
    public function discussionBoard($school_year = NULL, $employee_id = NULL, $grade_level = NULL, $section = NULL, $subject = NULL)
    {
        ...
        $data = array(
            ...
            'subjectDetails' => $classDetails->subjectDetails,
            'headerTitle' => 'Discussion Board | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>',
            'main_header' => '',
            'login_page' => 'login/logout',
            'title' => 'Online Platform for Learning',
            'modules' => 'opl',
        ...
        $data['sid'] = $section;
        if (!$this->session->isOplAdmin) :
            $data['discussionDetails'] = Modules::run('opl/opl_variables/getDiscussionList', ($employee_id == NULL ? $this->session->username : $employee_id), $grade_level, $section, $subject, $school_year);
        else :
            $data['discussionDetails'] = Modules::run('opl/opl_variables/getDiscussionList', NULL, $grade_level, $section, $subject, $school_year);
    ...
    public function newDiscussion($school_year = NULL)
    {
        ...
        $data = array(
            ...
            'isClass' => FALSE,
            'school_year' => $school_year,
            'gradeDetails' => array('grade_level_id' => $this->session->oplSessions['grade_level'], 'section_id' => $this->session->oplSessions['section']),
            'subjectDetails' => array('subject_id' => $this->session->oplSessions['subject_id']),
            'getSubjects' => Modules::run('opl/opl_widgets/mySubject', $this->session->username, $school_year),
            'headerTitle' => 'Create A Discussion | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>',
            'main_header' => '',
            'title' => 'Online Platform for Learning',
            'login_page' => 'login/logout',
            'modules' => 'opl',
    ...
    public function addTask()
    {
        ...
        $sys_code = $this->eskwela->codeCheck('opl_tasks', 'task_code', $this->eskwela->code());
        $school_year = $this->session->school_year;

        $hasUpload = $this->post('hasUpload');
        $alink = 'uploads/' . $school_year . '/faculty/' . $this->session->username . '/' . $subGradSec[0] . '/task';
        if ($hasUpload == 1) :
            $file = $this->post('userfile');
            $data['error'] = ''; //initialize image upload error array to empty
            ...
            $config['upload_path'] = FCPATH . 'uploads/' . $school_year . '/faculty/' . $this->session->username . '/' . $subGradSec[0] . '/task';
            // $config['upload_path'] = 'uploads/' . $school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $this->session->username . DIRECTORY_SEPARATOR . $subGradSec[0] . DIRECTORY_SEPARATOR . 'task';
            if (!is_dir($config['upload_path'] . '/')) :
                mkdir($config['upload_path'], 0777, TRUE);
            endif;
        ...
        $taskDetails = array(
            ...
            'task_start_time' => date('Y-m-d', strtotime($this->post('startDate'))) . ' ' . date('H:i:s', strtotime($this->post('timeStart'))),
            'task_end_time' => date('Y-m-d', strtotime($this->post('deadlineDate'))) . ' ' . date('H:i:s', strtotime($this->post('timeDeadline'))),
            ...
            'task_term' => $this->post('inputTerm'),
            'attachment_link' => ($uploaded_file != '' ? $alink : ''),
            'gs_component_id' => $this->input->post('gsComponent')
        );
    ...
    public function index()
    {
        if (!$this->session->has_userdata('isOplAdmin')) :
            if (
                $this->session->position == "Super Administrator"  ||
        ...
        $data = array(
            'isClass' => FALSE,
            'post' => $this->getPost(($this->session->isOplAdmin) ? NULL : $this->session->username),
            'subjectDetails' => Modules::run('opl/opl_widgets/mySubject', $this->session->username, $this->session->school_year),
            'school_year'   =>  $this->session->school_year,
            'headerTitle' => 'Dashboard - School Bulletin',
            'main_header' => '<strong>Sneak Peek!</strong> Welcome!</strong>.',
            'title' => 'Online Platform for Learning',
            'login_page' => 'login/logout',
    ...
    {
        $classDetails = json_decode(Modules::run('opl/opl_variables/getClassDetails', $this->session->oplSessions['grade_level'], $this->session->oplSessions['section'], $this->session->oplSessions['subject_id'], $school_year));
        $data['title'] = 'Online Platform for Learning';
        $data['subj_id'] = $this->session->oplSessions['subject_id'];
        $data['grade_id'] = $this->session->oplSessions['grade_level'];
        $data['section_id'] = $this->session->oplSessions['section'];
        $data['subjectDetails'] = [];
        $data['gradeDetails'] = [];
        $data['main_header'] = '';
        $data['unitDetails'] = $classDetails->unitDetails;
        $data['isClass'] = FALSE;
        $data['getSubjects'] = Modules::run('opl/opl_widgets/mySubject', $this->session->username, $school_year);
        $data['school_year'] = $school_year;
        $data['headerTitle'] = 'New Task | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
    ...
    public function classBulletin($school_year = NULL, $task = NULL, $grade_level, $section, $subject)
    {
        ...
        $data = array(
            ...
            'headerTitle' => 'Class Bulletin | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>',
            'main_header' => '<strong>Sneak Peek!</strong> Welcome to <strong>Learn Manager </strong>.',
            'login_page' => 'login/logout',
            'title' => 'Learn Manager',
            'modules' => 'opl',
        );
        ...
        switch ($task):
            case 'Students':
                ...
                $data['headerTitle'] = 'Class Lists | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
                $data['main_content'] = 'classList';
                break;
            case 'List':
                if ($grade_level != NULL) :
                    $data['headerTitle'] = 'Task List | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
                    $data['main_header'] = '';
                    $data['subject_id'] = $subject;
                    $data['grade_level'] = $grade_level;
                    $data['section_id'] = $section;
                    $data['school_year'] = $school_year;
                    $data['tasks'] = $this->opl_model->getTask($grade_level, $section, $subject, $school_year, null);
            ...
            default:
                ...
                $data['posts'] = $this->getPost($this->session->username, $grade_level, $section);
    ...
    {
        $data['tasks'] = json_decode($this->input->post('task'), true);
        $data['term'] = $this->input->post('term');
        $data['sy'] = $this->input->post('sy');
        echo $this->load->view('tasks/tr', $data);
    }
    ...
    function unitView($school_year = NULL, $task = NULL, $grade_level = NULL, $section = NULL, $subject = NULL)
    {
        ...
        switch ($task):
            case 'Add':
                $data['headerTitle'] = 'Add Unit | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
                $data['section_id'] = $section;
                $data['subject_id'] = $subject;
                $data['grade_level'] = $grade_level;
                ...
                break;
            case 'List':
                $data['headerTitle'] = 'List Unit | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ]</small>';
                if ($grade_level != NULL) :
                    $data['subject_id'] = $subject;
                    $data['grade_level'] = $grade_level;
...
<?php
    ...
    function taskNotif($grade_level, $section, $subject)
    {
        ...
        if ($this->session->employee_id) :
            if ($this->session->basicInfo->position_id == 39) :
                $uid = $this->session->employee_id;
                $subject_id = $this->session->oplSessions["subject_id"];
                $grade_id = $this->session->oplSessions["grade_level"];
                $section_id = $this->session->oplSessions["section"];

                $taskNotif = Modules::run('opl/opl_variables/taskNotif', $this->session->school_year, $uid, $subject_id, $grade_id, $section_id)->result();
                foreach ($taskNotif as $t) :
                    if (date('Y-m-d H:i:s', strtotime($t->task_start_time)) <= $dateNow) :
                        $cnt++;
                        if (date('Y-m-d H:i:s', strtotime($t->task_end_time)) <= $dateNow) :
                            $cnt--;
                        endif;
                    endif;
                endforeach;

            elseif ($this->session->basicInfo->position_id == 38) :

                $taskNotif = Modules::run('opl/opl_variables/taskNotif', $this->session->school_year, null, null, null, null)->result();
            ...
            if ($this->session->isStudent == 1) :
                $subject_id = ($this->session->oplSessions["subject_id"] ? $this->session->oplSessions["subject_id"] : null);
                $grade_id = $this->session->details->grade_level_id;
                $section_id = $this->session->details->section_id;

                $taskNotif = Modules::run('opl/opl_variables/taskNotif', $this->session->school_year, null, $subject_id, $grade_id, $section_id)->result();
                foreach ($taskNotif as $t) :
    ...
    function unitNotif($grade_level, $section, $subject)
    {
        ...
        if ($this->session->employee_id) :
            if ($this->session->basicInfo->position_id == 39) :
                ...
                $unitNotif = Modules::run('opl/opl_variables/unitNotif', $this->session->school_year, $uid, $dateNow, $subject_id, $grade_id, null);

            elseif ($this->session->basicInfo->position_id == 38) :
                $unitNotif = Modules::run('opl/opl_variables/unitNotif', $this->session->school_year, null, $dateNow, null, null, null);
            endif;
        else :
        ...
        print_r($unitNotif->num_rows());
    }


    function discussNotif()
    {
        ...
        if ($this->session->employee_id) :
            if ($this->session->basicInfo->position_id == 39) :
                ...
                $discussNotif = Modules::run('opl/opl_variables/discussNotif', $this->session->school_year, $uid, $subject_id, $grade_id, $section_id)->result();
                foreach ($discussNotif as $d) :
                    if (date('Y-m-d H:i:s', strtotime($d->dis_start_date)) <= $dateNow) :
                        $cnt++;
                    endif;
                endforeach;

            elseif ($this->session->basicInfo->position_id == 38) :
                $discussNotif = Modules::run('opl/opl_variables/discussNotif', $this->session->school_year, null, null, null, null, null);
            ...
            if ($this->session->isStudent == 1) :
                ...
                $discussNotif = Modules::run('opl/opl_variables/discussNotif', $this->session->school_year, null, $subject_id, $grade_id, $section_id, $this->session->isStudent)->result();
