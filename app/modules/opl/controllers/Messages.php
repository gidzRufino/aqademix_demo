...
class Messages extends MX_Controller
{
    ...
    function employee_inbox($emid, $subject = NULL, $grade_id = NULL, $section_id = NULL)
    {
        ...
        $data['headerTitle'] = 'Inbox | <small style="font-size: medium">' . $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ] </small>';
        $data['gradeDetails'] = $classDetails->basicInfo;
        $data['subjectDetails'] = $classDetails->subjectDetails;
        $data['messages'] = $page->result();
        $data['login_page'] = 'login/logout';
    ...
    function readMsge($id, $subject, $grade_id = NULL, $section_id = NULL, $isReply = NULL, $msg_id = NULL, $uid = null)
    {
        $sid = base64_decode($id);
        // $mainMsg = $this->messages_model->getMainMsg($id);
        // $replyMsg = $this->messages_model->getReplyMsg($id);
        $reply = [];
        $classDetails = json_decode(Modules::run('opl/opl_variables/getClassDetails', $grade_id, $section_id, $subject, $this->session->school_year));
        $data['headerTitle'] = $classDetails->subjectDetails->subject . ' - ' . $classDetails->basicInfo->level . ' [ ' . $classDetails->basicInfo->section . ' ] Class Bulletin';
        $data['mid'] = $sid;
        ...
        $data['msg'] = $this->messages_model->getMainMsg($sid, $uid, $isReply);
        $data['login_page'] = 'login/logout';
    ...
    function sendMsg()
    {
        ...
        $recipient = json_decode($this->input->post('recipient'), true);
    ...
    function getUnreadMsg($id, $subj_id = null)
    {
        return $this->messages_model->getUnreadMsg(base64_decode($id), $subj_id);
    }

    function getAllMsge($id)
    {
        return $this->messages_model->getAllMsge($id);
    }

    function getLatestTime($id)
    {
        return $this->messages_model->getLatestTime($id);
    }

    function getOrigMsg($pid)
    {
        return $this->messages_model->getOrigMsg($pid);
    }
}
