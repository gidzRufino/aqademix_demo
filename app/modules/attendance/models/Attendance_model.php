...
class attendance_model extends CI_Model
{
    ...
    {
        $this->db = $this->eskwela->db($year != NULL ? $year : $this->session->school_year);
        $this->db->select('*');
        ...
        if ($query->row()->account_type == 5):
            $this->db->select('*');
            $this->db->select('profile.user_id as uid');
            $this->db->select('profile_students_admission.section_id as sec_id');
    ...
    function timegroup($tg_id)
    {
        $this->db->select('*');
        $this->db->from('payroll_shift');
        $this->db->where('ps_id', $tg_id);
        $query = $this->db->get();
        return $query->row();
    }

    function default_time($day)
    {
        $this->db->select('*');
        $this->db->from('profile_employee_time_settings');
    ...
    function getIndividualMonthlyAttendance($student_id, $month, $year = null, $sy)
    {
        ...
        $pMonth = strtolower(date('M', strtotime($from)));
        $this->db->select('*');
        $this->db->from('attendance_sheet_' . $pMonth);
    ...
    function getEmployeeAttendance($date)
    {
        ...
        $this->db->select('*');
        $this->db->from('attendance_sheet_' . $month);
        $this->db->join('profile_employee', 'attendance_sheet_' . $month . '.att_st_id = profile_employee.employee_id', 'inner');
        $this->db->join('profile', 'profile_employee.user_id = profile.user_id', 'inner');
        $this->db->where('attendance_sheet_' . $month . '.date', $date);
        $this->db->where('account_type !=', 5);
    ...
    function getAttendanceAutoManual($section, $date = NULL)
    {
        ...
        $this->db = $this->eskwela->db($this->session->userdata('school_year'));
        $this->db->select('*');
        $this->db->select('profile_students.st_id as studID');
        $this->db->from('attendance_sheet_' . $month);
        $this->db->join('profile_students', 'attendance_sheet_' . $month . '.att_st_id = profile_students.st_id', 'left');
    ...
    function getCollegeAttendance($date = NULL)
    {
        ...
        $this->db->select('*');
        $this->db->from('attendance_sheet_' . $month);
        $this->db->join('profile', 'attendance_sheet_' . $month . '.u_rfid = profile.rfid', 'inner');
        ...
        if ($query->num_rows() == 0):
            ...
            $mo = strtolower(date('M', strtotime($date)));

            $this->db->select('*');
            $this->db->from('attendance_sheet_' . $mo);
            $this->db->join('profile_students', 'attendance_sheet_' . $mo . '.u_rfid = profile_students.st_id', 'left');
            $this->db->join('profile', 'profile_students.user_id = profile.user_id', 'inner');
            $this->db->join('profile_students_c_admission', ' profile_students_c_admission.user_id = profile.user_id', 'inner');
            $this->db->where('attendance_sheet_' . $mo . '.date', $date);
            $this->db->where('semester', $sem);
            $this->db->where('account_type', 5);
            $this->db->order_by('lastname', 'ASC');
            $query = $this->db->get();
    ...
    function getAbsents($section, $attend_auto, $date)
    {
        ...
        $month = strtolower(date('M', strtotime($date)));
        $attendance_sheet = 'esk_attendance_sheet_' . $month;
        $column_id = 'att_st_id';
        ...
        $this->db = $this->eskwela->db($year);
        $query = $this->db->query("Select *, esk_profile_students.st_id as s_id from esk_profile_students 
    ...
    function saveme($details)
    {
        $this->db->insert('attendance_sheet', $details);
        return $this->db->insert_id();
    }

    function saveTimeAttendance($details, $rfid, $date = NULL, $st_id = NULL)
    {
        $month = strtolower(date('M', strtotime($date)));
        // $this->db->insert('attendance_sheet_' . $month, $details);
        //s$settings = $this->eskwela->getSet();
        // $this->db = $this->eskwela->db($this->session->userdata('school_year'));
        if ($date == null):
            $date = date("Y-m-d");
        endif;
        ...
        // $this->db->or_where('att_st_id', $st_id);
        $q = $this->db->get('attendance_sheet_' . $month);
        if ($q->num_rows() > 0):
    ...
    function updateTimeAttendance($details, $rfid, $date)
    {
        ...
        $this->db->update('attendance_sheet_' . $month, $details);
    ...
    function ifPresent($st_id, $day, $month, $year = Null, $sy = NULL)
    {
        ...
        $this->db->where('date', $date);
        $query = $this->db->get('attendance_sheet_' . $mo);
        ...
        if ($query->num_rows() > 0):
            ...
            $this->db->where('date', $date);
            $query2 = $this->db->get('attendance_sheet_' . $mo);
    ...
    function getDailyTotalByGender($date, $section, $gender)
    {
        ...
        $this->db->join('profile', 'profile_students.user_id = profile.user_id', 'left');
        $this->db->join('attendance_sheet_' . $month, 'profile_students.st_id = attendance_sheet_' . $month . '.att_st_id', 'left');
        $this->db->join('profile_students_admission', ' profile_students_admission.user_id = profile.user_id', 'left');
        $this->db->join('section', 'section.section_id = profile_students_admission.section_id', 'left');
        $this->db->join('grade_level', 'grade_level.grade_id = section.grade_level_id', 'left');
        // $this->db->join('daily_remarks', 'daily_remarks.remark_to_id = profile.user_id', 'left');
        ...
        if ($gender != NULL):
            $this->db->where('profile.sex', $gender);
            $this->db->group_by('attendance_sheet_' . $month . '.att_st_id');
        endif;
    ...
    function getPresentByDate($section, $date)
    {
        ...
        $this->db->from('profile_students');
        $this->db->join('profile', 'profile_students.user_id = profile.user_id', 'left');
        $this->db->join('attendance_sheet_' . $month, 'profile.rfid = attendance_sheet_' . $month . '.u_rfid', 'left');
        $this->db->join('profile_students_admission', ' profile_students_admission.user_id = profile.user_id', 'left');
        $this->db->join('section', 'section.section_id = profile_students_admission.section_id', 'left');
        $this->db->join('grade_level', 'grade_level.grade_id = section.grade_level_id', 'left');
        $this->db->where('attendance_sheet_' . $month . '.date', "$date");
    ...
    function getAbsentByDatePerGender($section, $date, $attend_auto, $gender)
    {
        ...
        if ($attend_auto):
            $this->db->select('*');
            $this->db->from('attendance_sheet_' . $month);
            $this->db->join('profile_students', 'attendance_sheet_' . $month . '.st_id = profile_students.st_id', 'left');
            $this->db->join('profile', 'profile_students.user_id = profile.user_id', 'left');
    ...
    function addAttendanceRemark($details, $rfid, $date, $year = NULL)
    {
        $this->db = $this->eskwela->db($year == NULL ? $this->session->userdata('school_year') : $year);
        $month = strtolower(date('M', strtotime($date)));
        $this->db->where('date', $date);
        $this->db->where('att_st_id', $rfid);
        $q = $this->db->get('attendance_sheet_' . $month);
    ...
    function getAttendanceRemark($st_id, $date, $studID, $sy = NULL)
    {
        $month = strtolower(date('M', strtotime($date)));
        $this->db = $this->eskwela->db(($sy == NULL ? $this->session->school_year : $sy));
        $this->db->select('*');
        $this->db->from('attendance_sheet_' . $month);
        $this->db->join('remarks_category', 'attendance_sheet_' . $month . '.remarks = remarks_category.cat_id', 'left');
        $this->db->where('date', $date);
        $this->db->where('att_st_id', $st_id);
        $this->db->or_where('u_rfid', $st_id);
        $query = $this->db->get();
        return $query;
    }
    ...
    function getTardy($st_id, $month, $year, $sy)
    {
        if ($year == Null):
            $year = date('Y');
        endif;
    ...
    function deleteAttendance($att_id, $date)
    {
        $month = strtolower(date('M', strtotime($date)));
        $this->db = $this->eskwela->db($this->session->userdata('school_year'));
        $this->db->where('att_id', $att_id);
        $this->db->delete('attendance_sheet_' . $month);
        return TRUE;
    }
