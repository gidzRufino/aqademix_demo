        ...
        <div class="card card-widget " id="card_<?php echo $pd->task_auto_id ?>">
            <div class="card-header">
                <div class="user-block">
                    <!-- <img class="img-circle" width="50" src="<?php //echo base_url() . 'uploads/' . $pd->avatar; 
                                                                    ?>" alt="User Image"> -->
                    <?php if ($pd->avatar != ''):
                        if (file_exists('uploads/' . $pd->avatar)):
                    ?>
                            <img class="img-circle" style="border:5px solid #fff" src="<?php echo base_url() . 'uploads/' . $pd->avatar  ?>" />
                        <?php else: ?>
                            <img class="img-circle" style="border:5px solid #fff" src="<?php echo base_url() . 'images/avatar/' . ($pd->sex == 'Female' ? 'female.png' : 'male.png')  ?>" />
                        <?php endif; ?>
                    <?php else: ?>
                        <img class="img-circle" style="border:5px solid #fff" src="<?php echo base_url() . 'images/avatar/' . ($pd->sex == 'Female' ? 'female.png' : 'male.png')  ?>" />
                    <?php endif; ?>
            ...
            <div class="card-body">
                ...
                if ($pd->task_attachments != ""):
                    //                    $directory = UPLOADPATH . $this->session->school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $pd->task_author_id . DIRECTORY_SEPARATOR . $pd->task_subject_id . DIRECTORY_SEPARATOR . 'task';
                    $directory = 'uploads/' . $this->session->school_year . DIRECTORY_SEPARATOR . 'faculty' . DIRECTORY_SEPARATOR . $pd->task_author_id . DIRECTORY_SEPARATOR . $pd->task_subject_id . DIRECTORY_SEPARATOR . 'task';
                    ...
                    <div id="qWrapper">
                        <ol id="quizItems">
                            ...
                            for ($q = 0; $q <= count($quizItems) - 1; $q++):
                                $qItems = Modules::run('opl/qm/getQuestionItems', $quizItems[$q], $this->session->school_year);
