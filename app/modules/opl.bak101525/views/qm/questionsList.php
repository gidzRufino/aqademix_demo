		...
		<nav class="navbar navbar-expand-md">
			...
			<div class="navbar-collapse collapse w-100 order-3 dual-collapse2">
				<ul class="navbar-nav ml-auto">
					<li class="nav-item">
						<input type="text" class="form-control" placeholder="Type any key an press enter" onkeyup="if(event.which == 13) new qbClass().searchQB(this, 0);">
					</li>
					</li>
				</ul>
			</div>
	...
	class qbClass {
		...
		loadQuestions = function(page = 0) {
			$.ajax({
				...
				dataType: 'JSON',
				beforeSend: function() {
					$('#questionRows').html('<img style="display:block; margin:0 auto; width:125px;"  src="">');
				},
				success: function(data) {
					$("#questionRows").html(data.questions);
					$(".qPaginationRow").html(data.paginate);
