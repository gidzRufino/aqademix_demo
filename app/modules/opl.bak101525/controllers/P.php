...
<?php
    ...
    {
        $student = $this->p_model->getBasicStudent($st_id, $school_year, $sem);
        return $student;
    }
    ...
    {
        $child_links = $this->session->child_links;
        $child_ids = explode(',', $child_links);
        $student = [];
        foreach ($child_ids as $ids):
            if ($ids != ''):
                $isEnrolled = Modules::run('registrar/isEnrolled', $ids, $this->session->school_year);
                if (!$isEnrolled):
                ...
                $child = Modules::run('registrar/getSingleStudent', $ids, $school_year);
                $subjects = Modules::run('academic/getSpecificSubjectPerlevel', $child->grade_id);
                foreach ($subjects as $s):
                    $singleSub = Modules::run('academic/getSpecificSubjects', $s->sub_id);
                    $first = Modules::run('gradingsystem/getFinalGrade', $child->st_id, $singleSub->subject_id, 1, $school_year);
                    $second = Modules::run('gradingsystem/getFinalGrade', $child->st_id, $singleSub->subject_id, 2, $school_year);
                    $third = Modules::run('gradingsystem/getFinalGrade', $child->st_id, $singleSub->subject_id, 3, $school_year);
                    $fourth = Modules::run('gradingsystem/getFinalGrade', $child->st_id, $singleSub->subject_id, 4, $school_year);
                    $child->subjects[] = array(
                        'name' => $singleSub,
                        'q1' => $first->row()->final_rating,
                        'q2' => $second->row()->final_rating,
                        'q3' => $third->row()->final_rating,
                        'q4' => $fourth->row()->final_rating
                    );
                endforeach;
                $coreVal = Modules::run('reports/getCoreValues');
                foreach ($coreVal as $cv):
                    $bh1 = Modules::run('gradingsystem/getBHRating', $child->st_id, 1, $school_year, $cv->core_id);
                    $bh2 = Modules::run('gradingsystem/getBHRating', $child->st_id, 2, $school_year, $cv->core_id);
                    $bh3 = Modules::run('gradingsystem/getBHRating', $child->st_id, 3, $school_year, $cv->core_id);
                    $bh4 = Modules::run('gradingsystem/getBHRating', $child->st_id, 4, $school_year, $cv->core_id);
                    $child->core_val[] = array(
                        'cv' => $cv->core_values,
                        'bh1' => $bh1->row()->rate,
                        'bh2' => $bh2->row()->rate,
                        'bh3' => $bh3->row()->rate,
    ...
    public function getAcademicRecords()
    {
        ...
        $data['main_header'] = '';
        $data['students'] = $this->academicRecord();
        $data['modules'] = 'opl';
    ...
    function index()
    {
        $this->load->view('parent/pdashboard');
    }
}
