<div class="row">
    <div class="col-lg-12">
        ...
        <?php if(!Modules::run('main/isMobile')): ?>
            <button onclick="document.location='<?php echo base_url().'gradingsystem/downloadGStemplate/' ?>'+$('#section_id').val()+'/'+$('#subject_id').val()+'/'+$('#selectAssessmentCategory1').val()" id="q_template" class="hide btn btn-info btn-sm pull-left" style="margin-right: 10px; margin-top:5px;">Download Quiz Template</button>
        <?php endif; ?>
        </h1>
...
//                    $('#verify_icon').addClass('hide');
     ...
     $(document).ready(function() {
      ...
      function enterRawScore()
      {
            ...
            if(section_id!=""){
                var url = "<?php echo base_url() . 'gradingsystem/enterScore/' ?>" + subject_id + '/' + section_id + '/' + term + '/' + sy;
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function(data) {
