...
foreach ($getStudents->result() as $s) {
    ...
    $bdate = $s->temp_bdate;
    $age = floor((strtotime($firstFridayOfJune) - strtotime($bdate)) / 31556926);
...
//    $age = abs($now - $y);
    ...
    if ($s->f_mobile != "" && $s->m_mobile != "") {
        $separator = ' / ';
    } else {
        $separator = " ";
    }
    if ($s->f_firstname != "") {
        $fatherName = $s->f_lastname . ', ' . $s->f_firstname;
    } else {
        $fatherName = "";
    }
    if ($s->m_firstname != "") {
        $motherName = $s->m_lastname . ', ' . $s->m_firstname;
    } else {
        $motherName = "";
    }
    //
    ...
    $pdf->MultiCell(18, 16, $s->temp_bdate, 1, 'C', 0, 0, '', '', true, 0, false, true, 16, 'M');
    ...
    $pdf->MultiCell(18, 16, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 16, 'M');
    $pdf->MultiCell(20, 16, $s->f_mobile . $separator . $s->m_mobile, 1, 'C', 0, 0, '', '', true, 0, false, true, 16, 'M');
