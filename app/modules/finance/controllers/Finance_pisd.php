 ...
 * @author genesis
 */
class Finance_pisd extends MX_Controller {
    //put your code here
    
    function __construct() {
        parent::__construct();
        $this->load->model('finance_model_pisd');
    ...
    {
        $student = $this->finance_model_pisd->searchFundTransferAccount($value, $year);
        echo '<ul>';
    ...
    function processFundTransfer()
    {
        ...
        if($this->finance_model_pisd->updateFundTransfer($remainingFundUpdate, $transfer_trans_id, $transferSchoolYear)):
            
            $result = $this->finance_model_pisd->saveTransaction($transferTransactionDetails, $transferSchoolYearTo);
            if(!$result):
                ...
                if($this->finance_model_pisd->saveTransferFunds($transferDetails, $transferSchoolYear)):
    ...
    private function addExtraFinanceCharges($item, $sem, $amount, $sy, $user_id)
    {
        ...
        $result = $this->finance_model_pisd->addExtraFinanceCharges($charge, $sy);
    ...
    function setROFinanceAccount($grade_id, $user_id, $school_year)
    {
        ...
        $this->finance_model_pisd->setFinanceAccount($finDetails, $user_id, $plan->fin_plan_id, $school_year, $sem);
    ...
    {
        $result = $this->finance_model_pisd->getStudents($grade_id,$school_year);
        return $result;
    }
    ...
    {
        $data['discounts'] = $this->finance_model_pisd->getIndividualDiscountPerLevel($level_id,$item_id);
        $this->load->view('reports/pisd_discountList', $data);
    }
    
    function getDiscountPerLevelDetails($level_id, $item_id, $date, $school_year = NULL)
    {
        $discounts = $this->finance_model_pisd->getDiscountPerLevelDetails($level_id, $item_id, $date, ($school_year==NULL?$this->session->school_year:$school_year));
    ...
    {
        $discounts = $this->finance_model_pisd->getDiscountPerLevel($level_id, $item_id, $date,($school_year==NULL?$this->session->school_year:$school_year));
    ...
    {
        $charges = $this->finance_model_pisd->financeChargesByPlan($school_year, $sem, $plan, $isGroup);
        return $charges;
    }
    
    public function getTransactionByItemId($st_id, $sem, $school_year, $item_id=NULL, $type=NULL, $order = NULL, $isGroup=NULL)
    {
        $transaction = $this->finance_model_pisd->getTransactionByItemId($st_id, $sem, $school_year, $item_id, $type, $order,$isGroup);
        return $transaction;
    }
    ...
    function printSOA($option, $secIn = NULL)
    {
        ...
        if($option==0):
            $data['student'] = $this->finance_model_pisd->getBasicStudent( base64_decode($secIn), $this->session->school_year);
        else:
            $data['students'] = $this->finance_model_pisd->getStudentPerSection($secIn, $this->session->school_year);
        endif;
