...
foreach($collection as $c):
        foreach($c as $d):
                if($d->t_type != 3 && $d->t_type!=5):
                        $pdf->setX(0);
                        $overAll += $d->t_amount;
                        $z++;
                        if($overAll!=0):
                                $pdf->MultiCell(15, 3, '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                                $pdf->MultiCell(30, 3, date('F d, Y', strtotime($d->t_date)),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                                $pdf->MultiCell(30, 3, $d->ref_number,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                                $pdf->MultiCell(60, 3, $d->lastname.', '.$d->firstname,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                                $pdf->MultiCell(40, 3, $d->item_description,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                                $pdf->MultiCell(30, 3, number_format($d->t_amount, 2, ".", ','),1, 'R', 0, 0, '', '', true, 0, false, true, 10, 'M');
                                $pdf->ln();
                                if($z==34):
                                        $z=0;
                                        $pdf->AddPage();
