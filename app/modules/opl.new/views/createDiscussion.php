            ...
            <div class="card card-outline card-info">
                ...
                <div class="card-body">
                    ...
                    <!-- <div class="form-group col-xs-12 col-lg-6 float-left">
                        ...
                        <select id="gradeLevel" class="form-control" onclick="fetchLesson(this.value)">
                            ...
                            ?>
                                <option value="<?php // echo $gl->subject_id . '-' . $gl->grade_id . '-' . $gl->section_id 
                                                ?>"><?php // echo $gl->subject . ' - ' . $gl->level . ' [' . $gl->section . '] ' 
                                                    ?></option>
                            <?php // endforeach; 
                            ?>
                        </select>
                ...
                <div class="card-footer clearfix">
                    <div class="checkbox float-left">
                        <label>
                            <input id="goPublic" value="0" type="checkbox"> Go Public
                        </label>
                    </div>
...
<script type="text/javascript">
    $(function() {
        // Summernote
        fetchLesson('<?php echo $subjectDetails['subject_id'] . '-' . $gradeDetails['grade_level_id'] . '-' . $gradeDetails['section_id'] ?>')
        $('.textarea').summernote();
        ...
        saveUnitandUpload = function() {
            if ($('#userfile').get(0).files.length === 0) {
                ...
                fd.append("gradeLevel", '<?php echo $subjectDetails['subject_id'] . '-' . $gradeDetails['grade_level_id'] . '-' . $gradeDetails['section_id'] ?>');
        ...
        saveDiscussion = function() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    gradeLevel: '<?php echo $subjectDetails['subject_id'] . '-' . $gradeDetails['grade_level_id'] . '-' . $gradeDetails['section_id'] ?>',
                    startDate: $('#startDate').val() + ' ' + $('#timeStart').val(),
                    subject_id: $('#subject_id').val(),
                    goPublic: $('#goPublic').val(),
