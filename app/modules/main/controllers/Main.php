...
class Main extends MX_Controller
{
    ...
    // function that converts html data from opl tables to .txt file and save them on the server's filing system
    function convert_opl_tasks()
    {
        // load file helper
        $this->load->helper('file');

        // fetch data from db
        $data = $this->main_model->fetch_opl_tasks();

        // fetch data from db and loop depending on model iteration. It should be limitted so not to overwhelm the db.
        $count = 0;
        foreach ($data->result() as $file) {
            $count++;

            // create filename that is distinct and can be traceable
            $filename = $file->task_auto_id . '-' . $file->task_grade_id . '-' . $file->task_section_id . '-' . $file->task_subject_id . '-' . $file->task_lesson_id . '-' . $file->task_author_id . '-' . $file->task_code;

            // convert data to .txt file and store at the designated directory
            $path = FCPATH . 'docs/opl_tasks/' . $filename . '.txt';
            $paper = fopen($path, "w+") or die('File will not open');
            $content = $file->task_details;
            printf($count . ' - ');
            if (fputs($paper, $content)) {
                printf('successfully converted and saved!' . $file->task_auto_id . '<br />');
            } else {
                printf('Not successful. Please check server for error message.' . $file->task_auto_id . '<br />');
            }

            // save the file back to db row
            $this->main_model->update_opl_tasks($file->task_code, $filename . '.txt');
        }
    }

    function convert_opl_task_submitted()
    {
        // load file helper
        $this->load->helper('file');

        // fetch data from db
        $data = $this->main_model->fetch_opl_task_submitted();
        ...
        foreach ($data->result() as $file) {
            ...
            // create filename that is distinct and can be traceable
            $filename = $file->ts_id . '-' . $file->ts_task_id . '-' . $file->ts_submitted_by . '-' . $file->ts_code;

            // convert data to .txt file and store at the designated directory
            $path = FCPATH . 'docs/opl_task_submitted/' . $filename . '.txt';
            $paper = fopen($path, "w+") or die('File will not open');
            $content = $file->ts_details;
            printf($count . ' - ');
            if (fputs($paper, $content)) {
                printf('successfully converted and saved!' . $file->ts_id . '<br />');
            } else {
                printf('Not successful. Please check server for error message.' . $file->ts_id . '<br />');
            }

            // save the file back to db row
            $this->main_model->update_opl_task_submitted($file->ts_id, $filename . '.txt');
        }
    }

    function convert_opl_discussion()
    {
        // load file helper
        $this->load->helper('file');

        // fetch data from db
        $data = $this->main_model->fetch_opl_discussion();
        ...
        foreach ($data->result() as $file) {
            ...
            // create filename that is distinct and can be traceable
            $filename = $file->dis_id . '-' . $file->dis_unit_id . '-' . $file->dis_subject_id . '-' . $file->dis_grade_id . '-' . $file->dis_section_id . '-' . $file->dis_author_id;

            // convert data to .txt file and store at the designated directory
            $path = FCPATH . 'docs/opl_discussion/' . $filename . '.txt';
            $paper = fopen($path, "w+") or die('File will not open');
            $content = $file->dis_details;
            printf($count . ' - ');
            if (fputs($paper, $content)) {
                printf('successfully converted and saved!' . $file->dis_id . '<br />');
            } else {
                printf('Not successful. Please check server for error message.' . $file->dis_id . '<br />');
            }

            // save the file back to db row
            $this->main_model->update_opl_discussion($file->dis_id, $filename . '.txt');
        }
    }

    function convert_opl_comments()
    {
        // load file helper
        $this->load->helper('file');

        // fetch data from db
        $data = $this->main_model->fetch_opl_comments();
        ...
        foreach ($data->result() as $file) {
            ...
            // create filename that is distinct and can be traceable
            $filename = $file->com_id . '-' . $file->com_from . '-' . $file->com_to . '-' . $file->com_sys_code;

            // convert data to .txt file and store at the designated directory
            $path = FCPATH . 'docs/opl_comments/' . $filename . '.txt';
            $paper = fopen($path, "w+") or die('File will not open');
            $content = $file->com_details;
            printf($count . ' - ');
            if (fputs($paper, $content)) {
                printf('successfully converted and saved!' . $file->com_id . '<br />');
            } else {
                printf('Not successful. Please check server for error message.' . $file->com_id . '<br />');
            }

            // save the file back to db row
            $this->main_model->update_opl_comments($file->com_id, $filename . '.txt');
        }
    }

    function convert_opl_posts()
    {
        // load file helper
        $this->load->helper('file');

        // fetch data from db
        $data = $this->main_model->fetch_opl_posts();
        ...
        foreach ($data->result() as $file) {
            ...
            // create filename that is distinct and can be traceable
            $filename = $file->op_id . '-' . $file->op_owner_id . '-' . $file->op_target_type;

            // convert data to .txt file and store at the designated directory
            $path = FCPATH . 'docs/opl_post/' . $filename . '.txt';
            $paper = fopen($path, "w+") or die('File will not open');
            $content = $file->op_post;
            printf($count . ' - ');
            if (fputs($paper, $content)) {
                printf('successfully converted and saved!' . $file->op_id . '<br />');
            } else {
                printf('Not successful. Please check server for error message.' . $file->op_id . '<br />');
            }

            // save the file back to db row
            $this->main_model->update_opl_posts($file->op_id, $filename . '.txt');
        }
    }

    function convert_opl_qm_qq()
    {
        // load file helper
        $this->load->helper('file');

        // fetch data from db
        $data = $this->main_model->fetch_opl_qm_qq();
        ...
        foreach ($data->result() as $file) {
            ...
            // create filename that is distinct and can be traceable
            $filename = $file->qq_id . '-' . $file->qq_qt_id . '-' . $file->created_by;

            // convert data to .txt file and store at the designated directory
            $path = FCPATH . 'docs/opl_qm_qq/' . $filename . '.txt';
            $paper = fopen($path, "w+") or die('File will not open');
            $content = $file->question;
            printf($count . ' - ');
            if (fputs($paper, $content)) {
                printf('successfully converted and saved!' . $file->qq_id . '<br />');
            } else {
                printf('Not successful. Please check server for error message.' . $file->qq_id . '<br />');
            }

            // save the file back to db row
            $this->main_model->update_opl_qm_qq($file->qq_id, $filename . '.txt');
        }
    }

    // function that displays the content of the file from the filing system
    function open_opl_file()
    {
        // filename from the db
        $filename = '5-0-13-246391-0611919-246391-20';

        // path where the actual file was saved
        $path = FCPATH . 'docs/opl_submission/' . $filename . '.txt';
    ...
    function updateEnrollmentDate()
    {
        ...
        ($this->input->post('type') == 1) ? $data['enrollment_end'] = $this->input->post('date') : $data['enrollment_start'] = $this->input->post('date');
        if ($this->main_model->updateEnrollmentDate($school_id, $data) == TRUE):
            echo json_encode(array('status' => 1, 'msg' => "You have successfully update the enrollment dates"));
        else:
            echo json_encode(array('status' => 0, 'msg' => "Something went wrong. Please try again or contact an administrator if the problem persists."));
        endif;
    }
    ...
    function editTimeSettings($inAM, $outAM, $inPM, $outPM, $id, $option)
    {
        ...
        if ($result):
            ...
            $data = array('status' => FALSE, 'msg' => 'An Error Occured During Update');
        endif;
...
?>
    ...
    function saveAdmissionRemarks()
    {
        ...
        $checkRemarks = $this->getAdmissionRemarks($st_id, null);
        ...
        $remarks = $this->getAdmissionRemarks($st_id, null);
        $remarks = $remarks->row();
        echo $remarks->code . ' ' . $remarks->remarks . ' - ' . $remarks->remark_date;;
    }
...
<?php
    ...
    function enrollmentRequirements()
    {
        $settings = Modules::run('main/getSet');
        switch ($settings->short_name):
            case 'csfl':
                $this->load->view($settings->short_name . '_enrollment_requirements');
                break;
            default:
