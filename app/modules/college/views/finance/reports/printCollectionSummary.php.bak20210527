...
foreach($collection->result() as $c):
        if($c->t_type!=5 && $c->t_type!=3):
            $finItem = $c->t_charge_id;
            if (!in_array($finItem, $finItems)) {
                $cash = Modules::run('college/finance/getCashPayments', $from, $to, ($c->fused_category==0?$c->item_id:NULL), $c->fused_category, $c->t_school_year, $c->t_sem);
