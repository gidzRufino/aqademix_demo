<div class="col-lg-12 page-header no-margin">
    <h3 style="margin:10px 0;" class="col-lg-4"><a href="<?php echo base_url('college') ?>"><i class="fa fa-home fa-fw"></i></a> Grading System</h3>
</div>
<?php
$sy = $this->session->userdata('school_year');
//$deadline = Modules::run('college/gradingsystem/getDeadlineDates', $sy);
//$nxtSy = $sy + 1;
if ($this->session->userdata('position') == 'Registrar' || $this->session->userdata('position') == 'Super Administrator'):
    $d['sy'] = $sy;
    echo $this->load->view('registrar_gs', $d);
else:
    ?>
...
<input type="hidden" id="school_year" value="<?php echo $this->session->userdata('school_year'); ?>" />
...
<script type="text/javascript">
    ...
    function saveDdate(dd, field, dd_id, grading, sem) {
        var sy = $('#school_year').val();
        var emp_id = $('#faculty_id').val();
        var url = '<?php echo base_url() . 'college/gradingsystem/saveDeadlineDate' ?>';

        $.ajax({
            type: 'POST',
            url: url,
            data: 'emp_id=' + emp_id + '&dd_id=' + dd_id + '&dd=' + dd + '&field=' + field + '&grading=' + grading + '&sem=' + sem + '&sy=' + sy + '&csrf_test_name=' + $.cookie('csrf_cookie_name'),
    ...
    function postDate(value, field, id, sem) {
        var sy = $('#school_year').val();
        var emp_id = $('#faculty_id').val();
        var url = '<?php echo base_url() . 'college/gradingsystem/postDeadlineDate' ?>';

        $.ajax({
            type: 'POST',
            url: url,
            data: 'emp_id=' + emp_id + '&value=' + value + '&field=' + field + '&id=' + id + '&sem=' + sem + '&sy=' + sy + '&csrf_test_name=' + $.cookie('csrf_cookie_name'),
            success: function (data) {
                location.reload();
            }
        });
    ...
    $(function () {
        $('#faculty_id').val('<?php echo $this->session->userdata('employee_id') ?>');
    ...
    function editTable(id)
    {
        ...
        var isLock = $('#' + id).attr('lock_id');

        if (isLock == 1) {
            alert('Sorry system is already lock, Please contact the registrar for more info!');
        } else {
...
//        alert(id + ' ' + sub_code + ' ' + s_id + ' ' + sem_id);
    ...
    function search(value) {
        ...
        if (sem == 0) {
            ...
            if (option == 'profile_employee') {
                var url = "<?php echo base_url() . 'college/subjectmanagement/teacherSearch/' ?>";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: "value=" + value + '&school_year=' + sy + '&opt=1' + '&csrf_test_name=' + $.cookie('csrf_cookie_name'), // serializes the form's elements.
                    beforeSend: function () {
                        $('#teacherSearch').show();
                ...
                var url = '<?php echo base_url() . 'college/gradingsystem/searchStudent/' ?>' + option + '/' + value + '/' + sem + '/' + sy;
                $.ajax({
                    type: 'GET',
                    url: url,
                    data: 'id=' + value,
                    success: function (data) {
                        $('#gsListTable').html(data);
                    }
                });
            }
        }
    ...
    function gsUnLock(adm_id, sub_id, sy, val, sem, name, td, stid) {
//        alert(adm_id + ' ' + sub_id + ' ' + val + ' ' + td);
        var url = '<?php echo base_url() . 'college/gradingsystem/gsUnlock/' ?>' + adm_id + '/' + sub_id + '/' + name + '/' + val + '/' + sem + '/' + sy;
        var tdHTML = '';
        $.ajax({
            type: 'GET',
            url: url,
...
//                    alert('test');
                    if (val == 1) {
                        $('#newVal-' + stid + '-' + td).val(0);
                        tdHTML = '<i class="fa fa-lock" style="color: red">&nbsp;LOCKED</i>';
                        $('#tdn-' + stid + '-' + td).html(tdHTML);
                    } else {
                        $('#newVal-' + stid + '-' +  td).val(1);
                        tdHTML = '<i class="fa fa-unlock-alt" style="color: green">&nbsp;OPEN</i>';
                        $('#tdn-' + stid + '-' + td).html(tdHTML);
                    }
                }
            }
    ...
    function facultySubjList(id) {
        $('#teacherSearch').hide();
        var sem = $('#sySem').val();
        var sy = $('#syGS').val();
        var url = '<?php echo base_url() . 'college/subjectmanagement/getSchedulePerTeacher/' ?>' + id + '/' + sem + '/' + sy;
    ...
    function subjUnlock(value, subj_id, sec_id, section, sy, tdid) {
//        alert(tdid);
        var url = '<?php echo base_url() . 'college/gradingsystem/subjUnlock/' ?>' + value + '/' + subj_id + '/' + sec_id + '/' + section + '/' + sy;
        var tdHTML = '';
        $.ajax({
            type: 'GET',
            url: url,
