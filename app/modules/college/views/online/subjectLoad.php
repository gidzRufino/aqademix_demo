<div class="col-lg-12 no-padding">
        ...
        <tbody id="subjectLoadBody">
            ...
            if ($subjects):
                foreach ($subjects as $s):
                    $totalUnit += ($s->s_lect_unit + $s->s_lab_unit);
                    ?>
                    <tr class="trSched"
                        ...
                        <td class="hasValue l_units" id="units_<?php echo $s->s_id ?>"></td>
                        <td class="text-center action">
                            <button onclick="loadSchedule('<?php echo $s->sub_code ?>','<?php echo $s->s_id ?>')" title="search for schedule" class="btn btn-success btn-xs"><i class="fa fa-search"></i></button>
    ...
    <input type="hidden" id="tUnit" value="<?php echo $totalUnit ?>" />
</div>
