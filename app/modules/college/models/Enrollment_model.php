...
class Enrollment_model extends CI_Model
{
    ...
    function collegeStudEnrollmentCount($school_year = NULL, $semester = NULL, $st_id = NULL, $status = NULL)
    {
        ...
        $this->db->join('profile_students', 'profile.user_id = profile_students.user_id')
            ...
            ->order_by('profile_students_c_admission.date_admitted', 'DESC');
        if ($st_id != NULL):
    ...
    function studentEnrollmentCount($st_id = NULL, $school_year = NULL, $status = NULL, $sem = NULL)
    {
        ...
        $this->db->join('profile_students', 'profile.user_id = profile_students.user_id')
            ...
            // ->where('profile_students_admission.enrolled_online', 1)
            ->order_by('profile_students_admission.date_admitted', 'DESC');
        ($sem != NULL ? $this->db->where('semester', $sem) : '');
        if ($st_id != NULL):
    ...
    function skipEnrollment($st_id, $sy)
    {
        ...
        if (
            ...
            if (
                $this->db->where('st_id', $st_id)
                ->get('profile_students_c_admission')
                ->num_rows() > 0
            ):
                $this->db->where('st_id', $st_id)
                    ->update('profile_students_c_admission', array('status' => 1));
                if ($this->db->affected_rows() != 0):
                    return TRUE;
                else:
                    return FALSE;
    ...
    function isOldStudent($stid, $school_year, $opt = NULL)
    {
        ...
        $this->load->dbutil();
        while ($num_rows == 0):
            $db = DB_PREFIX . strtolower($this->eskwela->getSet()->short_name) . '_' . $school_year; // local db
    ...
    function isPreviouslyEnrolled($st_id, $school_year, $isCollege, $semester = NULL, $opt = NULL)
    {
        $num_rows = 0;
        $this->load->dbutil();
        $this->db = $this->eskwela->db($school_year);
    ...
    function addPadala($data, $sy = NULL)
    {
    ...
    function updateEnrollmentReq($stid, $link, $req_id, $school_year)
    {
        ...
        if ($q->num_rows() > 0):
            ...
            $data = array(
                'e_checklist_id' => $this->eskwela->code(),
    ...
    function approveBasicEdOnline($adm_id, $school_year = NULL, $status = NULL, $sem = NULL)
    {
        $this->db = $this->eskwela->db($school_year == NULL ? $this->session->school_year : $school_year);
        $this->db->where('admission_id', $adm_id);
        ($sem != NULL ? $this->db->where('semester', $sem) : '');
        if ($this->db->update('profile_students_admission', array('status' => $status, 'st_type' => 1))):
    ...
    function getStudPerStatus($status, $semester, $school_year, $dept = NULL)
    {
        $this->db = $this->eskwela->db($school_year == NULL ? $this->session->school_year : $school_year);
        if ($dept == 5):
            $this->db->where('semester', $semester);
            ...
            $this->db->where('semester', $semester);
            ($status == NULL ? '' : $this->db->where('profile_students_admission.status', $status));
            $this->db->join('grade_level', 'grade_level.grade_id = profile_students_admission.grade_level_id', 'left');
            $this->db->join('profile', 'profile_students_admission.user_id = profile.user_id', 'left');
            $this->db->order_by('date_admitted', 'ASC');
            $q = $this->db->get('profile_students_admission');
        endif;
    ...
    function saveOccupation($occupation, $school_year = NULL)
    {
        ...
        if ($query->num_rows() == 0):
            $this->db->insert('profile_occupation', array('occ_id' => $this->eskwela->code(), 'occupation' => $occupation));
            $runScript = $this->db->last_query();
            //            Modules::run('web_sync/saveRunScript', $runScript, $school_year);
    ...
    function setBasicEdInfo($uid, $profile_id, $gradeLevel, $section_id, $strand, $en_date, $school_year, $semester, $sla, $asla, $status = null)
    {
        $this->db = $this->eskwela->db($school_year);
    ...
    function savePaymentReceipt($st_id, $filename, $school_year, $semester, $fname, $isEnrollment)
    {
        $this->db = $this->eskwela->db($school_year);
        $details = array(
            'opr_id' => $this->eskwela->code(),
            'opr_st_id' => $st_id,
    ...
    function deleteOR($id, $stid, $sy)
    {
        ...
        if ($this->db->affected_rows() > 0):
            $data = array('status' => 3, 'orUpload' => 0);
            $this->db->update('profile_students_admission', $data);
        endif;
    ...
    function updateEnrollmentStatus($st_id, $status, $semester, $school_year, $ifBasicEd, $opt)
    {
        $this->db = $this->eskwela->db($school_year);
        $this->db->where('st_id', $st_id);
        $this->db->where('semester', $semester);
        ...
        if ($ifBasicEd == NULL):
            ...
            $data = ($opt != NULL ? array('status' => $status, 'orUpload' => $opt) : array('status' => $status));
            if ($this->db->update('profile_students_admission', $data)):
                $runScript = $this->db->last_query();
                //                Modules::run('web_sync/saveRunScript', $runScript, $school_year);
                return TRUE;
            else:
    ...
    function getAllEnrolleesForTimeline($school_year, $sem = NULL)
    {
        $allEnrollees = array();
        
        // Get Basic Education Enrollees (all statuses including officially enrolled)
        $this->db = $this->eskwela->db($school_year);
        $this->db->select('profile.*');
        $this->db->select('profile_students_admission.*');
        $this->db->select('profile_students_admission.status as status');
        $this->db->select('profile_students_admission.st_id as st_id');
        $this->db->select('profile_students_admission.date_admitted as date_admitted');
        $this->db->select('profile_students_admission.semester as semester');
        $this->db->select('grade_level.level as level');
        $this->db->join('profile', 'profile_students_admission.user_id = profile.user_id', 'left');
        $this->db->join('grade_level', 'profile_students_admission.grade_level_id = grade_level.grade_id', 'left');
        $this->db->where('enrolled_online', 1);
        if ($sem != NULL):
            $this->db->where('profile_students_admission.semester', $sem);
        endif;
        $this->db->order_by('profile_students_admission.status', 'ASC');
        $this->db->order_by('profile_students_admission.date_admitted', 'DESC');
        $this->db->group_by('profile_students_admission.st_id');
        $basicEdEnrollees = $this->db->get('profile_students_admission')->result();
        
        foreach ($basicEdEnrollees as $enrollee) {
            $allEnrollees[] = $enrollee;
        }
        
        // Get College Enrollees (all statuses including officially enrolled)
        $this->db = $this->eskwela->db($school_year);
        $this->db->select('profile.*');
        $this->db->select('profile_students_c_admission.*');
        $this->db->select('profile_students_c_admission.status as status');
        $this->db->select('profile_students_c_admission.st_id as st_id');
        $this->db->select('profile_students_c_admission.date_admitted as date_admitted');
        $this->db->select('profile_students_c_admission.semester as semester');
        $this->db->select('c_courses.course as course');
        $this->db->join('profile', 'profile_students_c_admission.user_id = profile.user_id', 'left');
        $this->db->join('c_courses', 'profile_students_c_admission.course_id = c_courses.course_id', 'left');
        $this->db->where('enrolled_online', 1);
        if ($sem != NULL):
            ...
            $this->db->where('profile_students_c_admission.semester <', 3);
        endif;
        $this->db->order_by('profile_students_c_admission.status', 'ASC');
        $this->db->order_by('profile_students_c_admission.date_admitted', 'DESC');
        $this->db->group_by('profile_students_c_admission.st_id');
        $collegeEnrollees = $this->db->get('profile_students_c_admission')->result();
        
        foreach ($collegeEnrollees as $enrollee) {
            $allEnrollees[] = $enrollee;
        }
        
        // Sort by status and date
        usort($allEnrollees, function($a, $b) {
            if ($a->status == $b->status) {
                $dateA = isset($a->date_admitted) ? strtotime($a->date_admitted) : 0;
                $dateB = isset($b->date_admitted) ? strtotime($b->date_admitted) : 0;
                return $dateB - $dateA;
            }
    ...
    function getSubject($value, $semester, $school_year, $subject_id)
    {
        $this->db = $this->eskwela->db($school_year);
        $this->db->select('*');
        $this->db->from('c_subjects');
        ...
        $this->db->join('c_schedule', 'c_subjects_per_course.spc_course_id = c_schedule.cs_spc_id', 'left');
        $this->db->where('c_section.sec_sem', $semester);
        $this->db->where('c_subjects.s_id', $subject_id);
    ...
    function getBasicEducationDetails($id, $sy = null, $sem = null)
    {
        $settings = Modules::run('main/getSet');
        $school_year = ($sy != null ? $sy : $settings->school_year);
        ...
        while ($num_rows == 0):
            ...
            if ($eskwelaDB):
                ...
                //                    $this->db->join('profile', 'profile_students.user_id = profile.user_id', 'left');
                //                    $this->db->join('profile_parent', 'profile.user_id = profile_parent.u_id', 'left');
                //                    $this->db->join('profile_students_admission', 'profile_students_admission.st_id = profile_students.st_id', 'left');
                //                    $this->db->join('grade_level', 'profile_students_admission.grade_level_id = grade_level.grade_id', 'left');
                //                    $this->db->join('profile_contact_details', 'profile.contact_id = profile_contact_details.contact_id', 'left');
                //                    ($sem != NULL ? $this->db->where('semester', $sem) : '');
    ...
    function getDetails($id, $sy = null, $sem = null)
    {
        $settings = Modules::run('main/getSet');
        $school_year = ($sy != null ? $sy : $settings->school_year);
        $num_rows = 0;
        ...
        while ($num_rows == 0):
            if ($school_year == 2018):
                ...
                if ($eskwelaDB):
                    ...
                    $this->db->join('c_courses', 'profile_students_c_admission.course_id = c_courses.course_id', 'left');
                    $this->db->where('profile_students_c_admission.semester', $sem);
                    $this->db->order_by('esk_profile_students_c_admission_code', 'DESC');
                    $query = $this->db->get('profile_students');
                    $num_rows = $query->num_rows();
                    if ($num_rows != 0):
                        break;
    ...
    function checkIfIDExist($id, $sy = NULL)
    {
        for ($i = $sy; $i >= 2023; $i--):
    ...
    function passwordReset($update, $uid)
    {
        $this->db->where('u_id', $uid);
        $this->db->update('ua_students', $update);
        return ($this->db->affected_rows() > 0 ? true : false);
    }

    function getContactID($uid)
    {
        $this->db->where('user_id', $uid);
        return $this->db->get('profile')->row();
    }

    function updateContactNumber($number, $user_id, $dept)
    {
        $pYear = date('Y');
        if ($dept == 5): //-------------- if college -----------//
            $contact_id = $this->getContactID($user_id);

            for ($x = 2019; $x <= $pYear; $x++):
                $this->db = $this->eskwela->db($x);
                $this->db->where('contact_id', $contact_id->contact_id);
                $q = $this->db->get('profile_contact_details');

                if ($q->num_rows() > 0):
                    $this->db = $this->eskwela->db($x);
                    $this->db->where('contact_id', $contact_id->contact_id);
                    $this->db->update('profile_contact_details', array('cd_mobile' => $number));
                endif;
            endfor;
            if ($this->db->affected_rows() > 0):
                return true;
            ...
            for ($x = 2019; $x <= $pYear; $x++):
                ...
                if ($q->num_rows() > 0):
                    $this->db = $this->eskwela->db($x);
                    $this->db->where('u_id', $user_id);
                    $this->db->update('profile_parent', array('ice_contact' => $number));
                endif;
            endfor;
            if ($this->db->affected_rows() > 0):
                return true;
    ...
    function checkEcontactNum($num, $stid)
    {
        $this->db->where('profile_students.st_id', $stid);
        $this->db->where('profile_parent.ice_contact', $num);
        $this->db->join('profile_students', 'profile_students.user_id = profile_parent.u_id', 'left');
        return $this->db->get('profile_parent');
    }
}
