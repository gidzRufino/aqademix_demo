...
class Finance_model extends CI_Model
{
    ...
    function getItemByID($item_id)
    {
        ...
        return $this->db->get('c_finance_items')->row();
    }

    function addDiscountCat($item)
    {
        $this->db->where('schlr_type', $item);
        $q = $this->db->get('c_finance_scholar_type');
        if ($q->num_rows() == 0):
            $this->db->insert('c_finance_scholar_type', array('schlr_type' => $item));
    ...
    function getScholarshipReport($school_year, $semester, $isGroup = FALSE)
    {
        ...
        $this->db->join('c_finance_transactions', 'c_finance_discounts.disc_id = c_finance_transactions.disc_id', 'left');
        if ($isGroup):
            $this->db->select('SUM(t_amount) as totalAmount');
            $this->db->group_by('disc_category');
        endif;
        $this->db->where('t_type', 2);
        $this->db->where('disc_school_year', $school_year);
        ($semester != NULL ? $semester != 3 ? $this->db->where('disc_sem <', 3) : $this->db->where('disc_sem', $semester) : '');

        $q = $this->db->get('c_finance_discounts');
        return $q->result();
    }

    function getCollectionSummary($from, $to, $school_year = NULL, $sem = NULL)
    {
        $this->db = $this->eskwela->db($school_year);
        $this->db->select('*');
        ...
        //($report_type!=0?$this->db->where('t_type', $report_type):$this->db->where('t_type !=', 2));

        $this->db->order_by('t_date', 'ASC');
        $this->db->order_by('ref_number', 'ASC');
        // $this->db->group_by('c_finance_items.item_description');
        // $this->db->group_by('c_finance_items.item_id');
        $q = $this->db->get('c_finance_transactions');
        return $q;
    }

    function getStudentListPerSem($sem, $school_year, $isCollege = TRUE)
    {
        if ($isCollege):
            $this->db = $this->eskwela->db($school_year);
            ...
            $this->db->select('firstname,lastname,short_code, year_level, profile_students_c_admission.st_id, profile_students_c_admission.user_id, profile_students_c_admission.course_id, semester, school_year, admission_id');
    ...
    function getCashPayments($from, $to, $item_id, $fused_cat, $school_year = NULL, $sem = NULL)
    {
        $this->db = $this->eskwela->db($school_year == NULL ? $this->session->school_year : $school_year);
        $this->db->select('SUM(t_amount) as amount');
        $this->db->where("t_date between '" . ($from == NULL ? date('Y-m-d') : $from) . "' and '" . ($to == NULL ? date('Y-m-d') : $to)  . "'");
    ...
    function getCollectionItems($from, $to, $school_year = NULL, $sem = NULL)
    {
        $this->db = $this->eskwela->db($school_year == NULL ? $this->session->school_year : $school_year);
        $this->db->select('*');
        $this->db->join('c_finance_items', 'c_finance_transactions.t_charge_id = c_finance_items.item_id', 'left');
    ...
    function getOtherAccountDetails($st_id, $school_year)
    {
        $this->db = $this->eskwela->db($school_year);
    ...
    function getChequePayments($from, $to, $item_id, $fused_cat, $school_year = NULL, $sem = NULL)
    {
        $this->db = $this->eskwela->db($school_year == NULL ? $this->session->school_year : $school_year);
        $this->db->where("t_date between'" . ($from == NULL ? date('Y-m-d') : $from) . "' and'" . ($to == NULL ? date('Y-m-d') : $to)  . "'");
        $this->db->where('t_type', 1);
    ...
    function getCollection($from, $to, $report_type = NULL, $school_year = NULL, $sem = NULL)
    {
        $this->db = $this->eskwela->db($school_year);
        $this->db->select('*');
        // $this->db->select('SUM(t_amount) as amount');
    ...
    function getCollectionReport($from, $to, $report_type = NULL)
    {
    ...
    function deleteExtraCharges($trans_id, $sy = NULL)
    {
    ...
    function getDiscountTransaction($st_id, $sem, $school_year, $t_type, $isGroup)
    {
        $this->db = $this->eskwela->db($school_year);
        $this->db->select('*');
    ...
    function getTransByRefNumber($st_id, $sem, $school_year, $t_type)
    {
        $this->db = $this->eskwela->db($school_year);
    ...
    function getDiscountByStudId($id)
    {

        $this->db->select('*');
        $this->db->join('esk_c_finance_scholar_type', 'esk_c_finance_discounts.disc_category = esk_c_finance_scholar_type.schlr_id');
        $this->db->where('disc_st_id', $id);
    ...
    function addExtraFinanceCharges($charge, $school_year, $item_id, $user_id)
    {
        $this->db = $this->eskwela->db($school_year == NULL ? $this->session->school_year : $school_year);
        if ($user_id != NULL):
            ...
            if ($q->num_rows() > 0):
                $data = array(
                    ...
                    'is_bal_forwarded' => $charge['is_bal_forwarded']
                );

                $this->db->where('extra_item_id', $item_id);
                $this->db->where('extra_st_id', $user_id);
                if ($this->db->update('c_finance_extra', $data)):
                    $runScript = $this->db->last_query();
                    //                    Modules::run('web_sync/saveRunScript', $runScript);
                    return TRUE;
                else:
    ...
    function getPlanByCourse($course_id, $level, $school_year)
    {
        $this->db = ($school_year == NULL ? $this->eskwela->db($this->session->school_year) : $this->eskwela->db($school_year));
    ...
    function getFinItems($school_year = NULL)
    {
        ...
        $this->db->join('c_finance_payment_terms', 'c_finance_payment_terms.pterm_id = c_finance_items.payment_term', 'left');
        $this->db->order_by('order', 'DESC');
    ...
    function addDiscCat($desc)
    {
        $this->db->where('schlr_type', $desc);
        $q = $this->db->get('c_finance_scholar_type');
        ...
        if ($q->num_rows() > 0):
            ...
            $data = array(
                'schlr_id' => $this->eskwela->code(),
                'schlr_type' => $desc
            );

            $this->db->insert('c_finance_scholar_type', $data);

            return TRUE;
        endif;
    }

    function editDiscCat($desc, $id)
    {
        $this->db->where('schlr_id', $id);
        $this->db->update('c_finance_scholar_type', array('schlr_type' => $desc));
    ...
    {
        $this->db->where('schlr_id', $id);
        $this->db->delete('c_finance_scholar_type');
        return ($this->db->affected_rows() > 0 ? TRUE : FALSE);
    }

    function getPaymentTerms()
    {
        return $this->db->get('c_finance_payment_terms');
    }

    function paymentTerm($id, $desc, $opt)
    {
        $data = array('term_desc' => $desc);
        switch ($opt):
            case 1:
                $this->db->insert('c_finance_payment_terms', $data);
                break;
            case 2:
                $this->db->where('pterm_id', $id);
                $this->db->update('c_finance_payment_terms', $data);
                break;
            case 3;
                $this->db->where('pterm_id', $id);
                $this->db->delete('c_finance_payment_terms');
                break;
        endswitch;
