    ...
    public function saveBasicEdAdmission()
{
    ...
    function enrollmentTimeline($sem = NULL, $school_year = NULL)
    {
        if ($this->session->is_logged_in):
            ...
            // Get all enrollees for timeline (including officially enrolled)
            $allEnrollees = array();
            if ($sem != NULL && $school_year != NULL):
                $allEnrollees = $this->enrollment_model->getAllEnrolleesForTimeline($school_year, $sem);
            endif;

            if (file_exists(APPPATH . 'modules/college/views/online/' . strtolower($this->eskwela->getSet()->short_name) . '/enrollmentTimeline.php')):
                $content = 'online/' . strtolower($this->eskwela->getSet()->short_name) . '/enrollmentTimeline';
            else:
                $content = 'online/enrollmentTimeline';
            endif;
    ...
    public function studentTimeline()
    {
        if ($this->session->is_logged_in):
            $data = array(
                'paymentCenter' => $this->getPadala(1, null),
                'login_page' => 'enrollment',
                'modules' => 'college',
                'main_content' => 'online/student_enrollment_timeline',
                'settings' => Modules::run('main/getSet')
            );
            echo Modules::run('templates/online_content', $data);
        else:
    ...
    public function verifyOTP()
    {
        ...
        if ($password_matched) {
            if ($admStat->row()->status == 1 && $usrAcc->num_rows() > 0 && $isEnrollment == 0):
                ...
                echo json_encode(array('status' => TRUE, 'url' => base_url('college/enrollment/studentTimeline'), 'isOld' => $isOld));
    ...
    public function requestOTP($id, $sy = null)
    {
...
<?php
    ...
    function verifyPassword()
    {
        ...
        if ($res):
            ...
            $userData = array(
                ...
                'isStudent' => TRUE,
                'isOld' => $isOld,
                'isEnrolled' => ($details->status == 1 ? 1 : 0),
                'isCollege' => $isCollege,
                'name' => $details->firstname,
                'fullname' => $details->firstname . ' ' . $details->lastname,
                'st_id' => $stid,
            ...
            $this->session->set_userdata($userData);
            Modules::run('main/logActivity', 'LOGIN', ($userData['fullname'] . ' has logged In'), $stid);
    ...
    function checkEcontactNum()
    {
        $num = $this->input->post('num');
        $stid = base64_decode($this->input->post('stid'));
        $q = $this->enrollment_model->checkEcontactNum($num, $stid);
        if ($q->num_rows() > 0):
            $r = $this->passwordReset($num, $stid);
            if ($r):
                echo json_encode(array('status' => true, 'msg' => 'A new generated password was sent to ' . $num . '. Please change the password once logged In.'));
            else:
                echo json_encode(array('status' => false, 'msg' => 'An error occured in generating password'));
            endif;
        else:
            echo json_encode(array('status' => false, 'msg' => 'The contact number entered is not registered on the system. Please contact the School Registrar'));;
        endif;
    }

    function passwordReset($num, $stid)
    {
        $random = Modules::run('registrar/rand_string', 8);
        ...
        $q = $this->enrollment_model->passwordReset($update, $stid);

        if ($q):
            $msg = 'Reset Student\'s password with ID number: ' . $stid;
            $title = 'Reset Password';
            Modules::run('registrar/logActivity', $title, $msg, $stid);
            $sms = 'Your Student\'s password has been reset. Use this Generated password to log in. Password: ' . $random;
            Modules::run('registrar/sendMsg', $num, $sms);
            return true;
        else:
            return false;
        endif;
