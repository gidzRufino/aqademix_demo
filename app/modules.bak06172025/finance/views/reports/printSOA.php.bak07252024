...
$pdf->MultiCell(100, 0,'Name: '. strtoupper($student->firstname.' '.($student->middlename!=""?substr($student->middlename,0,1).'. ':""). $student->lastname),0, 'L', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
foreach ($charges as $c):
    ...
    $pdf->SetX(10);
    $pdf->SetFont('helvetica', 'N', 12);
    $pdf->MultiCell(65, 0, strtoupper($c->item_description),0, 'L', 0, 0, '', '', true, 0, false, true, 0, 'M');
    $pdf->MultiCell(85, 0,'----------------------------------------------------------',0, 'L', 0, 0, '', '', true, 0, false, true, 0, 'M');
    $pdf->MultiCell(40, 0,(number_format($c->amount,2,'.',',')),0, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
