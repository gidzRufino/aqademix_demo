       ...
       //$charges = Modules::run('finance/financeChargesByPlan',"", $school_year, 0, $plan->fin_plan_id );
        ...
        if ($charges != 0) {
            foreach($charges as $c): 
                $discount = Modules::run('finance/billing/getTransactionByCategory', $student->st_id, 0,$student->school_year,$c->category_id,2, $c->item_id);
                ...
                $tfdiscount = Modules::run('finance/billing/getTransactionByCategory', $student->st_id, 0, $student->school_year, $c->category_id, 2, $c->item_id);
                ...
                foreach ($extraC->result() as $exc):
                    ...
                    if(!$incharges):
                        ...
                        <tr id="trp_<?php echo $exc->item_id ?>" tr_val="" totalvalue="">
                         ...
                         <td class="text-right" id="charges_<?php echo $exc->item_id ?>" tdCharges=<?php echo $exc->amount ?> ><?php echo number_format($exc->amount,2,".",",")  ?></td>
                         <td class="text-right" id="payments_<?php echo $exc->item_id ?>" tdPayment=<?php echo $totalMxpayments ?> ><?php echo number_format($totalMxpayments,2,".",",") ?></td>
            ...
            echo '<tr><td colspan="8" style="text-align: center; font-weight: bold" class="alert alert-danger">NO FEES SET</td></tr>';
        }                         
        ?>
    </tbody>
</table>
