<table style="font-size:12px;" class="tablesorter table table-striped">
    ...
    <?php
    $remarks = Modules::run('main/getAdmissionRemarks', $s->uid, NULL);
    ...
    if ($this->session->userdata('is_admin')):
        ?>
                ...
                <?php if ($s->rfid == "" || $s->rfid == "NULL"): ?>
                    ...
                    <a href="#rollOver" data-toggle="modal" onclick="$('#ro_st_id').val('<?php echo $s->uid ?>'), $('#curr_grade_id').val('<?php echo $s->grade_id ?>'), $('#ro_grade_id').val('<?php echo $nxt_lvl->grade_id ?>'), $('#curr_lDesc').html('<?php echo $s->level ?>'), $('#stName').html('<?php echo $name ?>'), $('#new_lDesc').html('<?php echo $nxt_lvl->level ?>')"  class="text-success" >ROLL OVER</a> |
