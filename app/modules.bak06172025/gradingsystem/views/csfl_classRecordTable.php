...
<?php if ($this->session->position_id == 97): 
    ?>
    <div class="pull-right" style="background-color: lightblue; padding: 5px">
            ...
            <tr class="main"> 
                ...
                foreach ($category as $cat => $k) {
                    ...
                    <td  class="text-center pointer"  data-target="#assess_details" data-toggle="modal" onclick="getIndividualAssessment('<?php echo $student->st_id ?>',<?php echo $subject_id ?>,<?php echo $k->code ?>, <?php echo $term ?>)" class="values" >
                        <?php
                        $record = Modules::run('gradingsystem/getTotalScoreByStudent', $student->st_id, $k->code, $term, $subject_id, ($this->session->position_id == 97 ? $fac_assign->employee_id : $this->session->userdata('employee_id')), $student->section_id);

                        $numberOfAssessment = Modules::run('gradingsystem/getEachScoreByStudent', $student->st_id, $k->code, $term, $subject_id, 1, $student->section_id, ($this->session->position_id == 97 ? $fac_assign->employee_id : $this->session->userdata('employee_id')), $strand_id);
