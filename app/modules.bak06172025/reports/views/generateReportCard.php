...
// if($gs_settings->gs_used==1):
        ...
        <h5>Learner's Observed Values
            <select class="pull-right">
                ...
                <?php foreach ($behavior as $cv): ?>
                    <option value="<?php echo $cv->core_id ?>" onclick="getBehaviorStatement(this.value)"><?php echo $cv->core_values ?></option>
                <?php endforeach; ?>
            </select>
        </h5>
        <hr />
        <table class="table table-striped" id="cv">
    ...
    <?php else: ?>
        ...
        <table class="table table-striped">
            ...
            $behaviorRating = Modules::run('gradingsystem/getFinalBHRate', $student->uid, $term, $sy, $beh->core_id);
                ?>
                <tr>
                    <td><?php echo $beh->core_values ?></td>
                    <td>
                        <select onclick="submitRating('<?php echo $student->uid ?>', this.value, <?php echo $term ?>,<?php echo $sy ?>, <?php echo $beh->core_id ?>)" tabindex="-1" style="width:200px" class="span2">
                            <?php
