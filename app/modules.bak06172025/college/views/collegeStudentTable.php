...
<!--                <input style="width:250px;" onkeypress="if (event.keyCode == 13) {
                            search(this.value)
                        }" class="form-control" id="verify" placeholder="Search" type="text">-->
...
//                        echo $status;
                    ?>
                    <span class="pointer" onclick="$('#adminRemarks').modal('show'), $('#st_id').val('<?php echo $s->st_id ?>'), $('#us_id').val('<?php echo $s->u_id ?>'), $('#input_sem').val('<?php echo $s->semester ?>'), $('#input_sy').val('<?php echo $s->school_year ?>')">
                        <?php
                        if ($s->status == 7 || $s->status == 8 || $s->status == 9):
                            echo $status . '<br>' . $remarks->row()->remarks . ' - ' . $remarks->row()->remark_date;
                        else:
                            echo $status;
                        endif;
                        ?>
                        <i class="fa <?php echo $fa ?> pull-right" style="color: <?php echo $color ?>"></i>
                    </span>
                </td>
                <?php
                if ($this->session->userdata('is_admin')):
