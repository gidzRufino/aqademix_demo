...
$pdf->MultiCell(101, 5, '', 'TRB', 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
$pdf->Ln();
$pdf->SetX(10);
$pdf->MultiCell(120, 5, '', 'TBL', 'L', 0, 0, '', '', true, 0, false, true, 5, 'M');
$pdf->MultiCell(261, 5, '', 'TRB', 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
$pdf->Ln();
// set cell padding
...
for ($sub = 1; $sub <= 6; $sub++):
    $pdf->MultiCell(31.5, 11, 'Subject Code', 1, 'C', 0, 0, '', '', true, 0, true, true, 11, 'M');
...
foreach ($students->result() as $st):
    ...
    if ($st->status == 1):
        ...
        $pdf->MultiCell(85, 5, ($y == 1 ? strtoupper($st->lastname) . ', ' . ucwords(strtolower($st->firstname)) . ' ' . ucwords(strtolower($middlename)) : ""), 1, 'L', 0, 0, '', '', true, 0, false, true, 5, 'M');
        ...
        foreach ($loadedSubject as $subject):
            ...
            $grade = Modules::run('college/gradingsystem/getFinalGrade', $st->st_id, NULL, $subject->s_id, segment_4, segment_5, 4);
            switch (TRUE):
            ...
            $pdf->MultiCell(31.5, 5, $subject->sub_code, 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
            $pdf->MultiCell(14.5, 5, $fgrade, 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');

            if ($subcount == 7 || ($subcount == 13 && $count > 13)):
                $pdf->Ln();
                $pdf->setX(115);
                $x++;
            endif;
        ...
        for ($sub = 1; $sub <= $scount; $sub++):
            $pdf->MultiCell(31.5, 5, '', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
