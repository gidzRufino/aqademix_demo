...
//echo $students->course_id;
$acadSubjects = Modules::run('college/subjectmanagement/getLoadedSubject', $students->admission_id, $students->semester, $school_year);
//echo $students->admission_id;
?>
<h3 class="pull-left col-lg-6">GPA : <span class="text-danger" id="gpa"></span> | Total Units : <span class="text-danger" id="totalGPAUnits"></span></h3>
<?php if ($term->result()) : ?> 
    <?php if ($this->session->position_id == 1 || $this->session->position == 'Registrar' || $this->session->position == 'Registrar\'s Staff' || $this->session->position == 'Student Assistant (Registrar)'): ?>
        <div class="col-lg-6 pull-right">
            <button title="Print Card" onclick="printClassCard('<?php echo base64_encode($details->st_id) ?>', '<?php echo $sem ?>', '<?php echo $school_year ?>')" class="btn btn-success pull-right">PRINT CARD</button><br /> <br />
        </div>
    <?php
    endif;
endif;
...
<div class="col-lg-12 page-header">
    <table class="table table-hover">
        ...
        foreach ($acadSubjects as $as):
            $subject = Modules::run('college/subjectmanagement/getSubjectPerId', $as->s_id, $as->sec_school_year);
            //print_r($subject);
        //echo $as->sec_school_year;
            switch ($as->sec_sem):
                case 1:
                    $semester = 'First';
                    break;
            ...
            <tr>        
                ...
                foreach ($term->result() as $t):
                    $grade = Modules::run('college/gradingsystem/getFinalGrade', $students->st_id, NULL, $as->s_id, $as->sec_sem, $as->sec_school_year, $t->gst_id);

                    if ($this->session->is_admin):
                        ?>
                    <input type="hidden" id="raw_<?php echo $as->s_id . '_' . $grade->gsa_id . '_' . $t->gst_id ?>" value="<?php echo $grade->gsa_grade; ?>" />
                    <?php
                endif;

                if ($t->gst_id == 4):
                    $totalUnits += $subject->s_lect_unit;
                    ?>
                    <input type="hidden" class="finalEq" value="<?php echo ($grade->gsa_grade * $subject->s_lect_unit) ?>" />
                    <?php
                endif;
                ?>
                <td
                    ...
                    <strong><?php echo number_format($grade->gsa_grade, 1, '.', ','); ?></strong>   
                </td>
    ...
    <?php if ($this->session->is_superAdmin || $this->session->position == 'Registrar'): ?>
                <td class="text-right">
    ...
    <div id="confirmPrintCard"  style="margin: 50px auto;"  class="modal fade col-lg-3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="panel panel-red" style='width:100%;'>
            ...
            <div class="panel-body">
                ...
                <div class="form-group pull-left">
                    <select tabindex="-1" id="inputTerm" style="width:200px; font-size: 15px;" class=" ">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var sumEq = 0;
        var sumUnits = $('#totalUnits').val();
        var GPA = 0;
        $('.finalEq').each(function () {
            sumEq += parseFloat($(this).val());
        });

        GPA = parseFloat(sumEq) / parseFloat(sumUnits);

        $('#gpa').html(GPA.toFixed(2));
        $('#totalGPAUnits').html($('#totalUnits').val())
    ...
    function printClassCard(st_id, sem, sy)
    {
        ...
        $('#printCardBtn').click(function () {
            ...
            if ($('#claimedCheck').is(':checked')) {
                var url = "<?php echo base_url('college/subjectmanagement/printClassCard') ?>/" + st_id + '/' + semester + '/' + school_year + '/' + term + '/' + 1;
            } else {
                var url = "<?php echo base_url('college/subjectmanagement/printClassCard') ?>/" + st_id + '/' + semester + '/' + school_year + '/' + term;
            }
