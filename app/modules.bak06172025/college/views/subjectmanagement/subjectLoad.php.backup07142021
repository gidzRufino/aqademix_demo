<div class="page-header" style="color:black; margin:20px 0;width: 100%;">
    ...
    <input style="display: none;float: right;" name="enDate" type="text" data-date-format="yyyy-mm-dd" id="enDate" value="<?php echo $students->date_admitted; ?>" placeholder="Date Enrolled" 
           ...
           onkeypress="if (event.keyCode == 13) {
                       editEnBdate(this.value, '<?php echo $student->u_id ?>')
                   }"   
    ...
    <table class="table table-stripped table-hover">
        ...
        <tbody id="subjectTaken">
            ...
            foreach ($loadedSubject as $ls):
                if ($ls->cl_sub_id == $ls->sec_sub_id):
                    $aprv += $ls->is_final;
                    $isOnline = ($student->enrolled_online ? TRUE : FALSE);

                    if ($this->session->userdata('position_id') == 1 || $this->session->position == 'Registrar' || $this->session->position == 'Dean' || $this->session->position == 'Dean Secretary' || $this->session->position == 'Registrar\'s Staff' || $this->session->position == 'OICT Department Head' || $this->session->position == 'IT Coordinator' || $this->session->position == 'Quality Assurance Director' || $this->session->position == 'IBDE Department Head'):
                        ?>
                        <tr class="pointer" id="trtaken_<?php echo $ls->s_id ?>" onclick="removeSubject('<?php echo $ls->s_id ?>', '<?php echo $ls->pre_req ?>')">
                            ...
                            <td class="text-center">
                                <?php
                                $scheds = Modules::run('college/schedule/getSchedulePerSection', $ls->cl_section, $student->semester, $student->school_year);
        ...
        <?php if ($this->session->userdata('position_id') == 1 || $this->session->position == 'Registrar' || $this->session->position == 'OICT Department Head' || $this->session->position == 'IT Coordinator' || $this->session->position == 'Quality Assurance Director'):
            ?>
            <tfoot>
                <tr>
                    <?php if ($count != $aprv): ?>
        ...
        <?php if ($this->session->position == 'Dean' || $this->session->position == 'Dean Secretary' || $this->session->position == 'IBDE Department Head'): ?>
            <tfoot>
                <tr>
                    <?php if ($count != $aprv): ?>
                        <td colspan="4">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#btnApprove').click(function () {
            var url = "<?php echo base_url() . 'college/subjectmanagement/approveLoad/' ?>" + '<?php echo $student->admission_id ?>/<?php echo $students->u_id ?>'; // the script where you handle the form input.
                    ...
                    $('#onlineBtnApprove').click(function () {
                        ...
                        var url = "<?php echo base_url() . 'college/subjectmanagement/approveLoadOnline/' ?>" + '<?php echo $student->admission_id ?>/<?php echo $students->u_id ?>/<?php echo $student->school_year ?>'; // the script where you handle the form input.
