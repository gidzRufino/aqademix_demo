...
switch ($student->status):
    case 0:
        $student_stat = 'Enrollment Application Approval';
        $color = 'red';
        break;
    case 1:
        $student_stat = 'Officially Enrolled';
        $color = 'green';
        break;
    case 3:
        $student_stat = 'For Payment';
        $color = 'red';
        break;
    case 4:
        $student_stat = 'For Payment Evaluation';
        $color = 'red';
        break;
    case 5:
        $student_stat = 'For Payment Confirmation';
        $color = 'red';
        break;
    case 6:
        $student_stat = 'For Evaluation';
        $color = 'red';
        break;
endswitch;
?>
<div class="well col-lg-12" id="profBody">
    ...
    <h5>Enrollment Status: <b style="color: <?php echo $color ?>"><?php echo $student_stat ?></b></h5>
    ...
    <input type="hidden" id="enrollOpt" />

</div>
<div class="modal fade" id="skipEnrollment">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-body text-center">
                <h1><i class="fa fa-exclamation text-danger"></i></h1>
                <h4>You are about to skip <span id="stName"></span> from the enrollment procedure.</h4>
                <h5>Do you really want to enroll this student officially?</h5>
                <small>Note: This process cannot be undone!</small>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger" id="skipBtn" onclick="skipEnrollment(this), $('#enrollOpt').val(0)">Proceed</button>
...
<div class="col-lg-12 no-padding">
    ...
    <?php
    if ($student->status == 6):
        if ($this->session->position_id == 123 || $this->session->position_id == 49):
            ?>
            <div class="col-lg-12 col-xs-12">
                <div class="col-lg-4 col-md-4 col-xs-12"></div>
                <div class="col-lg-4 col-md-4 col-xs-12">
                    <button id="enrollOfficially" onclick="officiallyEnStudent('<?php echo base64_encode($student->st_id) ?>')" class="btn btn-warning pull-right">Enroll Student Officially?</button>
                </div>
                <div class="col-lg-4 col-md-4 col-xs-12"></div>
            </div>
            <?php
                        ...
                        <tr>
                            <td colspan="5">
                                <button class="pull-left btn btn-small btn-primary"
                                    ...
                                    <?php else: ?>
                                        <button onclick="approveOnlineApp()" class="btn btn-warning pull-right">Approve Online Application ?</button>
                                        <!--<button id="onlineBtnApprove" class="btn btn-warning pull-right">Approve Online Application ?</button>-->
                                    <?php endif; ?>
                                <?php else: ?>
        ...
        <div class="col-lg-12" style="margin: 10px;">
            <div class="panel panel-primary ">
                ...
                <div class="panel-body">
                    <?php
                    $directory = 'UPLOADSa' . DIRECTORY_SEPARATOR . $student->school_year . DIRECTORY_SEPARATOR . 'students' . DIRECTORY_SEPARATOR . $student->st_id . DIRECTORY_SEPARATOR . 'files';
                    // echo $directory;
                    ...
                    foreach ($files as $file):
                        ...
                        switch ($ext):
                            ...
                            case 'jpeg':
                            case 'png':
                                $link = base_url($directory . '/' . $file);
                                break;
                            case 'pdf':
                                $link = base_url('images/pdf-img.png');
                                break;
                            case 'doc':
                            case 'docx':
                                $link = base_url('images/word-img.png');
                                break;
                            case 'xlsx':
                            case 'xls':
                                $link = base_url('images/excel-img.png');
                                break;
                        endswitch;
                        ?>
                        <div class="col-lg-4">
                            <button onclick="deleteEnrollmentFile('<?php echo $directory . '/' . $file ?>')" class="btn btn-xs btn-danger pull-right">x</button>
                            <a href="<?php echo base_url() . 'opl/downloads/' . base64_encode($directory . '/' . $file) ?>">
                                <img class="img-responsive" style="width: 100px; height: 100px" src="<?php echo $link ?>" alt="<?php echo $file ?>">
                            </a><br>
                            <h6><?php echo $file ?></h6>
    ...
    if ($this->session->userdata('position_id') == 1 ||
            ...
            $this->session->position == "Accountant"):
        $totalCharges = 0;
        $totalExamFee = 0;
        $totalLab = 0;
        $fusedCharges = 0;
        if ($student->status): //SUBJECTS ALREADY APPROVED
            ...
            if ($student->is_old):
                switch ($student->semester):
                    case 0:
                        $p_sy = $student->school_year - 1;
                        $p_sem = 3;
                        break;
                    case 3:
                        $p_sy = $student->school_year;
                        $p_sem = 0;
                        break;
                endswitch;
                $result = Modules::run('college/enrollment/isPreviouslyEnrolled', $student->st_id, $p_sy, 0, 0);
                echo $result;
                $hasBalance = json_decode(Modules::run('finance/getPrevBal', $student->st_id, $result->semester, $result->school_year, $student->semester, $student->school_year));
            else:
                $hasBalance = json_decode(json_encode(array('status' => FALSE)));
            endif;
//            $previousRecord = json_decode(Modules::run('finance/getRunningBalance', base64_encode($student->st_id), ($student->semester == 3 ? $student->school_year : ($student->school_year - 1)), ($student->semester == 3 ? 0 : $student->semester), 1));
                    ...
                    <div class="panel-body">
                        <table class="table table-striped table-responsive">
                            ...
                            if ($hasBalance->rawBalance > 0):
                                ...
                                ?>
                                <tr class="danger">
                                    <td style="font-size: 20px;">PREVIOUS BALANCE <h5><?php echo $hasBalance->msg; ?></td>
                            ...
                            $payment = Modules::run('finance/getPayments', $student->school_year, $student->semester, base64_encode($student->st_id), 1);
                            if ($paymentReceipt->row() || $payment > 0):
                                ?>
                                <tr>
                                ...
                                <tr>
                                    <td colspan="2" class="text-center">
                                        ...
                                        <?php
                                        $directory = 'UPLOADSa' . DIRECTORY_SEPARATOR . $student->school_year . DIRECTORY_SEPARATOR . 'students' . DIRECTORY_SEPARATOR . $student->st_id . DIRECTORY_SEPARATOR . 'online_payments';
                                        // echo $directory;
                                        $scanFiles = scandir($directory);
                                        $files = array_diff($scanFiles, array('..', '.'));
                                        foreach ($files as $file):
                                            ?>
                                            <a href="<?php echo base_url() . 'opl/downloads/' . base64_encode($directory . '/' . $file) ?>"><img class="img-responsive myImg" style="width: 200px;" src="<?php echo base_url($directory . '/' . $file) ?>" alt="<?php echo $file ?>"></a>
                                ...
                                <?php
                                if ($this->session->userdata('position_id') == 1 || $this->session->position == "Cashier" ||
                                        $this->session->position == 'School Administrator' ||
                                        $this->session->position == "Accounting Staff"):
                                    if ($payment > 0):
                                        ?>
                                        ...
                                        <tr>
                                            <td colspan="2" class="text-center">
                                                <button class="btn btn-xs btn-block btn-dark" disabled>Payment not yet credited to enrollees account!</button>
                                            </td>
                                        </tr>
                                    <?php
                                    endif;
                                ...
                                <div class="alert alert-info">
                                    <h4 class="text-center"><i class="fa fa-info-circle fa-fw"></i> Student hasn't made any payment yet.</h4>
                                </div>
                            <?php
                            endif;
                            ?>    
...
<script type="text/javascript">
    ...
    $(document)[_0x3822('0x3')](function () {
        ...
        $(_0x3822('0x38'))[_0x3822('0x18')](function () {
            var _0x57cc75 = confirm(_0x3822('0x37'));
            if (_0x57cc75 == !![]) {
                var _0x4b0daf = _0x5903be + _0x3822('0x30');
                $[_0x3822('0x2d')]({'type': _0x3822('0x14'), 'url': _0x4b0daf, 'dataType': 'json', 'data': {'st_id': $('#st_id')[_0x3822('0x35')](), 'user_id': $('#user_id')['val'](), 'semester': $(_0x3822('0x1f'))['val'](), 'school_year': $(_0x3822('0x20'))['val'](), 'csrf_test_name': $[_0x3822('0x36')](_0x3822('0x15'))}, 'beforeSend': function () {
                        $(_0x3822('0x38'))[_0x3822('0x12')](_0x3822('0x25'));
                        $(_0x3822('0x38'))[_0x3822('0x29')]('disabled');
                    }, 'success': function (_0x1f03cc) {
                        $('#confirmPayment')[_0x3822('0x12')]('PAYMENT\x20CONFIRMED');
                        $('#confirmPayment')[_0x3822('0x2c')](_0x3822('0x1c'));
                        $(_0x3822('0x38'))[_0x3822('0x29')](_0x3822('0x24'));
                        alert(_0x1f03cc[_0x3822('0xc')]);
                        location[_0x3822('0x31')]();
                    }});
                return![];
            }
        });
        $(_0x3822('0x5'))[_0x3822('0x18')](function () {
            var _0x4a9c9e = _0x5903be + _0x3822('0xa') + $(_0x3822('0x9'))[_0x3822('0x35')]() + '/' + $(_0x3822('0x33'))['val']();
            $[_0x3822('0x2d')]({'type': _0x3822('0x34'), 'url': _0x4a9c9e, 'dataType': 'json', 'data': _0x3822('0x28') + $[_0x3822('0x36')]('csrf_cookie_name'), 'success': function (_0x2b2e9b) {
                    alert(_0x2b2e9b[_0x3822('0xc')]);
                    location[_0x3822('0x31')]();
                }});
            return![];
        });
        $(_0x3822('0x1a'))[_0x3822('0x18')](function () {
            var _0x1becd0 = confirm(_0x3822('0xf'));
            if (_0x1becd0 == !![]) {
                var _0x361c0d = _0x5903be + _0x3822('0x39') + $(_0x3822('0x9'))[_0x3822('0x35')]() + '/' + $(_0x3822('0x33'))['val']() + '/' + $(_0x3822('0x20'))[_0x3822('0x35')]() + '/' + $(_0x3822('0x1f'))[_0x3822('0x35')]();
                $['ajax']({'type': _0x3822('0x34'), 'url': _0x361c0d, 'dataType': _0x3822('0xb'), 'data': _0x3822('0x28') + $[_0x3822('0x36')](_0x3822('0x15')), 'beforeSend': function () {
                        $(_0x3822('0x19'))[_0x3822('0xd')](_0x3822('0x2b'));
                    }, 'success': function (_0x25aa64) {
                        $(_0x3822('0x19'))[_0x3822('0xd')](_0x3822('0x22'));
                        alert(_0x25aa64[_0x3822('0xc')]);
                        location[_0x3822('0x31')]();
                    }});
                return![];
            }
        });
        var _0x5903be = $('#base_url')[_0x3822('0x35')]();
        sendAdmissionRemark = function () {
            var _0x39e7ec = $(_0x3822('0x2'))[_0x3822('0x35')]();
            var _0x1a708c = $(_0x3822('0x13'))[_0x3822('0x35')]();
            var _0x4bc94a = $('#school_year')[_0x3822('0x35')]();
            var _0x26b86a = $(_0x3822('0x1f'))[_0x3822('0x35')]();
            var _0x21fc82 = _0x5903be + _0x3822('0x2e');
            $['ajax']({'type': _0x3822('0x14'), 'url': _0x21fc82, 'data': {'csrf_test_name': $[_0x3822('0x36')]('csrf_cookie_name'), 'st_id': _0x39e7ec, 'semester': _0x26b86a, 'school_year': _0x4bc94a, 'remarks': _0x1a708c}, 'success': function (_0x52fa83) {
                    alert(_0x52fa83);
                    location[_0x3822('0x31')]();
                }});
            return![];
        ...
        sendSMS = function () {
            var _0x4e77bf = $(_0x3822('0x17'))[_0x3822('0x35')]();
            var _0x177768 = _0x5903be + _0x3822('0x23') + _0x4e77bf;
            var _0x22aaab = $(_0x3822('0x10'))[_0x3822('0x35')]();
            $[_0x3822('0x2d')]({'type': _0x3822('0x14'), 'url': _0x177768, 'data': _0x3822('0x26') + _0x22aaab + '&csrf_test_name=' + $[_0x3822('0x36')](_0x3822('0x15')), 'beforeSend': function () {
                    $(_0x3822('0x19'))[_0x3822('0xd')]('show');
                }, 'success': function (_0x4a2d00) {
                    $(_0x3822('0x19'))[_0x3822('0xd')](_0x3822('0x22'));
                    alert(_0x4a2d00);
                    location[_0x3822('0x31')]();
                }});
        ...
        sendRemarks = function () {
            var _0x5ac03f = $('#st_id')[_0x3822('0x35')]();
            var _0xcd45af = $(_0x3822('0x1b'))[_0x3822('0x35')]();
            var _0x12620b = $(_0x3822('0x20'))[_0x3822('0x35')]();
            var _0x3e5a8b = $(_0x3822('0x1f'))[_0x3822('0x35')]();
            var _0x55b42e = $(_0x3822('0x9'))[_0x3822('0x35')]();
            var _0x2d0679 = _0x5903be + _0x3822('0x32');
            $[_0x3822('0x2d')]({'type': _0x3822('0x14'), 'url': _0x2d0679, 'data': {'csrf_test_name': $[_0x3822('0x36')]('csrf_cookie_name'), 'st_id': _0x5ac03f, 'semester': _0x3e5a8b, 'school_year': _0x12620b, 'remarks': _0xcd45af, 'adm_id': _0x55b42e}, 'success': function (_0x37ef71) {
                    alert(_0x37ef71);
                    location[_0x3822('0x31')]();
                }});
            return![];
        };
        removeSubject = function (_0x43edc1) {
            var _0x119664 = $(_0x3822('0x33'))['val']();
            var _0x51d73e = parseInt($(_0x3822('0x3a'))[_0x3822('0x12')]());
            _0x51d73e -= parseInt($(_0x3822('0x2f') + _0x43edc1)['html']());
            $(_0x3822('0x3a'))['html'](_0x51d73e);
            var _0x171363 = _0x5903be + 'college/enrollment/removeSubject/';
            $['ajax']({'type': _0x3822('0x14'), 'url': _0x171363, 'dataType': _0x3822('0xb'), 'data': {'csrf_test_name': $[_0x3822('0x36')](_0x3822('0x15')), 'user_id': _0x119664, 'subject_id': _0x43edc1, 'school_year': $(_0x3822('0x20'))[_0x3822('0x35')]()}, 'success': function (_0x211623) {
                    if (_0x211623['status'] == !![]) {
                        $(_0x3822('0x1d') + _0x43edc1)['remove']();
                    }
                    alert(_0x211623['msg']);
                }});
            return![];
    ...
    function removeEnrollmentDetails(_0x1865e6, _0x4d7984) {
        var _0x3c61c0 = $(_0x3822('0x2a'))['val']();
        var _0x5182e9 = _0x3c61c0 + _0x3822('0x1e');
        var _0x38d132 = confirm(_0x3822('0x21'));
        if (_0x38d132 == !![]) {
            $[_0x3822('0x2d')]({'type': _0x3822('0x14'), 'url': _0x5182e9, 'data': {'adm_id': _0x1865e6, 'school_year': _0x4d7984, 'csrf_test_name': $[_0x3822('0x36')](_0x3822('0x15'))}, 'beforeSend': function () {
                    $('#loadMsg')[_0x3822('0x2b')]();
                    $('#loadMsg\x20td')[_0x3822('0x12')](_0x3822('0xe'));
                }, 'success': function (_0x3b2751) {
                    alert(_0x3b2751);
                    document[_0x3822('0x4')] = _0x3c61c0 + _0x3822('0x16');
                }});
            return![];
        }
    }
    ...
    function readySkip(btn) {
        var st_id = $(btn).attr('st-id'),
                st_name = $(btn).attr('st-name'),
                modal = $("#skipEnrollment");
        console.info(st_name);
        modal.find("#stName").html(st_name);
        modal.find("#skipBtn").attr('st-id', st_id);
        modal.modal();
    }

    function officiallyEnStudent(stid) {
        var u_id = $('#user_id').val();
        var sy = $('#school_year').val();
        var sem = $('#semester').val();
        var url = '<?php echo base_url() . 'college/enrollment/skipEnrollment' ?>';
        ...
        $.ajax({
            ...
            success: function (data) {
                alert('Student Enrolled Officially');
                location.reload();
            }
        });
    }

    function skipEnrollment(btn) {
        var u_id = $('#user_id').val(),
                sy = $('#school_year').val(),
                st_id = $(btn).attr('st-id'),
                base = $("#base").val(),
                sem = $('#semester').val(),
                url = base + "college/enrollment/skipEnrollment";
        $.ajax({
            url: url,
            type: "POST",
            dataType: "JSON",
