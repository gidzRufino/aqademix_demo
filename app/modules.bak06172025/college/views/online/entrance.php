...
<div id="verifyStudentNumber" class="modal fade col-lg-3 col-xs-12" style="margin:10px auto;" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-header clearfix" style="background:#fff;border-radius:15px 15px 0 0; ">
        ...
        if ($elapsed > 0):
            ?>
            <h4 class="text-center text-success" >- ONLINE ENROLLMENT SYSTEM <?php echo $bosy ?> -</h4>
        <?php else: ?>
            ...
            <?php else: ?>
                ...
                <div id="onlineEnrollmentFooter" style="display: none;">
                    <div class="modal-body">
                        ...
                        <div class="form-group" required>
                            <label>Please Select Your Department</label>
                            <select class="form-control" id="departmentEnSelect" onchange="deptSelected(this.value)">
                                <?php
                                switch ($settings->level_catered):
                                    case 0:
                                        ?>
                        ...
                        <div class="form-group" required id="cSem" hidden="">
                            <label>Please Select Semester</label>
                            <select class="form-control" id="semesterEnSelect">
                                <option value="none">Please Select Semester</option>
                                <option value="1" id="fSem">First Semester</option>
                                <option value="1" id="nxtSy">Next School Year</option>
                                <option value="2" id="secSem">Second Semester</option>
                                <option value="3" id="sumSem">Summer</option>
                            </select>
                        </div>
                        <?php 
                        else:
                            if($curdate < $boe):
                                echo '<p>Start of Enrollment will be on ' . Date('F j, Y', strtotime($settings->enrollment_start)) . '</p><p>End of Enrollment Date: ' .  Date('F j, Y', strtotime($settings->enrollment_start)) . '</p>';
                            $aa = 1;
                            endif;
                            if($curdate > $eoe):
                                echo '<b>Sorry, Enrollment period has ended</b>';
                            $aa = 1;
                            endif;
                        endif; ?>
                    </div>
                    <div class="modal-footer">
                        <div class="form-group success">
                            <div class="controls">
                                <button id="requestBtn" onclick="requestEnrollmentEntry()" class="btn btn-info btn-block" aria-hidden="true"  <?php echo ($aa == 1 ? 'disabled=""' : ''); ?>>REQUEST ENTRY</button>
                                <!--<button class="btn btn-danger btn-block" data-dismiss="modal" aria-hidden="true">Cancel</button>-->
                                <div id="resultSection" class="help-block success" ></div>
                               <?php if($aa == 0): ?>
                                <p class="text-center">New Student ? <a href="#" onclick="$('#selectNewOption').modal('show')">[ CLICK HERE ]</a></p>
    ...
    <script type="text/javascript">
        ...
        function verifyOTP()
        {
            var isEnrollment = $('#isEnrollment').val();
            ...
            if (id != "") {
                ...
                if ($("#departmentSelect").val() == "none") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: 'id=' + id + '&department=' + department + '&semester=' + semester + '&isEnrollment=' + isEnrollment + '&csrf_test_name=' + $.cookie('csrf_cookie_name'), // serializes the form's elements.
                        dataType: 'json',
                        beforeSend: function () {
...
//                    alert(semester + ' ' + isEnrollment + ' ' + department);
                    $.ajax({
                        type: "POST",
                        url: url,
                        data: 'id=' + id + '&department=' + department + '&semester=' + semester + '&isEnrollment=' + isEnrollment + '&csrf_test_name=' + $.cookie('csrf_cookie_name'), // serializes the form's elements.
