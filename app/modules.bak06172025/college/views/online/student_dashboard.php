<?php
$admInfo = Modules::run('opl/college/getAdmissionInfo', $this->session->details->st_id, $this->session->semester, $this->session->school_year);
...
if ($admInfo->status == 1):
    ...
    switch (TRUE):
        ...
        case $this->session->details->school_year != $this->session->school_year && $this->session->details->status == 0:
            ...
            break;
        case $this->session->details->school_year == $this->session->school_year:
            if ($this->session->details->status == 6):
        ...
        <div style="background: #fff; border-radius:0 0 15px 15px ; padding: 5px 10px 10px; overflow-y: scroll">  
            ...
            <div class="modal-footer clearfix" id="confirmGrp">
                ...
                <div class="col-lg-6 col-md-12 col-xs-12">
                    <button onclick="submitEnrollmentData()" style="margin: 0 auto; display: none;" id="confirmBtn" class="btn btn-small btn-success btn-block">CONFIRM</button><br />
                    <button style="margin: 0 auto" onclick="document.location = '<?php echo base_url('entrance') ?>'" class="btn btn-small btn-danger btn-block">CANCEL</button>
                </div>
            </div>
    ...
    <input type="hidden" id="currentStatus" value="<?php echo $this->session->status ?>" />
    <input type="hidden" id="alreadyListed" value="<?php echo $alreadyListed ?>" />
    <input type="hidden" id="adm_id" value="<?php echo $this->session->details->admission_id ?>" />


    <div id="confirmEnrollment" class="modal fade col-lg-6 col-xs-12" style="margin:15px auto;" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-header clearfix alert-danger" style="border-radius:15px 15px 0 0; ">
            <h4 class="pull-left">Enrollment Confirmation</h4>
        </div>
        <div class="modal-body" style=" background: #fff;">
            <div id="confirmAlert" class="alert alert-info alert-dismissable">
                Are you sure you want to enroll this Semester? You may be required to pay a minimum amount of ₱ 1,000.00, a non-refundable matriculation fee.
                <div class="float-right">
                    <button onclick="confirmEnrolment('<?php echo $this->session->semester ?>')" class="btn btn-success btn-xs pull-right" style="margin-left: 5px;">YES</button>
                    <button onclick="document.location = '<?php echo base_url('entrance') ?>'" class="btn btn-danger btn-xs pull-right">NO</button>&nbsp;
                </div>
            </div>
    ...
    <div id="dpModal" class="modal fade col-lg-6 col-xs-12" style="margin:15px auto;" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-header clearfix alert-danger" style="border-radius:15px 15px 0 0; ">
            <h4 class="pull-left">Finance</h4>
        </div>
        <div id="dpModalBody" class="modal-body" style=" background: #fff; min-height: 185px;">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            loadDpModal = function (sem)
            {
                ...
                var url = base + 'college/enrollment/loadDpModal/'; // the script where you handle the form input.
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
            ...
            confirmEnrolment = function (sem)
            {
                ...
                var st_id = $('#studentID').val();
                var user_id = $('#user_id').val();
//                                alert(is_old + ' ' + year_level + ' ' + currentSem);
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        ...
                        $('#confirmAlert').html('System is processing...Thank you for waiting patiently');
                    },
                    success: function (data)
                    {
                        if (data.status == true) {
                            $('#confirmAlert').hide();
                            $('#confirmEnrollment').modal('hide');
                            loadDpModal();
                            $('#dpModal').modal('show');
            ...
            {
                var req_unit = $('#tUnit').val();
                //                var tunits = (parseInt(totalUnits) + parseInt(units));
                var exist = 0;
                var sum = 0;
                ...
                var tunits = sum + parseInt(units);
                totalUnits = tunits;
                //                alert(tunits + ' ' + req_unit);
                if (tunits > req_unit) {
                    exist++;
                    alert('Unit required for the semester has been reached');
                }

                if (exist == 0) {
                    var condition = 0;
                    $('#totalUnits').html(tunits);
                    $('#subjectLoadBody').append(
                            ...
                            '<td class="hasValue l_units" id="units_' + sub_id + '">' + units + '</td>' +
...
//                            alert($(this).attr('id') + ' ' + 'tr_' + sub_id)
            ...
            removeSubject = function (sub_id)
            {
                ...
                var tunits = sum - parseInt($('#units_' + sub_id).html());
            ...
            loadEnrollmentData = function (adm_id = 0, st_id, user_id, justLoaded = 0)
            {
                ...
                if (sem != 0) {
                    ...
                    switch (currentStatus)
                    {
                        ...
                        case "4":
                            ...
                            break;
                        case "5":
                            $('#schedDetails').html('<h5 class="text-center">Your payment is still being confirmed, We will notify you once your payment is finally confirm. Please come back soon.</h5>');
                            break;
                        default:
                            var alreadyListed = $('#alreadyListed').val();
                            var st_id = $('#studentID').val();
                            var course_id = $('#course_id').val();
                            var year_level = $('#year_level').val();
                            var school_year = (alreadyListed ? $('#currentSchoolYear').val() : $('#previous_school_year').val());
                            var base = $('#base').val();
        ...
        function submitEnrollmentData()
        {
            ...
            var user_id = $('#user_id').val();
            var alreadyListed = $('#alreadyListed').val();
            var admission_id = $('#adm_id').val();
            //            alert(admission_id);
            if (alreadyListed) {
                loadEnrollmentData(admission_id, st_id, user_id, 1);
            } else {
                loadEnrollmentData(admission_id, st_id, user_id, 0);
                var url = base + 'college/enrollment/saveCollegeRO/'; // the script where you handle the form input.
                $.ajax({
                    type: "POST",
                    url: url,
                    ...
                    {
                        if (data.status == true) {
                            loadEnrollmentData(data.admission_id, st_id, user_id, 0);
                            console.log(data);
                        } else {
                            alert(data.msg)
                        }
        ...
        function loadSchedule(sub_code, s_id)
        {
            var semester = $('#inputSem').val();
            var school_year = $('#currentSchoolYear').val();
            var base = $('#base').val();
            var url = base + 'college/enrollment/loadSchedule/' + s_id + '/' + semester + '/' + school_year + '/' + sub_code; // the script where you handle the form input.
            //            alert(url);
            $.ajax({
                type: "POST",
                url: url,
