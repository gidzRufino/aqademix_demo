...
//                'm_bdate' => $this->input->post('m_inputBdate'),
            ...
            $msg = 'Your Information is successfully submitted.  You can use this ID #: ' . $st_id . ' for ' . strtoupper($name) . ' to access the system. Login Now to upload the required documents';
...
//     }
    ...
    function financeDetails($isBasicEd = NULL) {
        ...
        if ($this->session->is_logged_in):
            $data = array(
                ...
                'pads' => $this->enrollment_model->fetchPadala(1),
                'main_content' => $content,
                'settings' => Modules::run('main/getSet')
            );
    ...
    public function verifyOTP() {
        ...
        $fetched_user = $this->enrollment_model->fetch_otp($id, $settings->school_year);
        ...
        if ($password_matched) {
            if ($admStat->row()->status == 1 && $usrAcc->num_rows() > 0 && $isEnrollment == 0):
                echo json_encode(array('status' => TRUE, 'url' => base_url('college/enrollment/entrance/' . base64_encode($id) . '/' . $department . '/MA==/' . $isEnrollment . '/' . base64_encode($usrAcc->row()->secret_key) . '/' . base64_encode($id . $department))));
            else:
        ...
        if ($result->status):
            if ($admStat->row()->status == 1 && $usrAcc->num_rows() > 0 && $isEnrollment == 0):
                $jsonResponse = array(
                    ...
                    'contact_num' => '1234',
                    'url' => base_url('college/enrollment/entrance/' . base64_encode($idNumber) . '/' . $department . '/' . base64_encode($this->post('semester')) . '/' . $isEnrollment . '/' . base64_encode($usrAcc->row()->secret_key))
                );

                echo json_encode($jsonResponse);
            else:
                $otp = $this->requestOTP($idNumber, $settings->school_year);
                if ($department == 5):
                ...
                $jsonResponse = array('status' => TRUE,
                    ...
                    'url' => base_url('college/enrollment/entrance/' . base64_encode($idNumber) . '/' . $department . '/' . base64_encode($this->post('semester')) . '/' . $isEnrollment));
    ...
    function verifyPassword() {
        ...
        if ($res):
            if ($department == 5): //college
                ...
                $details = $this->enrollment_model->getBasicEducationDetails($stid);
                $course_id = 0;
                $school_year = ($semester == 3 ? ($settings->school_year - 1) : $settings->school_year);
                $isCollege = FALSE;
                $isOld = $this->enrollment_model->isPreviouslyEnrolled($stid, $school_year, $isCollege);
