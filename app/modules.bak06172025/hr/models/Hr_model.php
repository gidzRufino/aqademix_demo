...
class Hr_model extends CI_Model
{
    ...
    function getEmployeeName($id)
    {
        ...
        $this->db->join('department', 'department.dept_id = profile_position.position_dept_id', 'left');
        $this->db->where('employee_id', $id);
        $q = $this->db->get();
        return $q->row();
    }
    ...
    function updateTime($column, $date, $time, $rfid, $stid)
    {
        $month = strtolower(date('M', strtotime($date)));

        $this->db->where('date', $date);
        $this->db->where('att_st_id', $stid);
        // $this->db->or_where('u_rfid', $rfid);
        ...
        if ($q->num_rows() > 0):
            ...
            $this->db->where('att_st_id', $stid);
            // $this->db->or_where('u_rfid', $rfid);
            $this->db->update('attendance_sheet_' . $month, array($column => $time));
        else:
            $d = array(
                'att_id'    => $this->eskwela->codeCheck('attendance_sheet_' . $month, 'att_id', $this->eskwela->code()),
                'att_st_id' => $stid,
                'u_rfid'    => $rfid,
                $column     => $time,
            ...
            $this->db->insert('attendance_sheet_' . $month, $d);
        endif;
