...
class vm extends MX_Controller {
	...
	public function index(){
        if(!$this->session->userdata('is_logged_in')){
            ...
            $data['vlog'] = $this->vm_model->display_log($this->session->school_year);
    ...
    public function status()
    {
        if(!$this->session->userdata('is_logged_in')){
            ...
            $data['vid'] = $this->vm_model->vm_get_vid($this->session->school_year);
    ...
    public function timeline()
    {
        if(!$this->session->userdata('is_logged_in')){
            ...
            $data['log'] = $this->vm_model->display_log($this->session->school_year);
    ...
    public function id_status()
    {
        if(!$this->session->userdata('is_logged_in')){
            ...
            $data['vinfo'] = $this->vm_model->display_log_vid(base64_decode($vid), $sy);
            $data['pinfo'] = $this->vm_model->display_vid_profile(base64_decode($vid), $sy);
    ...
    public function vm_status()
    {
        if(!$this->session->userdata('is_logged_in')){
            ...
            $sy = $this->session->school_year;
            $data['minfo'] = $this->vm_model->display_log_vm(base64_decode($vm), $sy);
            $data['pminfo'] = $this->vm_model->display_vm_profile(base64_decode($vm), $sy);
    ...
    public function register()
    {
        if(!$this->session->userdata('is_logged_in')){
            ...
            $sy = $this->session->school_year;
            $data['vm_accounts'] = $this->vm_model->vm_accounts($sy);
            $data['vm_dept'] = $this->vm_model->vm_dept();
            $data['vm_rfid'] = $this->vm_model->vm_get_id($sy);
