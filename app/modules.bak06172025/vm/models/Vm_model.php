...
class Vm_model extends CI_Model {
   ...
   function vm_get_vid($sy)
   {
      ...
      $this->db->join('vm_accounts', 'profile.stats = vm_accounts.va_id', 'left');
      $this->db->where('profile.account_type', $account_type);
      $query = $this->db->get();
      return $query->result();
   }

   function display_log_vm($vm, $sy)
   {
      // $sy = 2016;
      $this->db = $this->eskwela->db($sy);
      $this->db->select('*');
   ...
   function display_vm_profile($vm, $sy)
   {
      // $sy = 2016;
      $this->db = $this->eskwela->db($sy);
      $this->db->select('*');
   ...
   function display_log_vid($uid, $sy)
   {
      // $sy = 2016;
      $this->db = $this->eskwela->db($sy);
      $this->db->select('*');
   ...
   function display_vid_profile($uid, $sy)
   {
      // $sy = 2016;
      $this->db = $this->eskwela->db($sy);
      $this->db->select('*');
