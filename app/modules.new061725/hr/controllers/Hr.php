...
class hr extends MX_Controller
{
    ...
    function getNumberOfDaysWork($startDate, $endDate)
    {
        ...
        foreach ($daterange as $date) {
            ...
            if (date('D', strtotime($date)) != 'Sun' && date('D', strtotime($date)) != 'Sat'):
                $isHoliday = Modules::run('calendar/isHoliday', $date);
                if (!$isHoliday):
                    $days++;
                endif;
            endif;
        ...
        //     $date = date('Y-m-d', strtotime(date('Y-' . $m . '-' . $totalDays)));
        //     echo $date . ' ' . date('D', strtotime($date)) . '<br>';
        //     if (date('D', strtotime($date)) != "Sun" && date('D', strtotime($date)) != "Sat"):
        //         $isHoliday = Modules::run('calendar/isHoliday', $date);
        //         if (!$isHoliday):
        //             $days++;
    ...
    function editHrTime()
    {
        ...
        $e = $this->hr_model->updateTime($column, $newDate, $time, $rfid, $st_id);
...
?>
    ...
    function getAllEmployee($option = NULL, $page = null)
    {
        ...
        $data['links'] = $this->pagination->create_links();
        $data['leave'] = $this->payroll_model->fetchLeaveList(null);
    ...
    public function saveProfile()
    {
        ...
        $this->hr_model->addEducHis($details, $st_id, null, null);
    ...
    function searchDTRbyDateForPayroll()
    {
        ...
        $data['hRate'] = $this->input->post('hRate');
        ...
        $data['info'] = $this->getEmployee($this->input->post('emp_id'));
        $data['hrdb'] = Modules::load('hr/hrdbprocess/');
        ...
        ?>
            ...
            <ol>
                ...
                ?>
                    ...
                    <h5>
                        <li><input style="margin:0 10px;" type="checkbox" value="<?php echo $assoc->dh_id ?>" /><?php echo $assoc->lastname . ', ' . $assoc->firstname ?></li>
                    </h5>
    ...
    <?php
        ...
        function deleteAttendance()
        {
            ...
            $date = $this->post('date');
            $q = $this->hr_model->deleteAttendance($att_id, $date);

            if ($q):
                echo json_encode(array('status' => TRUE, 'msg' => 'Attendance Successfuly Deleted'));
            else:
