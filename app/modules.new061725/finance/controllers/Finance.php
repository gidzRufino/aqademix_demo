...
//        $plan_id = $this->finance_model->addPlan($plan, $course_id, 0);
    ...
    function paymentVsCollectibleGraph() {
        ...
        for ($x = $st; $x <= $en; $x++):
            ...
            array_push($data2, [$x, $toCollect]);
            array_push($details ,[$x , ($r->row()->amount != '' ? $r->row()->amount : 0)]);
            array_push($tr, [$x, $this->getMonth($n)]);
        endfor;
        $data['details'] = json_encode($details);
        $data['data2'] = json_encode($data2);
        $data['tr'] = json_encode($tr);
        $data['max'] = $collectibles + 5000;
